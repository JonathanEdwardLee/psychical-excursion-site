# PEx Chapter 6 Work Order — Build the Current.
...
`JonathanEdwardLee/psychical-excursion-site`

Start from accepted main:

`978fbcbaa2fc0c162c75fdcdd4fac98061249f65`
...
Chapter 6 is founder-approved for implementation:

**Build the Current.**

CloudDev is the implementation worker. PEX_PRIMARY owns research conclusions, manuscript voice, and source acceptance.

Implement the supplied prose faithfully. Do not rewrite, condense, embellish, or originate guidebook prose.

## Objective

Publish Chapter 6 and preserve the guidebook architecture already established.

Acceptance criteria:

1. Chapter 6 is available at a dedicated public route.
2. Chapter 5 links forward to Chapter 6.
3. Chapter 6 links back to Chapter 5 through the existing previous-chapter arrow pattern.
4. The existing **Relax the body** deep-link convention continues to resolve to Chapter 4 **A Nighttime Body Release**.
5. Chapter 6 renders the established **Summary / Experiment / Intention** component immediately before References.
6. Existing header, Sky Clock, Chapter 5 attention instrument, typography, reading width, section reveals, footer, theme behavior, and previous/next navigation patterns remain intact.
7. No held feature is exposed.
8. No new unsupported scientific or metaphysical claims are added by implementation.
...
`src/content/guidebookChapter06.source.md`

Use the existing manuscript-driven chapter architecture.

Add Chapter 5's forward reading link:

**Build the Current. →**

Chapter 6 has no next-reading link yet.

## Navigation

Preserve the existing previous-chapter control.

Chapter 6 previous destination:

**Move Your Attention.**

Do not add a chapter menu or index.

Do not alter the locked header to implement navigation.

## Relaxation deep link

The manuscript contains:

`[**Relax the body**](#/feel-the-body#nighttime-body-release)`

Preserve this exact behavior.

It must land at the Chapter 4 **A Nighttime Body Release** section, not the top of Chapter 4.

Use the existing reusable rich-text/deep-link implementation.

Do not create a second competing link mechanism.

## Visual direction

Chapter 6 does **not** require a new major visual experiment.

Do not invent one.

The Chapter 5 geometric attention instrument remains available in Chapter 5 and should not be duplicated into Chapter 6 unless the manuscript explicitly calls it.

A restrained ambient variation may reuse the existing guidebook ambient system only if this falls naturally out of the current architecture and requires no redesign. Otherwise preserve the existing default/orbit treatment.

No new visual dependency.

## Manuscript fidelity

Use the exact approved manuscript below.
...
- claims that qi, prana, meridians, nadis, vagal tone, nerve electricity, or any other model are equivalent;
- claims that a body sensation proves a subtle-energy pathway;
- unsupported medical or therapeutic claims.

Traditional models may be presented only as they are presented in the manuscript.
...
Preserve the existing scroll-to-top behavior for chapter navigation.

### Practice component

Render the manuscript's:

- Summary
- Experiment
- Intention

inside the existing reusable chapter practice component.

Do not create a Chapter 6-specific visual treatment.

### Cross-links

The explicit **Relax the body** link must resolve through the existing Chapter 4 relaxation deep-link behavior.

Do not globally auto-link arbitrary occurrences of “relax” or “body.”

### References

Preserve numbered inline citations and clickable source/DOI behavior using the existing guidebook reference renderer.

Do not rewrite source citations during implementation.
...
- semantic headings;
- keyboard focus;
- narrow/mobile readability;
- reduced-motion compatibility;
- section reveals;
- previous/next navigation accessibility;
- citation navigation;
...
- logo treatment;
- Sky Clock;
- Chapter 5 attention instrument;
- core typography;
- reading width;
- footer;
- theme behavior;
- established reveal behavior;
- previous-arrow visual grammar;
...
No new dependency unless strictly required; stop and report first if one seems necessary.

## Verification

Add/update tests covering at minimum:

- Chapter 6 manuscript parses;
- Chapter 6 title and route;
- Chapter 5 → Chapter 6 next link;
- Chapter 6 → Chapter 5 previous link;
- Chapter 6 starts at top after ordinary chapter navigation;
- Chapter 6 references render;
- Chapter 6 has exactly one Summary / Experiment / Intention component;
- **Relax the body** deep link resolves to Chapter 4 nighttime relaxation;
- Chapter 5 attention instrument remains unchanged;
- Chapters 1–5 previous/next behavior remains intact;
- reduced-motion readability remains intact;
...
- `npm test`
- `npm run build`
- existing canonical/release/privacy checks used by CI where applicable.

## Stop conditions

Stop and report rather than widening scope if:

- implementing Chapter 6 requires rewriting shared manuscript architecture materially;
- any manuscript statement appears technically impossible to render without altering prose;
- a new dependency/service appears necessary;
- the Chapter 4 deep link would need a competing second mechanism;
- implementation would require changing the locked header;
- held app features would become public.

## Return to PEX_PRIMARY

Open a new implementation PR against `main`.
...
3. files changed;
4. Chapter 6 route;
5. Chapter 5 forward-link behavior;
6. Chapter 6 back-arrow behavior;
7. relaxation deep-link verification;
8. Summary / Experiment / Intention rendering verification;
9. tests/typecheck/lint/build results;
10. desktop/mobile verification if available;
11. any concern requiring PEX_PRIMARY input.
