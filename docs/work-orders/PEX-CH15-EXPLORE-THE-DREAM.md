# PEx Chapter 15 Work Order — Explore the Dream.

## Baseline

Implement from accepted main:

`3e9034d9744eac4674df7f2a79a955b80cd308e2`

Chapter 14 **Stabilize the Dream.** is the latest accepted chapter.

Jonathan approved continuing into the next pass on 2026-09-24.

## Objective

Publish Chapter 15 **Explore the Dream.** as the next guidebook chapter without changing the locked public shell or widening D-081 analytics.

Central teaching:

**Explore before you explain. Investigate dream-generated experience without treating it as automatically verified information about waking reality.**

Keep observation, influence, memory, expectation, interpretation and external verification distinct.
...
- Preserve the accepted manuscript wording except for renderer-required markup and reference normalization that do not change visible meaning.
- Use the established chapter source/parser/renderer architecture.
- Add route/hash for Chapter 15 **Explore the Dream.**
- Chapter 14 next-reading must point to Chapter 15.
- Chapter 15 previous navigation must point to Chapter 14.
- Chapter 15 is the newest published chapter, so do not add a Chapter 16 next link yet.
- Render **Summary / Experiment / Intention** in the established reusable practice component immediately before References.
- Preserve experiment title **One Question** and subheadings **Observe first**, **Ask**, **Separate surprise from interpretation**, **Check memory**, **External verification**, **Problem exploration**.
- Preserve all ten references using the established reference convention.
- No chapter-specific interactive visual is required.
- Preserve compact header, observational Sky Clock, Light/Dark, responsive reading width, whole-section reveals, reduced-motion support, previous arrow, editorial next-reading pattern, Chapter 5 geometric attention instrument, Chapter 4 deep link `#/feel-the-body#nighttime-body-release`, and D-081 GA4.
- Public chapter text must contain no internal **PEx** wording.
- Keep exploration separate from dream control.
- Keep dream-generated content separate from externally verified facts.
- Do not present dream characters as proven independent consciousnesses.
- Do not present dream incubation or problem-solving findings as paranormal evidence.
- Do not reopen fixed 60-day framing, Google sign-in, Dream Journal, voice, Drive, Calendar, progress/checkmarks/streaks, Days index, separate Astronomy page, ads, payments/donations, outreach, credentials, or new recurring cost.
- Do not widen GA4 beyond D-081.

## Verification

Cover at minimum:
- Chapter 15 title, route, hash and parser;
- Chapter 14 → Chapter 15 next navigation;
- Chapter 15 → Chapter 14 previous navigation;
- no Chapter 16 next link yet;
- one Summary/Experiment/Intention practice component;
- **One Question** plus six experiment subheadings;
- ten reference anchors;
- no internal “PEx” wording in rendered Chapter 15;
- no dream-content-as-external-fact language introduced by implementation;
- GA4 regression unchanged and privacy-bounded;
- all non-analytics held features remain absent;
- Chapter 4 deep link and Chapter 5 instrument remain intact;
- all prior chapter routes/navigation remain intact.

Run the full repository verification suite used by CI:
- `npm test`
- `npm run typecheck`
- `npm run lint`
- `npm run build`
- release/canonical/brand/privacy checks.

## Return

CloudDev should open a separate implementation PR and report:
