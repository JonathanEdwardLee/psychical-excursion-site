# PEx CloudDev Work Order — Chapter 7: Quiet the Mind.

## Authority and baseline

Founder-approved manuscript. Implement from accepted main:

`426a02acda0533d9d4f09a10bfb5218575233f47`

Do not merge. Open a separate implementation PR against `main` for PEX_PRIMARY source review.

## Objective

Publish the next guidebook chapter, **Quiet the Mind.**, preserving the established guidebook reading flow and visual grammar.

## Required route and navigation

- Route: `#/quiet-the-mind`
- Chapter 6 **Build the Current.** next-reading link becomes **Quiet the Mind. →**
- Chapter 7 previous-chapter arrow returns to **Build the Current.**
- Chapter 7 has no next-reading link yet.
- Preserve the established **Relax the body** deep link to `#/feel-the-body#nighttime-body-release`.

## Manuscript rule

The manuscript below is founder-approved and authoritative for this implementation. Do not rewrite, summarize, shorten, expand, or substitute wording.

Public-book authoring rule: do not insert internal product language such as “PEx,” “PEx method,” governance language, approval/disapproval language, or editorial policy commentary into the manuscript.
...
- Preserve the reusable **Summary / Experiment / Intention** component immediately before References.
- Preserve numbered reference rendering and external source links.
- No new major chapter-specific visual/instrument is requested for Chapter 7.
- Preserve the existing compact header, Sky Clock, typography, reading width, footer, Light/Dark behavior, reveal behavior, accessibility/focus behavior, and reduced-motion support.
- Do not alter the founder-accepted Chapter 5 attention instrument.
...
- Chapter 7 title, route, hash, parser, practice block, references.
- Chapter 6 → Chapter 7 navigation.
- Chapter 7 → Chapter 6 previous navigation.
- No Chapter 8 next link yet.
- Relax-the-body deep link remains exact.
- No held features appear.
- Existing guidebook/public tests remain passing.

Run the complete repository verification suite used by CI, including:
- `npm test`
- `npm run typecheck`
- `npm run lint`
- `npm run build`
- release/canonical/brand/privacy checks defined by the repository CI workflow.

## Return to PEX_PRIMARY

Open a separate implementation PR against `main` and report:
1. PR number and URL
2. implementation branch
3. implementation head SHA
4. baseline SHA
5. changed files
6. route/navigation behavior
7. manuscript preservation result
8. Relax-the-body deep-link result
9. Summary/Experiment/Intention + references result
10. exact verification commands and outcomes
11. any unresolved issue or stop condition
