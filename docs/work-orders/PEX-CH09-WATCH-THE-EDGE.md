# CloudDev Work Order — Chapter 9: Watch the Edge.

## Authority and baseline

Founder-approved manuscript. Implement from accepted implementation main:

`6fce2abff4644fd2a42985f35aa3403874bb22ef`

Implement in a separate implementation PR against `main`. Preserve manuscript wording. Routine plan-conforming implementation is standing-approved after Primary source review PASS; do not merge yourself.

## Objective

Publish **Watch the Edge.** as the next guidebook chapter following **See the Image.**

## Required route and navigation

- Route: `#/watch-the-edge`
- Chapter 8 **See the Image.** next-reading link becomes **Watch the Edge. →**
- Chapter 9 previous-chapter arrow returns to **See the Image.**
- Chapter 9 has no next-reading link yet.
- Where the visible phrase **Relax the body** appears in the Experiment, link that visible phrase to the established `#/feel-the-body#nighttime-body-release` target without changing the visible manuscript wording.

## Public-book language rule

Do not insert internal project/product/governance vocabulary into guidebook prose. Never add “PEx,” “PEx method,” Council/Primary/CloudDev language, approval/disapproval framing, or implementation-policy commentary.

The public book presents researched knowledge, traditional/historical methods, first-person experience where supplied, and experiments for readers to investigate.
...
Begin with **Relax the body** if that is useful, then allow your breathing to continue naturally.
...
- Preserve manuscript verbatim except renderer-required link markup that does not alter visible wording.
- Use established chapter source/parser/renderer architecture.
- Render **Summary / Experiment / Intention** in the reusable practice component immediately before References.
- Preserve numbered/linkable references and source links where provided by existing manuscript conventions.
- Preserve compact header, Sky Clock, typography, reading width, footer, Light/Dark behavior, reveal behavior, accessibility/focus behavior and reduced-motion support.
- Preserve the Chapter 5 attention instrument.
- No new major chapter-specific interactive visual is required.
- Do not reopen held product features.
- No analytics, payments, ads, sign-in, Journal, voice capture, Drive/Calendar, progress/streaks, Days index, Astronomy page, fixed 60-day structure, new recurring cost, credentials or unrelated scope.

## Tests

Add/update tests covering at minimum:
- title, route, hash and parser for Chapter 9;
- Chapter 8 → Chapter 9 next navigation;
- Chapter 9 → Chapter 8 previous navigation;
- no Chapter 10 next link yet;
- one Summary/Experiment/Intention practice component;
- 14 references with stable reference anchors;
- exact **Relax the body** href;
- no internal “PEx” wording in rendered Chapter 9 text;
- held features remain absent;
- prior chapter routes/navigation remain intact.

Run the complete repository verification suite used by CI:
- `npm test`
- `npm run typecheck`
- `npm run lint`
- `npm run build`
- release/canonical/brand/privacy checks defined by the repo.

## Return on PR

Report implementation PR, branch, head SHA, baseline SHA, changed files, navigation, manuscript preservation, practice/reference rendering, exact Relax-the-body link result, verification commands/results, and unresolved issues. Do not merge.