# CloudDev Work Order — Chapter 8: See the Image.

## Authority and baseline

Founder-approved manuscript. Implement from accepted implementation main:

`83b9104bab506279a0837140552fecc715e53802`

CloudDev is subscribed to the controlling PR workflow. Implement in a separate implementation PR against `main`. Do not rewrite the manuscript. Routine plan-conforming implementation is standing-approved after Primary source review PASS; do not merge yourself.

## Objective

Publish the next guidebook chapter, **See the Image.**, as the continuation after **Quiet the Mind.**

## Required route and navigation

- Route: `#/see-the-image`
- Chapter 7 **Quiet the Mind.** next-reading link becomes **See the Image. →**
- Chapter 8 previous-chapter arrow returns to **Quiet the Mind.**
- Chapter 8 has no next-reading link yet.

## Public-book language rule

Do not insert internal project/product/governance vocabulary into guidebook prose. In particular, never add “PEx,” “PEx method,” Council/Primary/CloudDev language, approval/disapproval framing, or implementation policy commentary.

The guidebook presents researched knowledge, historical/traditional methods, personal experience, and experiments for the reader to investigate.
...
- Preserve the manuscript verbatim except for renderer-required link markup that does not change visible wording.
- Use the established chapter source/parser/renderer architecture.
- Render **Summary / Experiment / Intention** in the existing reusable practice component immediately before References.
- Preserve numbered/linkable references and external links.
- No new major interactive visual is required for this chapter unless existing architecture needs a minimal non-content change to support the approved manuscript.
- Preserve compact header, Sky Clock, typography, line length, footer, Light/Dark behavior, reveal behavior, focus/accessibility and reduced-motion support.
- Preserve the founder-accepted Chapter 5 attention instrument.
- Do not reopen held product features.
- No new analytics, payments, ads, sign-in, Journal, voice capture, Drive/Calendar, progress/streaks, Days index, Astronomy page, fixed 60-day structure, new cost, credentials, or unrelated work.

## Tests

Add/update tests covering at minimum:
- title, route, hash and parser for Chapter 8;
- Chapter 7 → Chapter 8 next navigation;
- Chapter 8 → Chapter 7 previous navigation;
- no Chapter 9 next link yet;
- one Summary/Experiment/Intention practice component;
- nine references and stable reference anchors;
- no internal “PEx” wording in rendered Chapter 8 text;
- held features remain absent;
- previous chapter routes remain intact.
...
- `npm run lint`
- `npm run build`
- repository release/canonical/brand/privacy checks.

## Return on PR

Report:
1. implementation PR URL/number
2. branch
3. head SHA
4. baseline SHA
5. changed files
6. route/navigation result
7. manuscript preservation result
8. practice-component/reference result
9. exact verification outcomes
10. unresolved issues or stop conditions

Do not invent new manuscript content.