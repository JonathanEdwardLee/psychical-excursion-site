...
`,Xn="nighttime-body-release",Jo="A Nighttime Body Release",Dn="relax-the-body",Ko="Relax the body",Pn=`#/feel-the-body#${Xn}`,Zn="pex:attention-instrument",Qo={[Jo]:Xn,References:"references"};function Xo(n){return Qo[n]}const Zo={[Dn]:{id:Dn,href:Pn,label:Ko}};function et(n=window.location.hash){const t=n.replace(/^#/,"").split("?")[0]||"/",[a,o=""]=t.split("#");return{path:(a&&a.startsWith("/")?a:`/${a??""}`)||"/",fragment:o}}const nt="You Are Dreaming. Remember.",tt="/you-are-dreaming-remember",va=`#${tt}`,On=["Summary","Experiment","Intention"];function Ta(n){return On.includes(n)}function Fn(n){const e=n.match(/^#{2,3}\s+(.*)$/);if(!e)return null;const t=e[1].trim();return Ta(t)?t:null}function ka(n){const e=n.trim();return!e||e==="---"||n.startsWith("## ")||n.startsWith("### ")||n.startsWith("> ")||n.trim()===Zn||/^\d+\.\s/.test(n)||/^-\s/.test(n)}function H(n){const e=n.replace(/\r\n/g,`
`).trim().split(`
`);if(!e[0]?.startsWith("# "))throw new Error("Chapter manuscript must begin with a top-level heading");const t=e[0].slice(2).trim(),a=[],o=[];let i=1,r=!1;for(;i<e.length;){const s=e[i];if(r){const c=s.trim();c&&o.push(c),i+=1;continue}if(!s.trim()){i+=1;continue}if(s.trim()==="---"){a.push({kind:"rule"}),i+=1;continue}if(s.trim()===Zn){a.push({kind:"attention"}),i+=1;continue}if(Fn(s)){const c=ni(e,i);a.push(c.block),i=c.nextIndex,i<e.length&&e[i].trim()==="---"&&(i+=1);continue}if(s.startsWith("## ")){const c=s.slice(3).trim();if(c==="References"){r=!0,i+=1;continue}a.push({kind:"heading",text:c}),i+=1;continue}if(s.startsWith("> ")){const c=[];for(;i<e.length&&e[i].startsWith("> ");)c.push(e[i].slice(2).trim()),i+=1;a.push({kind:"quote",text:c.join(" ")});continue}if(/^\d+\.\s/.test(s)){const c=[];for(;i<e.length&&/^\d+\.\s/.test(e[i]);)c.push(e[i].replace(/^\d+\.\s+/,"").trim()),i+=1;a.push({kind:"list",items:c,ordered:!0});continue}if(/^-\s/.test(s)){const c=[];for(;i<e.length&&/^-\s/.test(e[i]);)c.push(e[i].replace(/^-\s+/,"").trim()),i+=1;a.push({kind:"list",items:c,ordered:!1});continue}const h=[];for(;i<e.length;){const c=e[i];if(ka(c)&&c.trim()!==""||!c.trim())break;h.push(c.trim()),i+=1}const d=h.join(" ");d.startsWith("**")&&d.endsWith("**")&&!d.slice(2,-2).includes("**")?a.push({kind:"emphasis",text:d.slice(2,-2)}):d&&a.push({kind:"paragraph",text:d})}return{title:t,blocks:a,references:o}}function ei(n,e,t){const a=[];let o=e;for(;o<n.length;){const i=n[o];if(t(i))break;if(!i.trim()){o+=1;continue}if(i.trim()==="---"){a.push({kind:"rule"}),o+=1;continue}if(i.trim()===Zn){a.push({kind:"attention"}),o+=1;continue}if(i.startsWith("### ")){const l=i.slice(4).trim();if(!Ta(l)){a.push({kind:"subheading",text:l}),o+=1;continue}}if(i.startsWith("> ")){const l=[];for(;o<n.length&&n[o].startsWith("> ");)l.push(n[o].slice(2).trim()),o+=1;a.push({kind:"quote",text:l.join(" ")});continue}if(/^\d+\.\s/.test(i)){const l=[];for(;o<n.length&&/^\d+\.\s/.test(n[o]);)l.push(n[o].replace(/^\d+\.\s+/,"").trim()),o+=1;a.push({kind:"list",items:l,ordered:!0});continue}if(/^-\s/.test(i)){const l=[];for(;o<n.length&&/^-\s/.test(n[o]);)l.push(n[o].replace(/^-\s+/,"").trim()),o+=1;a.push({kind:"list",items:l,ordered:!1});continue}const r=[];for(;o<n.length;){const l=n[o];if(t(l)||!l.trim()||ka(l)&&l.trim()!=="")break;r.push(l.trim()),o+=1}const s=r.join(" ");s.startsWith("**")&&s.endsWith("**")&&!s.slice(2,-2).includes("**")?a.push({kind:"emphasis",text:s.slice(2,-2)}):s&&a.push({kind:"paragraph",text:s})}return{blocks:a,nextIndex:o}}function ni(n,e){const t=[];let a=e;for(const o of On){if(Fn(n[a]??"")!==o)throw new Error(`Practice component must use ${On.join(", then ")} headings`);a+=1;const r=ei(n,a,s=>!!(s.startsWith("## ")||Fn(s)||s.trim()==="---"));t.push({label:o,blocks:r.blocks}),a=r.nextIndex}return{block:{kind:"practice",parts:t},nextIndex:a}}let nn=null;function ti(){return nn||(nn=H(Vo)),nn}const ai=`
...
`,oi="An experiment in dreams, consciousness, energy, and out-of-body experience";function ii(n){const e=n.trim(),t=e.match(/^#\s+(.+)\n+/);if(!t)throw new Error("Guidebook manuscript must begin with a top-level heading");const a=t[1].trim(),i=e.slice(t[0].length).split(/\n## /),r=Ft(i[0]??""),s=[];let l=[];for(let h=1;h<i.length;h+=1){const d=i[h],c=d.indexOf(`
`),g=(c===-1?d:d.slice(0,c)).trim(),b=c===-1?"":d.slice(c+1);if(g==="References"){l=ri(b);continue}s.push({heading:g,paragraphs:Ft(b)})}return{openingHeading:a,openingParagraphs:r,sections:s,references:l}}function Ft(n){return n.split(/\n---\n/).flatMap(e=>e.split(/\n\n+/)).map(e=>e.replace(/\n/g," ").trim()).filter(Boolean)}function ri(n){return n.split(/\n+/).map(e=>e.trim()).filter(Boolean)}let tn=null;function si(){return tn||(tn=ii(ai)),tn}function Lt(n){return n.replace(/[.,;:!?)]+$/u,"")}function ze(n,e={}){const{linkCitations:t=!0}=e,a=m("span",{class:"guidebook-rich-text"}),o=t?/(\[[^\]]+\]\((?:pex:[a-z0-9-]+|#\/[^)\s]+)\)|\*\*[^*]+\*\*|\*[^*]+\*|\[\d+\]|https:\/\/\S+|doi:10\.\S+)/gi:/(\[[^\]]+\]\((?:pex:[a-z0-9-]+|#\/[^)\s]+)\)|\*\*[^*]+\*\*|\*[^*]+\*|https:\/\/\S+|doi:10\.\S+)/gi;let i=0;for(const r of n.matchAll(o)){const s=r.index??0;s>i&&a.append(n.slice(i,s));const l=r[0],h=l.match(/^\[([^\]]+)\]\((pex:[a-z0-9-]+|#\/[^)\s]+)\)$/i);if(h){const d=h[1],c=h[2],g=c.toLowerCase().startsWith("pex:")?c.slice(4).toLowerCase():"",b=g?Zo[g]:void 0,f=b?.href??(c===Pn||c.endsWith(`#${Xn}`)?Pn:c),T=m("a",{href:f,class:"guidebook-pex-link","data-pex-link":b?.id??Dn}),M=d.match(/^\*\*(.+)\*\*$/);M?T.append(m("strong",{},[M[1]])):T.append(d),a.append(T)}else if(l.startsWith("**"))a.append(m("strong",{},[l.slice(2,-2)]));else if(l.startsWith("*"))a.append(m("em",{},[l.slice(1,-1)]));else if(/^\[\d+\]$/.test(l)&&t){const d=l.slice(1,-1);a.append(m("a",{href:`#ref-${d}`,class:"guidebook-citation"},[l]))}else if(l.toLowerCase().startsWith("doi:10.")){const d=Lt(l.slice(4));a.append(m("a",{href:`https://doi.org/${d}`,class:"guidebook-doi",rel:"noreferrer"},[l]))}else if(l.toLowerCase().startsWith("https://")){const d=Lt(l);a.append(m("a",{href:d,class:"guidebook-external",rel:"noreferrer"},[l]))}else a.append(l);i=s+l.length}return i<n.length&&a.append(n.slice(i)),a.childNodes.length||a.append(n),a}function Ln(n,e){const t=m("p",{});return t.append(ze(n,e)),t}function Ia(n){const e=n.match(/^(?:\*\*)?\[(\d+)\](?:\*\*)?\s*(.*)$/),t=e?.[1]??"0",a=e?.[2]??n,o=m("li",{id:`ref-${t}`,class:"guidebook-reference-item"}),i=m("span",{class:"guidebook-ref-num"},[`[${t}] `]);return o.append(i,ze(a,{linkCitations:!1})),o.tabIndex=-1,o}function li(n){for(const e of n.querySelectorAll("a.guidebook-citation"))e.addEventListener("click",t=>{const o=(e.getAttribute("href")??"").match(/^#ref-(\d+)$/);if(!o)return;const i=n.querySelector(`#ref-${o[1]}`);if(!i)return;t.preventDefault();const r=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;i.scrollIntoView({behavior:r?"auto":"smooth",block:"start"}),i.focus({preventScroll:!0})});for(const e of n.querySelectorAll("a.guidebook-pex-link"))e.addEventListener("click",t=>{const a=e.getAttribute("href")??"",o=a.startsWith("#")?a:"";if(!o.includes("#",1)&&!o.includes("#"))return;const i=o.replace(/^#/,"").split("#")[1];if(!i)return;const r=n.querySelector(`[id="${i}"]`);if(!r)return;t.preventDefault(),r.closest(".guidebook-section")?.classList.add("is-visible");const s=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;r.scrollIntoView({behavior:s?"auto":"smooth",block:"start"}),r.focus({preventScroll:!0})})}function Oe(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function hi(n){const e=si(),t=m("article",{class:"guidebook-article"});let a=Oe(!0);t.append(a),a.append(m("p",{class:"guidebook-subtitle"},[oi]),m("h1",{class:"guidebook-opening-title"},[e.openingHeading]),...e.openingParagraphs.map(o=>Ln(o)));for(const o of e.sections){a=Oe(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title"},[o.heading]));for(const i of o.paragraphs){if(i.startsWith("**")&&i.endsWith("**")){a.append(m("p",{class:"guidebook-emphasis-line"},[i.slice(2,-2)]));continue}a.append(Ln(i))}}if(e.references.length){a=Oe(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const o=m("ul",{class:"guidebook-references"});for(const i of e.references)o.append(Ia(i));a.append(o)}a=Oe(!1),t.append(a),a.append(m("p",{class:"guidebook-next"},[m("a",{href:va,class:"guidebook-next-link",id:"guidebook-next-chapter"},[nt,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])])),n.append(t)}const Fe={innerSeconds:180,midSeconds:240,outerSeconds:300,breatheSeconds:96},j=500,G=500;function v(n,e={}){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,o]of Object.entries(e))t.setAttribute(a,o);return t}function x(n,e){const t=(e-90)*Math.PI/180;return[j+n*Math.cos(t),G+n*Math.sin(t)]}function xa(n,e,t=0){return Array.from({length:n},(a,o)=>x(e,360/n*o+t).join(",")).join(" ")}function Wt(n,e,t,a=0){const o=[];for(let i=0;i<n;i+=1)o.push(x(e,360/n*i+a).join(",")),o.push(x(t,360/n*i+180/n+a).join(","));return o.join(" ")}function an(n,e,t,a){return[x(t,n),x((e+t)/2,n-a),x(e,n),x((e+t)/2,n+a)].map(o=>o.join(",")).join(" ")}function ci(n,e,t,a){return[x(t,n),x(e,n-a),x(e,n+a)].map(o=>o.join(",")).join(" ")}function _(n,e,t,a,o){n.append(v("polygon",{class:o,points:xa(e,t,a)}))}function di(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function ui(){const n=di(),e=m("figure",{class:n?"pex-attention-instrument is-static":"pex-attention-instrument"});e.setAttribute("role","img"),e.setAttribute("aria-label","Original Psychical Excursion attention instrument: nested thin-line geometry around a fixed center. Looking toward the center is optional. Seeing no afterimage or mental picture is not required.");const t=v("svg",{viewBox:"0 0 1000 1000",focusable:"false"});t.style.setProperty("--pex-attention-inner",`${Fe.innerSeconds}s`),t.style.setProperty("--pex-attention-mid",`${Fe.midSeconds}s`),t.style.setProperty("--pex-attention-outer",`${Fe.outerSeconds}s`),t.style.setProperty("--pex-attention-breathe",`${Fe.breatheSeconds}s`);const a=v("g",{class:"pex-attention-layer pex-attention-beyond"});a.append(v("polygon",{class:"pex-attention-line pex-attention-faintest",points:xa(36,486,5)})),a.append(v("circle",{class:"pex-attention-line pex-attention-faintest",cx:String(j),cy:String(G),r:"478"}));for(let h=0;h<72;h+=1){const d=h*5,c=x(468,d),g=x(h%6===0?492:484,d);a.append(v("line",{class:"pex-attention-line pex-attention-faintest",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const o=v("g",{class:"pex-attention-layer pex-attention-spin-outer"});o.append(v("circle",{class:"pex-attention-line pex-attention-strong",cx:String(j),cy:String(G),r:"438"})),o.append(v("circle",{class:"pex-attention-line",cx:String(j),cy:String(G),r:"418"}));for(let h=0;h<144;h+=1){const d=h*2.5,c=h%12===0,g=h%4===0,b=x(c?398:g?406:410,d),f=x(438,d);o.append(v("line",{class:c?"pex-attention-line pex-attention-strong":"pex-attention-line pex-attention-fine",x1:b[0].toFixed(2),y1:b[1].toFixed(2),x2:f[0].toFixed(2),y2:f[1].toFixed(2)}))}for(let h=0;h<8;h+=1){const d=h*45;o.append(v("polygon",{class:"pex-attention-line",points:an(d,442,462,3.2)}));const c=x(h%2===0?454:448,d);o.append(v("circle",{class:"pex-attention-fill pex-attention-fine",cx:c[0].toFixed(2),cy:c[1].toFixed(2),r:h%2===0?"2.1":"1.3"}))}for(const h of[18,138,258])o.append(v("path",{class:"pex-attention-line pex-attention-arc",d:mi(394,h,h+84)}));const i=v("g",{class:"pex-attention-layer pex-attention-spin-mid"});for(const h of[372,344,316,288])i.append(v("circle",{class:"pex-attention-line",cx:String(j),cy:String(G),r:String(h)}));_(i,12,372,0,"pex-attention-line pex-attention-soft"),_(i,12,372,15,"pex-attention-line pex-attention-soft");for(let h=0;h<24;h+=1)i.append(v("polygon",{class:"pex-attention-line",points:an(h*15,300,368,5.4)}));for(let h=0;h<36;h+=1)i.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:ci(h*10,248,286,3.6)}));for(let h=0;h<24;h+=1){const d=h*15,c=x(132,d),g=x(h%2===0?368:316,d);i.append(v("line",{class:h%2===0?"pex-attention-line":"pex-attention-line pex-attention-fine",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const r=v("g",{class:"pex-attention-layer pex-attention-breathe"});_(r,8,236,0,"pex-attention-line pex-attention-strong"),_(r,8,236,22.5,"pex-attention-line"),_(r,6,204,0,"pex-attention-line"),_(r,6,204,30,"pex-attention-line"),_(r,4,176,0,"pex-attention-line pex-attention-soft"),_(r,4,176,45,"pex-attention-line pex-attention-soft"),r.append(v("polygon",{class:"pex-attention-line",points:Wt(16,220,148,0)})),r.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:Wt(12,190,126,15)}));for(const h of[168,148,128])r.append(v("circle",{class:"pex-attention-line pex-attention-fine",cx:String(j),cy:String(G),r:String(h)}));const s=v("g",{class:"pex-attention-layer pex-attention-spin-inner"});for(let h=0;h<12;h+=1){const[d,c]=x(34,h*30);s.append(v("circle",{class:"pex-attention-line",cx:d.toFixed(2),cy:c.toFixed(2),r:"34"}))}for(let h=0;h<8;h+=1)s.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:an(h*45+22.5,58,108,8)}));_(s,16,96,0,"pex-attention-line pex-attention-fine"),_(s,8,72,0,"pex-attention-line"),s.append(v("circle",{class:"pex-attention-line",cx:String(j),cy:String(G),r:"54"}));for(let h=0;h<32;h+=1){const d=h*11.25,c=x(46,d),g=x(h%4===0?70:62,d);s.append(v("line",{class:"pex-attention-line pex-attention-fine",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const l=v("g",{class:"pex-attention-layer pex-attention-center"});return l.append(v("circle",{class:"pex-attention-line pex-attention-strong",cx:String(j),cy:String(G),r:"18"})),l.append(v("circle",{class:"pex-attention-line pex-attention-fine",cx:String(j),cy:String(G),r:"8"})),l.append(v("circle",{class:"pex-attention-fill pex-attention-center-point",cx:String(j),cy:String(G),r:"2.2"})),t.append(a,o,i,r,s,l),e.append(t),e}function mi(n,e,t){const a=x(n,e),o=x(n,t),i=t-e>180?1:0;return`M ${a[0].toFixed(2)} ${a[1].toFixed(2)} A ${n} ${n} 0 ${i} 1 ${o[0].toFixed(2)} ${o[1].toFixed(2)}`}function de(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function Aa(n,e){for(const t of e){if(t.kind==="emphasis"){n.append(m("p",{class:"guidebook-emphasis-line"},[t.text]));continue}if(t.kind==="quote"){const a=m("blockquote",{class:"guidebook-pull"});a.append(ze(t.text)),n.append(a);continue}if(t.kind==="list"){const a=m(t.ordered?"ol":"ul",{class:"guidebook-steps"});for(const o of t.items){const i=m("li",{});i.append(ze(o)),a.append(i)}n.append(a);continue}if(t.kind==="rule"){n.append(m("hr",{class:"guidebook-rule"}));continue}if(!(t.kind==="heading"||t.kind==="practice"||t.kind==="attention")){if(t.kind==="subheading"){n.append(m("h3",{class:"guidebook-practice-subheading"},[t.text]));continue}n.append(Ln(t.text))}}}function pi(n){const e=m("section",{class:"guidebook-practice-part"});return e.append(m("h2",{class:"guidebook-practice-label"},[n.label])),Aa(e,n.blocks),e}function gi(n){return m("nav",{class:"guidebook-prev","aria-label":"Previous reading"},[m("a",{href:n.href,class:"guidebook-prev-link",id:n.id},[m("span",{class:"guidebook-prev-arrow","aria-hidden":"true"},["← "]),n.title])])}function Y(n,e,t={}){const a=m("article",{class:"guidebook-article guidebook-chapter"});let o=de(!0);a.append(o),t.previous&&o.append(gi(t.previous)),o.append(m("h1",{class:"guidebook-chapter-title"},[e.title]));for(const i of e.blocks){if(i.kind==="heading"){o=de(!1),a.append(o);const r=m("h2",{class:"guidebook-section-title"},[i.text]),s=Xo(i.text);s&&(r.id=s,r.tabIndex=-1),o.append(r);continue}if(i.kind==="practice"){o=de(!1),a.append(o);const r=m("aside",{class:"guidebook-practice","aria-label":"Summary, experiment, and intention"});for(const s of i.parts)r.append(pi(s));o.append(r);continue}if(i.kind==="attention"){o=de(!1),a.append(o),o.append(ui());continue}Aa(o,[i])}if(e.references.length){o=de(!1),a.append(o),o.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const i=m("ul",{class:"guidebook-references"});for(const r of e.references)i.append(Ia(r));o.append(i)}t.next&&(o=de(!1),a.append(o),o.append(m("p",{class:"guidebook-next"},[m("a",{href:t.next.href,class:"guidebook-next-link",id:t.next.id},[t.next.title,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])]))),n.append(a)}const yi=`# You Are Dreaming. Notice.
...
`,Wn="You Are Dreaming. Notice.",at="/you-are-dreaming-notice",Yt=`#${at}`;let on=null;function fi(){return on||(on=H(yi)),on}const Sa=`# You Are Dreaming. Recognize.
...
`,Be="You Are Dreaming. Recognize.",ot="/you-are-dreaming-recognize",Ht=`#${ot}`,bi="PEX-IMPLEMENTATION-PLACEHOLDER";let rn=null;function wi(){const n=Sa.trim();return!n||n.includes(bi)?!1:n.startsWith(`# ${Be}`)}function vi(){if(!wi())throw new Error("Chapter 3 manuscript has not been supplied by PEX_PRIMARY");return rn||(rn=H(Sa)),rn}const Ti=`# Feel the Body.
...
`,Yn="Feel the Body.",it="/feel-the-body",zt=`#${it}`;let sn=null;function ki(){return sn||(sn=H(Ti)),sn}const Ea=`# Move Your Attention.
...
`,qe="Move Your Attention.",rt="/move-your-attention",Bt=`#${rt}`,Ii="PEX-IMPLEMENTATION-PLACEHOLDER";let ln=null;function Ie(){const n=Ea.trim();return!n||n.includes(Ii)?!1:n.startsWith(`# ${qe}`)}function xi(){if(!Ie())throw new Error("Chapter 5 manuscript has not been supplied by PEX_PRIMARY");return ln||(ln=H(Ea)),ln}const Ai=`# Build the Current.
...
`,Hn="Build the Current.",st="/build-the-current",qt=`#${st}`;let hn=null;function Si(){return hn||(hn=H(Ai)),hn}const Ei=`# Quiet the Mind.
...
`,zn="Quiet the Mind.",lt="/quiet-the-mind",_t=`#${lt}`;let cn=null;function Ri(){return cn||(cn=H(Ei)),cn}const Mi=`# See the Image.
...
`,Bn="See the Image.",ht="/see-the-image",jt=`#${ht}`;let dn=null;function Ni(){return dn||(dn=H(Mi)),dn}const Ci=`# Watch the Edge.
...
`,qn="Watch the Edge.",ct="/watch-the-edge",Gt=`#${ct}`;let un=null;function Di(){return un||(un=H(Ci)),un}const Pi=`# Let the Body Sleep.
...
`,_n="Let the Body Sleep.",dt="/let-the-body-sleep",Ut=`#${dt}`;let mn=null;function Oi(){return mn||(mn=H(Pi)),mn}const Fi=`# Move Without Moving.
...
`,jn="Move Without Moving.",ut="/move-without-moving",$t=`#${ut}`;let pn=null;function Li(){return pn||(pn=H(Fi)),pn}const Wi=`# Feel the Shift.
...
`,Gn="Feel the Shift.",mt="/feel-the-shift",Vt=`#${mt}`;let gn=null;function Yi(){return gn||(gn=H(Wi)),gn}const Hi=`# Know the Threshold.
...
`,Un="Know the Threshold.",pt="/know-the-threshold",Jt=`#${pt}`;let yn=null;function zi(){return yn||(yn=H(Hi)),yn}const Bi=`# Stabilize the Dream.
...
`,Ra="Stabilize the Dream.",gt="/stabilize-the-dream",qi=`#${gt}`;let fn=null;function _i(){return fn||(fn=H(Bi)),fn}function I(n,e){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,o]of Object.entries(e))t.setAttribute(a,o);return t}function ji(n){const e=m("div",{class:`pex-ambient pex-ambient-${n}`,"aria-hidden":"true"}),t=I("svg",{viewBox:"0 0 1200 900",focusable:"false"}),a=I("g",{class:"pex-ambient-spin pex-ambient-spin-slow"});a.append(I("circle",{class:"pex-ambient-arc",cx:"620",cy:"430",r:"318"}),I("circle",{class:"pex-ambient-arc pex-ambient-arc-faint",cx:"620",cy:"430",r:"214"}),I("circle",{class:"pex-ambient-point",cx:"938",cy:"430",r:"2.2"}),I("circle",{class:"pex-ambient-point",cx:"406",cy:"238",r:"1.6"}));const o=I("g",{class:"pex-ambient-spin pex-ambient-spin-mid"});o.append(I("ellipse",{class:"pex-ambient-arc",cx:"580",cy:"400",rx:"430",ry:"168"}),I("path",{class:"pex-ambient-arc pex-ambient-arc-partial",d:"M220 520 C 380 220, 820 180, 1040 470"}),I("circle",{class:"pex-ambient-point",cx:"1010",cy:"400",r:"1.8"}));const i=I("g",{class:"pex-ambient-axis"});if(i.append(I("line",{class:"pex-ambient-line",x1:"600",y1:"40",x2:"600",y2:"860"}),I("line",{class:"pex-ambient-line pex-ambient-line-soft",x1:"80",y1:"390",x2:"1120",y2:"510"})),t.append(i,a,o),n==="memory"){const r=I("g",{class:"pex-ambient-memory-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"430",cy:"360",rx:"92",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"760",cy:"500",rx:"70",ry:"110"}),I("circle",{class:"pex-ambient-form pex-ambient-form-c",cx:"620",cy:"280",r:"36"})),t.append(r)}if(n==="notice"||n==="recognize"){const r=I("g",{class:"pex-ambient-notice-align"});r.append(I("path",{class:"pex-ambient-arc pex-ambient-align-a",d:"M260 300 C 480 220, 700 240, 940 360"}),I("path",{class:"pex-ambient-arc pex-ambient-align-b",d:"M300 620 C 520 420, 760 380, 980 520"}),I("circle",{class:"pex-ambient-point pex-ambient-align-point",cx:"640",cy:"390",r:"2.4"})),n==="recognize"&&r.append(I("circle",{class:"pex-ambient-form pex-ambient-recognize-ring",cx:"640",cy:"390",r:"5.5"})),t.append(r)}if(n==="body"){const r=I("g",{class:"pex-ambient-body-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"620",cy:"318",rx:"36",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"620",cy:"468",rx:"58",ry:"92"}),I("circle",{class:"pex-ambient-point",cx:"620",cy:"390",r:"1.8"})),t.append(r)}return e.append(t),e}const Gi=[{name:"Aries",glyph:"♈︎"},{name:"Taurus",glyph:"♉︎"},{name:"Gemini",glyph:"♊︎"},{name:"Cancer",glyph:"♋︎"},{name:"Leo",glyph:"♌︎"},{name:"Virgo",glyph:"♍︎"},{name:"Libra",glyph:"♎︎"},{name:"Scorpio",glyph:"♏︎"},{name:"Sagittarius",glyph:"♐︎"},{name:"Capricorn",glyph:"♑︎"},{name:"Aquarius",glyph:"♒︎"},{name:"Pisces",glyph:"♓︎"}];function Kt(n){const e=(n%360+360)%360;return Gi[Math.floor(e/30)%12]}function Ui(n){const e=new Intl.DateTimeFormat(void 0,{month:"short",day:"numeric"}).format(n),t=new Intl.DateTimeFormat(void 0,{hour:"numeric",minute:"2-digit"}).format(n),a=new Intl.DateTimeFormat(void 0,{month:"long",day:"numeric",hour:"numeric",minute:"2-digit"}).format(n);return{date:e,time:t,spoken:a}}function Qt(n,e,t,a){const o=n==="sun"?`Sun in ${t}`:`Moon in ${t}`;return m("span",{class:"sky-widget-pair",tabindex:"0",role:"img","aria-label":o,"data-sky-body":n},[m("span",{class:"sky-widget-body","aria-hidden":"true"},[e]),m("span",{class:"sky-widget-sign","aria-hidden":"true"},[a]),m("span",{class:"sky-widget-tip","aria-hidden":"true"},[o])])}function $i(){const n=new Date,e=Uo(n),t=Kt(e.sun.eclipticLongitudeDeg),a=Kt(e.moon.eclipticLongitudeDeg),o=_o(n),i=o?Ui(o):null,r=i?`Sun in ${t.name}. Moon in ${a.name}. Next full moon ${i.spoken}.`:`Sun in ${t.name}. Moon in ${a.name}.`,s=i?m("span",{class:"sky-widget-full"},[m("span",{class:"sky-widget-full-label"},["Full Moon · "]),m("span",{class:"sky-widget-full-when"},[`${i.date} · ${i.time}`])]):m("span",{class:"sky-widget-full"},["Full Moon"]);return m("div",{class:"sky-widget-compact",role:"group","aria-label":r},[Qt("sun",`${fa}${Ot}`,t.name,t.glyph),Qt("moon",`${ba}${Ot}`,a.name,a.glyph),s])}function Vi(n){return n==="chapter01"?"memory":n==="chapter02"?"notice":n==="chapter03"?"recognize":n==="chapter04"?"body":"orbit"}function Ji(n){return n==="chapter01"?`${nt} · Psychical Excursion`:n==="chapter02"?`${Wn} · Psychical Excursion`:n==="chapter03"?`${Be} · Psychical Excursion`:n==="chapter04"?`${Yn} · Psychical Excursion`:n==="chapter05"?`${qe} · Psychical Excursion`:n==="chapter06"?`${Hn} · Psychical Excursion`:n==="chapter07"?`${zn} · Psychical Excursion`:n==="chapter08"?`${Bn} · Psychical Excursion`:n==="chapter09"?`${qn} · Psychical Excursion`:n==="chapter10"?`${_n} · Psychical Excursion`:n==="chapter11"?`${jn} · Psychical Excursion`:n==="chapter12"?`${Gn} · Psychical Excursion`:n==="chapter13"?`${Un} · Psychical Excursion`:n==="chapter14"?`${Ra} · Psychical Excursion`:"Psychical Excursion"}function Ki(n,e){Jn(),document.title=Ji(e),n.replaceChildren();const t=m("a",{class:"skip-link",href:"#main"},["Skip to content"]),a=Vi(e),o=ji(a),i=Xi(),r=m("header",{class:"app-header guidebook-header"},[m("a",{href:"#/",class:"brand-link","aria-label":"Psychical Excursion home"},[m("img",{class:"brand-logo brand-logo-light",src:"/brand/pex-logo-primary.svg",alt:"",width:"220",height:"52",decoding:"async"}),m("img",{class:"brand-logo brand-logo-reverse",src:"/brand/pex-logo-primary-reverse.svg",alt:"",width:"220",height:"52",decoding:"async"})]),m("div",{class:"header-tools guidebook-tools"},[$i(),m("span",{class:"guidebook-tools-rule","aria-hidden":"true"}),i])]),s=m("div",{id:"live-status",class:"visually-hidden","aria-live":"polite"}),l=m("main",{id:"main",class:"main-stage guidebook-main",tabindex:"-1","data-guidebook-page":e}),h=m("footer",{class:"site-footer guidebook-footer"},[m("p",{class:"attribution"},[qa("Website by "),m("a",{href:"https://hoopsnakedesigns.com/",rel:"noreferrer"},["Hoopsnake Designs"])])]),d=m("div",{class:"app-frame guidebook-frame","data-ambient":a},[r,s,l,h]);return n.append(t,o,d),{main:l}}function Qi(n){li(n)}function Xi(){const n=Vn()==="bedtime",e=m("button",{type:"button",id:"theme-light-dark",class:"theme-switch",role:"switch",dir:"ltr","aria-checked":n?"true":"false","aria-label":"Dark appearance"},[m("span",{class:"theme-switch-track","aria-hidden":"true",dir:"ltr"},[m("span",{class:"theme-switch-thumb"})])]);return e.addEventListener("click",()=>{const a=Wa()==="bedtime";e.setAttribute("aria-checked",a?"true":"false")}),e}function Zi(n=window.location.hash){return et(n).path}function er(n=window.location.hash){const e=Zi(n);return e===tt?"chapter01":e===at?"chapter02":e===ot?"chapter03":e===it?"chapter04":e===rt&&Ie()?"chapter05":e===st?"chapter06":e===lt?"chapter07":e===ht?"chapter08":e===ct?"chapter09":e===dt?"chapter10":e===ut?"chapter11":e===mt?"chapter12":e===pt?"chapter13":e===gt?"chapter14":"home"}function nr(){const{path:n}=et();n==="/"||n===tt||n===at||n===ot||n===it||n===rt&&Ie()||n!==st&&n!==lt&&n!==ht&&n!==ct&&n!==dt&&n!==ut&&n!==mt&&n!==pt&&n!==gt&&window.history.replaceState(null,"",`${window.location.pathname}${window.location.search}#/`)}let Ma=null;function tr(n){return Ma!==n}function ar(n){Ma=n}function bn(){typeof history<"u"&&"scrollRestoration"in history&&(history.scrollRestoration="manual"),document.documentElement.scrollTop=0,document.body.scrollTop=0;const n=navigator.userAgent??"";typeof window.scrollTo=="function"&&n.length>0&&!n.includes("jsdom")&&window.scrollTo(0,0)}function or(n,e){if(!e)return;const t=n.querySelector(`[id="${e}"]`);if(!t)return;t.closest(".guidebook-section")?.classList.add("is-visible");const a=navigator.userAgent??"";if(typeof t.scrollIntoView=="function"&&a.length>0&&!a.includes("jsdom")){const o=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;t.scrollIntoView({behavior:o?"auto":"smooth",block:"start"})}t.focus({preventScroll:!0})}const $n=[];function Na(){for(;$n.length;)$n.pop()?.()}function ir(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function rr(n){Na();const e=[...n.querySelectorAll(".pex-reveal")];if(e.length===0)return;if(ir()||typeof IntersectionObserver!="function"){for(const a of e)a.classList.add("is-visible");return}e[0]?.classList.add("is-visible");const t=new IntersectionObserver(a=>{for(const o of a)o.isIntersecting&&(o.target.classList.add("is-visible"),t.unobserve(o.target))},{threshold:.08,rootMargin:"0px 0px -10% 0px"});for(const a of e)t.observe(a);$n.push(()=>t.disconnect())}const Xt=[];function sr(){for(;Xt.length;)Xt.pop()?.()}async function lr(n){{nr(),_a(),$o(),sr(),Na();const e=er(),t=et().fragment,a=tr(e);a&&!t&&bn(),ar(e);const{main:o}=Ki(n,e),i={href:"#/",title:"Introduction",id:"guidebook-prev-home"};if(e==="chapter01"?Y(o,ti(),{previous:i,next:{href:Yt,title:Wn,id:"guidebook-next-chapter-2"}}):e==="chapter02"?Y(o,fi(),{previous:{href:va,title:nt,id:"guidebook-prev-chapter-1"},next:{href:Ht,title:Be,id:"guidebook-next-chapter-3"}}):e==="chapter03"?Y(o,vi(),{previous:{href:Yt,title:Wn,id:"guidebook-prev-chapter-2"},next:{href:zt,title:Yn,id:"guidebook-next-chapter-4"}}):e==="chapter04"?Y(o,ki(),{previous:{href:Ht,title:Be,id:"guidebook-prev-chapter-3"},next:Ie()?{href:Bt,title:qe,id:"guidebook-next-chapter-5"}:void 0}):e==="chapter05"&&Ie()?Y(o,xi(),{previous:{href:zt,title:Yn,id:"guidebook-prev-chapter-4"},next:{href:qt,title:Hn,id:"guidebook-next-chapter-6"}}):e==="chapter06"?Y(o,Si(),{previous:{href:Bt,title:qe,id:"guidebook-prev-chapter-5"},next:{href:_t,title:zn,id:"guidebook-next-chapter-7"}}):e==="chapter07"?Y(o,Ri(),{previous:{href:qt,title:Hn,id:"guidebook-prev-chapter-6"},next:{href:jt,title:Bn,id:"guidebook-next-chapter-8"}}):e==="chapter08"?Y(o,Ni(),{previous:{href:_t,title:zn,id:"guidebook-prev-chapter-7"},next:{href:Gt,title:qn,id:"guidebook-next-chapter-9"}}):e==="chapter09"?Y(o,Di(),{previous:{href:jt,title:Bn,id:"guidebook-prev-chapter-8"},next:{href:Ut,title:_n,id:"guidebook-next-chapter-10"}}):e==="chapter10"?Y(o,Oi(),{previous:{href:Gt,title:qn,id:"guidebook-prev-chapter-9"},next:{href:$t,title:jn,id:"guidebook-next-chapter-11"}}):e==="chapter11"?Y(o,Li(),{previous:{href:Ut,title:_n,id:"guidebook-prev-chapter-10"},next:{href:Vt,title:Gn,id:"guidebook-next-chapter-12"}}):e==="chapter12"?Y(o,Yi(),{previous:{href:$t,title:jn,id:"guidebook-prev-chapter-11"},next:{href:Jt,title:Un,id:"guidebook-next-chapter-13"}}):e==="chapter13"?Y(o,zi(),{previous:{href:Vt,title:Gn,id:"guidebook-prev-chapter-12"},next:{href:qi,title:Ra,id:"guidebook-next-chapter-14"}}):e==="chapter14"?Y(o,_i(),{previous:{href:Jt,title:Un,id:"guidebook-prev-chapter-13"}}):hi(o),rr(o),Qi(n),Ha(),t)or(n,t);else if(a){bn();const r=navigator.userAgent??"";typeof requestAnimationFrame=="function"&&r.length>0&&!r.includes("jsdom")&&requestAnimationFrame(()=>bn())}return}}function hr(){const n=document.getElementById("app");if(!n)throw new Error("Missing #app");return n}async function cr(){Jn(),Fa();const n=hr(),e=()=>{lr(n)};window.addEventListener("hashchange",e),e();try{await Zt.open(),await La()}catch{}}cr();
