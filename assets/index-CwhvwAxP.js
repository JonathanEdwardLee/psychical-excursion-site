const lt="0.6.0";const Ba="pex-local";const y={entries:"entries",media:"media",progress:"progress",settings:"settings"};function ie(n){const e=typeof n.updatedAt=="number"?n.updatedAt:n.createdAt;return{id:n.id,type:n.type,createdAt:n.createdAt,updatedAt:e,note:typeof n.note=="string"?n.note:"",audioId:n.audioId??null,audioMimeType:n.audioMimeType??null,audioByteLength:n.audioByteLength??null,syncState:n.syncState??"LOCAL",localSafeAt:n.localSafeAt??e,syncVersion:typeof n.syncVersion=="number"?n.syncVersion:1,remoteVersion:n.remoteVersion??null,remoteFileId:n.remoteFileId??null,remoteMediaFileId:n.remoteMediaFileId??null,syncErrorCode:n.syncErrorCode??null,pexDay:n.pexDay??null,phaseId:n.phaseId??null}}class U extends Error{code;recoverable;constructor(e,t,a=!0){super(t),this.name="AppError",this.code=e,this.recoverable=a}}function en(n){return{day:n,unlocked:!0,visitedAt:null,completedAt:null}}function De(n){return{day:n.day,unlocked:!0,visitedAt:typeof n.visitedAt=="number"?n.visitedAt:null,completedAt:typeof n.completedAt=="number"?n.completedAt:null}}function D(n){return new Promise((e,t)=>{n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error??new Error("indexeddb-request-failed"))})}function P(n){return new Promise((e,t)=>{n.oncomplete=()=>e(),n.onerror=()=>t(n.error??new Error("indexeddb-transaction-failed")),n.onabort=()=>t(n.error??new Error("indexeddb-transaction-aborted"))})}function we(n){const e=n instanceof DOMException?n.name:"",t=n instanceof Error?n.message:String(n);return e==="QuotaExceededError"||/quota/i.test(t)?new U("quota-exceeded","This browser does not have enough space to save. Export existing entries if you can, then free space and retry."):new U("save-failed","The local save did not complete. Your entry was not discarded silently; retry the save.")}class Ya{constructor(e=globalThis.indexedDB??null){this.factory=e}factory;assertAvailable(){if(!this.factory)throw new U("indexeddb-unavailable","IndexedDB is not available in this browser, so journal entries cannot be stored locally.",!1)}async open(){this.assertAvailable();try{const e=this.factory.open(Ba,3);e.onupgradeneeded=o=>{const i=e.result,r=e.transaction;if(i.objectStoreNames.contains(y.entries)){if(o.oldVersion<2){const s=r.objectStore(y.entries);s.indexNames.contains("syncState")||s.createIndex("syncState","syncState")}}else{const s=i.createObjectStore(y.entries,{keyPath:"id"});s.createIndex("createdAt","createdAt"),s.createIndex("type","type"),s.createIndex("syncState","syncState")}i.objectStoreNames.contains(y.media)||i.createObjectStore(y.media,{keyPath:"id"}).createIndex("entryId","entryId",{unique:!0}),i.objectStoreNames.contains(y.progress)||i.createObjectStore(y.progress,{keyPath:"day"}),i.objectStoreNames.contains(y.settings)||i.createObjectStore(y.settings,{keyPath:"key"})};const t=await D(e);return await this.ensureSeed(t)&&(await this.migrateLegacyEntries(t),await this.bumpSchemaRecord(t)),t}catch(e){throw e instanceof U?e:new U("indexeddb-open-failed","The local database could not be opened.",!1)}}async ensureSeed(e){const t=e.transaction([y.progress,y.settings],"readwrite"),a=t.objectStore(y.progress),o=t.objectStore(y.settings);for(let s=1;s<=60;s+=1){const l=await D(a.get(s));l?a.put(De(l)):a.put(en(s))}const i=await D(o.get("schema"));let r=!1;return i?i.schemaVersion<3&&(r=!0):o.put({key:"schema",schemaVersion:3,appVersion:lt,createdAt:Date.now()}),await P(t),r}async bumpSchemaRecord(e){const t=e.transaction(y.settings,"readwrite"),a=t.objectStore(y.settings),o=await D(a.get("schema"));o&&a.put({...o,schemaVersion:3,appVersion:lt}),await P(t)}async migrateLegacyEntries(e){const t=e.transaction(y.entries,"readwrite"),a=t.objectStore(y.entries),o=await D(a.getAll());for(const i of o)i.syncState===void 0&&a.put(ie(i));await P(t)}async listEntries(){const e=await this.open();try{const t=e.transaction(y.entries,"readonly"),a=t.objectStore(y.entries),o=await D(a.getAll());return await P(t),o.map(i=>ie(i)).sort((i,r)=>r.createdAt-i.createdAt)}finally{e.close()}}async getEntry(e){const t=await this.open();try{const a=t.transaction([y.entries,y.media],"readonly"),o=await D(a.objectStore(y.entries).get(e));if(!o)return await P(a),null;const i=ie(o);let r=null;return i.audioId&&(r=await D(a.objectStore(y.media).get(i.audioId))??null),await P(a),{entry:i,media:r}}finally{t.close()}}async saveCapture(e){const t=await this.open(),a=e.createdAt,o=ie({id:e.id,type:e.type,createdAt:a,updatedAt:a,note:e.note.trim(),audioId:e.audio?.id??null,audioMimeType:e.audio?.mimeType??null,audioByteLength:e.audio?e.audio.blob.size:null,syncState:e.syncState??"LOCAL",localSafeAt:a,syncVersion:e.syncVersion??1,remoteVersion:e.remoteVersion??null,remoteFileId:e.remoteFileId??null,remoteMediaFileId:e.remoteMediaFileId??null,syncErrorCode:null,pexDay:e.pexDay??null,phaseId:e.phaseId??null});try{const i=t.transaction(e.audio?[y.entries,y.media]:y.entries,"readwrite");if(i.objectStore(y.entries).put(o),e.audio){const r={id:e.audio.id,entryId:o.id,mimeType:e.audio.mimeType,blob:e.audio.blob,byteLength:e.audio.blob.size,createdAt:e.createdAt};i.objectStore(y.media).put(r)}return await P(i),o}catch(i){throw we(i)}finally{t.close()}}async updateNote(e,t){const a=await this.open();try{const o=a.transaction(y.entries,"readwrite"),i=o.objectStore(y.entries),r=await D(i.get(e));if(!r)throw new U("save-failed","That entry is no longer in local storage.");const s=ie(r),l=ie({...s,note:t.trim(),updatedAt:Date.now(),syncVersion:s.syncVersion+1,syncState:s.syncState==="SYNCED"?"PENDING_SYNC":s.syncState});return i.put(l),await P(o),l}catch(o){throw o instanceof U?o:we(o)}finally{a.close()}}async updateSyncState(e,t){const a=await this.open();try{const o=a.transaction(y.entries,"readwrite"),i=o.objectStore(y.entries),r=await D(i.get(e));if(!r)throw new U("save-failed","That entry is no longer in local storage.");const s=ie({...r,...t,updatedAt:Date.now()});return i.put(s),await P(o),s}catch(o){throw o instanceof U?o:we(o)}finally{a.close()}}async deleteEntry(e){const t=await this.open();try{const a=t.transaction([y.entries,y.media],"readwrite"),o=await D(a.objectStore(y.entries).get(e));o?.audioId&&a.objectStore(y.media).delete(o.audioId),a.objectStore(y.entries).delete(e),await P(a)}catch{throw new U("delete-failed","The entry could not be deleted. It should still be present in the journal.")}finally{t.close()}}async listProgress(){const e=await this.open();try{const t=e.transaction(y.progress,"readonly"),a=await D(t.objectStore(y.progress).getAll());return await P(t),a.map(o=>De(o)).sort((o,i)=>o.day-i.day)}finally{e.close()}}async getDayProgress(e){const t=await this.open();try{const a=t.transaction(y.progress,"readonly"),o=await D(a.objectStore(y.progress).get(e));return await P(a),o?De(o):en(e)}finally{t.close()}}async writeDayProgress(e,t){const a=await this.open();try{const o=a.transaction([y.progress,y.settings],"readwrite"),i=o.objectStore(y.progress),r=De(await D(i.get(e))??en(e)),s=t(r);return i.put(s),o.objectStore(y.settings).put({key:"resumeDay",value:e}),await P(o),s}catch(o){throw we(o)}finally{a.close()}}async markDayVisited(e){return this.writeDayProgress(e,t=>({...t,unlocked:!0,visitedAt:Date.now()}))}async completeDay(e){return this.writeDayProgress(e,t=>({...t,unlocked:!0,visitedAt:t.visitedAt??Date.now(),completedAt:Date.now()}))}async undoDayCompletion(e){return this.writeDayProgress(e,t=>({...t,unlocked:!0,completedAt:null}))}async loadResumeDay(){return this.getSetting("resumeDay")}async getSetting(e){const t=await this.open();try{const a=t.transaction(y.settings,"readonly"),o=await D(a.objectStore(y.settings).get(e));return await P(a),o?.value}finally{t.close()}}async setSetting(e,t){const a=await this.open();try{const o=a.transaction(y.settings,"readwrite");o.objectStore(y.settings).put({key:e,value:t}),await P(o)}catch(o){throw we(o)}finally{a.close()}}async getSchemaInfo(){const e=await this.open();try{const t=e.transaction(y.settings,"readonly"),a=await D(t.objectStore(y.settings).get("schema"));return await P(t),a}finally{e.close()}}async savePersistenceReport(e){await this.setSetting("persistenceReport",e)}async loadPersistenceReport(){return this.getSetting("persistenceReport")}async exportBundle(){const e=await this.open();try{const t=e.transaction([y.entries,y.media],"readonly"),a=await D(t.objectStore(y.entries).getAll()),o=await D(t.objectStore(y.media).getAll());return await P(t),{entries:a.sort((i,r)=>i.createdAt-r.createdAt),media:o}}finally{e.close()}}}const aa=new Ya;function za(){typeof window>"u"||"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").then(n=>{n.addEventListener("updatefound",()=>{const e=n.installing;e&&e.addEventListener("statechange",()=>{e.state==="installed"&&navigator.serviceWorker.controller&&document.dispatchEvent(new CustomEvent("pex-sw-update"))})})})})}async function qa(n=navigator.storage){const e=[],t=!!n,a=typeof n?.persist=="function";let o=null,i=!1,r=null,s=null,l=null;if(!t)e.push("navigator.storage is not present. Persistence cannot be requested in this browser.");else{if(typeof n?.persisted=="function")try{o=await n.persisted()}catch{e.push("storage.persisted() threw; treated as unknown.")}else e.push("storage.persisted() is not present.");if(typeof n?.estimate=="function")try{const d=await n.estimate();s=typeof d.usage=="number"?d.usage:null,l=typeof d.quota=="number"?d.quota:null}catch{e.push("storage.estimate() threw.")}else e.push("storage.estimate() is not present.");if(a&&o!==!0){i=!0;try{r=await n.persist(),r!==!0&&e.push("persist() returned a non-true result. The UI must not claim persistence was granted.")}catch{r=!1,e.push("persist() threw. Persistence is not claimed.")}}else a?(r=!0,e.push("Storage was already marked persisted before this request.")):e.push("storage.persist() is not present.")}const h={inspectedAt:Date.now(),storageApiPresent:t,persistApiPresent:a,persistedBeforeRequest:o,persistRequestAttempted:i,persistGranted:r,estimateUsageBytes:s,estimateQuotaBytes:l,notes:e};try{await aa.savePersistenceReport(h)}catch{e.push("The persistence report could not be stored in IndexedDB.")}return h}const oa="pex-theme";function Un(){try{const n=localStorage.getItem(oa);if(n==="bedtime"||n==="light")return n}catch{}return"light"}function Vn(n=Un()){const e=document.documentElement;e.dataset.theme=n;const t=n==="bedtime"?"#0c0c0b":"#f7f5ef",a=document.querySelector('meta[name="theme-color"]');a&&a.setAttribute("content",t);try{localStorage.setItem(oa,n)}catch{}return n}function ja(){const n=Un()==="bedtime"?"light":"bedtime";return Vn(n)}const ia=`# Move Your Attention: Focused Attention and Body Awareness Meditation
...
`,_a=`# Remember Your Dreams: Dream Recall Techniques and Research
...
`,ct="Notice Your Dreams: Dream Awareness, Patterns and Dream Signs",mo="/dream-awareness-signs/",dt=mo;let an=null;function po(){return an||(an=N(uo)),an}const da=`# Recognize the Dream: Lucid Dreaming, Reality Checks and Dream Signs
...
`,Dn="Recognize the Dream: Lucid Dreaming, Reality Checks and Dream Signs",go="/lucid-dreaming-reality-checks/",ut=go,yo="PEX-IMPLEMENTATION-PLACEHOLDER";let on=null;function fo(){const n=da.trim();return!n||n.includes(yo)?!1:n.startsWith(`# ${Dn}`)}function bo(){if(!fo())throw new Error("Chapter 3 manuscript has not been supplied by PEX_PRIMARY");return on||(on=N(da)),on}const wo=`# Feel the Body: Body Scan Meditation and Deep Relaxation
...
`,mt="Feel the Body: Body Scan Meditation and Deep Relaxation",vo="/body-scan-meditation/",pt=vo;let rn=null;function To(){return rn||(rn=N(wo)),rn}const ko=`# Build the Current: Tingling, Energy Sensations and Focused Attention
...
`,gt="Build the Current: Tingling, Energy Sensations and Focused Attention",Io="/energy-sensations-meditation/",yt=Io;let sn=null;function xo(){return sn||(sn=N(ko)),sn}const Ao=`# Quiet the Mind: Meditation Techniques for Sleep and Dream Awareness
...
`,ft="Quiet the Mind: Meditation Techniques for Sleep and Dream Awareness",So="/meditation-for-lucid-dreaming/",bt=So;let ln=null;function Eo(){return ln||(ln=N(Ao)),ln}const Ro=`# See the Image: Visualization, Mental Imagery and Hypnagogic Imagery
...
`,wt="See the Image: Visualization, Mental Imagery and Hypnagogic Imagery",Mo="/visualization-hypnagogic-imagery/",vt=Mo;let hn=null;function Do(){return hn||(hn=N(Ro)),hn}const Co=`# Watch the Edge: Hypnagogia and the Transition Into Lucid Dreaming
...
`,Tt="Watch the Edge: Hypnagogia and the Transition Into Lucid Dreaming",No="/hypnagogia-lucid-dreaming/",kt=No;let cn=null;function Oo(){return cn||(cn=N(Co)),cn}const Po=`# Let the Body Sleep: The “Mind Awake, Body Asleep” Route to Lucid Dreaming
...
`,It="Let the Body Sleep: The “Mind Awake, Body Asleep” Route to Lucid Dreaming",Lo="/mind-awake-body-asleep/",xt=Lo;let dn=null;function Wo(){return dn||(dn=N(Po)),dn}const Fo=`# Move Without Moving: Motor Imagery, Dream Movement and Sleep-Onset Practice
...
`,At="Move Without Moving: Motor Imagery, Dream Movement and Sleep-Onset Practice",Ho="/motor-imagery-lucid-dreaming/",St=Ho;let un=null;function Bo(){return un||(un=N(Fo)),un}const Yo=`# Feel the Shift: Vibrations, Floating and Out-of-Body Sensations Near Sleep
...
`,Et="Feel the Shift: Vibrations, Floating and Out-of-Body Sensations Near Sleep",zo="/out-of-body-sensations-sleep/",Rt=zo;let mn=null;function qo(){return mn||(mn=N(Yo)),mn}const jo=`# Know the Threshold: How to Recognize When Waking Imagery Becomes a Dream
...
`,Mt="Know the Threshold: How to Recognize When Waking Imagery Becomes a Dream",_o="/entering-a-lucid-dream/",Dt=_o;let pn=null;function Go(){return pn||(pn=N(jo)),pn}const Uo=`# Stabilize the Dream: Lucid Dream Stabilization Techniques and Research
...
`,Ct="Stabilize the Dream: Lucid Dream Stabilization Techniques and Research",Vo="/lucid-dream-stabilization/",Nt=Vo;let gn=null;function $o(){return gn||(gn=N(Uo)),gn}const Ko=`# Explore the Dream: Lucid Dream Experiments, Dream Control and Research
...
`,Ot="Explore the Dream: Lucid Dream Experiments, Dream Control and Research",Jo="/lucid-dream-experiments/",Pt=Jo;let yn=null;function Qo(){return yn||(yn=N(Ko)),yn}const Xo=`# Loosen the Body: Body Ownership, Self-Location and Out-of-Body Experience
...
`,Lt="Loosen the Body: Body Ownership, Self-Location and Out-of-Body Experience",Zo="/out-of-body-experience-body-ownership/",Wt=Zo;let fn=null;function ei(){return fn||(fn=N(Xo)),fn}const ni=`# Cross the Threshold: Astral Projection, OBE Techniques and Lucid Dreaming
...
`,Ft="Cross the Threshold: Astral Projection, OBE Techniques and Lucid Dreaming",ti="/astral-projection-obe-techniques/",Ht=ti;let bn=null;function ai(){return bn||(bn=N(ni)),bn}const oi=`# Test the Experience: Can Lucid Dreams and Out-of-Body Experiences Be Verified?
...
`,Ar="An experiment in dreams, consciousness, energy, and out-of-body experience";function Sr(n){const e=n.trim(),t=e.match(/^#\s+(.+)\n+/);if(!t)throw new Error("Guidebook manuscript must begin with a top-level heading");const a=t[1].trim(),i=e.slice(t[0].length).split(/\n## /),r=Qt(i[0]??""),s=[];let l=[];for(let h=1;h<i.length;h+=1){const d=i[h],c=d.indexOf(`
`),g=(c===-1?d:d.slice(0,c)).trim(),b=c===-1?"":d.slice(c+1);if(g==="References"){l=Er(b);continue}s.push({heading:g,paragraphs:Qt(b)})}return{openingHeading:a,openingParagraphs:r,sections:s,references:l}}function Qt(n){return n.split(/\n---\n/).flatMap(e=>e.split(/\n\n+/)).map(e=>e.replace(/\n/g," ").trim()).filter(Boolean)}function Er(n){return n.split(/\n+/).map(e=>e.trim()).filter(Boolean)}let In=null;function Rr(){return In||(In=Sr(xr)),In}function Xt(n){return n.replace(/[.,;:!?)]+$/u,"")}function qe(n,e={}){const{linkCitations:t=!0}=e,a=m("span",{class:"guidebook-rich-text"}),o=t?/(\[[^\]]+\]\((?:pex:[a-z0-9-]+|#\/[^)\s]+)\)|\*\*[^*]+\*\*|\*[^*]+\*|\[\d+\]|https:\/\/\S+|doi:10\.\S+)/gi:/(\[[^\]]+\]\((?:pex:[a-z0-9-]+|#\/[^)\s]+)\)|\*\*[^*]+\*\*|\*[^*]+\*|https:\/\/\S+|doi:10\.\S+)/gi;let i=0;for(const r of n.matchAll(o)){const s=r.index??0;s>i&&a.append(n.slice(i,s));const l=r[0],h=l.match(/^\[([^\]]+)\]\((pex:[a-z0-9-]+|#\/[^)\s]+)\)$/i);if(h){const d=h[1],c=h[2],g=c.toLowerCase().startsWith("pex:")?c.slice(4).toLowerCase():"",b=g?Ka[g]:void 0,f=b?.href??(c===Sn||c.endsWith(`#${$n}`)?Sn:c),T=m("a",{href:f,class:"guidebook-pex-link","data-pex-link":b?.id??An}),M=d.match(/^\*\*(.+)\*\*$/);M?T.append(m("strong",{},[M[1]])):T.append(d),a.append(T)}else if(l.startsWith("**"))a.append(m("strong",{},[l.slice(2,-2)]));else if(l.startsWith("*"))a.append(m("em",{},[l.slice(1,-1)]));else if(/^\[\d+\]$/.test(l)&&t){const d=l.slice(1,-1);a.append(m("a",{href:`#ref-${d}`,class:"guidebook-citation"},[l]))}else if(l.toLowerCase().startsWith("doi:10.")){const d=Xt(l.slice(4));a.append(m("a",{href:`https://doi.org/${d}`,class:"guidebook-doi",rel:"noreferrer"},[l]))}else if(l.toLowerCase().startsWith("https://")){const d=Xt(l);a.append(m("a",{href:d,class:"guidebook-external",rel:"noreferrer"},[l]))}else a.append(l);i=s+l.length}return i<n.length&&a.append(n.slice(i)),a.childNodes.length||a.append(n),a}function _n(n,e){const t=m("p",{});return t.append(qe(n,e)),t}function Oa(n){const e=n.match(/^(?:\*\*)?\[(\d+)\](?:\*\*)?\s*(.*)$/),t=e?.[1]??"0",a=e?.[2]??n,o=m("li",{id:`ref-${t}`,class:"guidebook-reference-item"}),i=m("span",{class:"guidebook-ref-num"},[`[${t}] `]);return o.append(i,qe(a,{linkCitations:!1})),o.tabIndex=-1,o}function Mr(n){for(const e of n.querySelectorAll("a.guidebook-citation"))e.addEventListener("click",t=>{const o=(e.getAttribute("href")??"").match(/^#ref-(\d+)$/);if(!o)return;const i=n.querySelector(`#ref-${o[1]}`);if(!i)return;t.preventDefault();const r=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;i.scrollIntoView({behavior:r?"auto":"smooth",block:"start"}),i.focus({preventScroll:!0})});for(const e of n.querySelectorAll("a.guidebook-pex-link"))e.addEventListener("click",t=>{const a=e.getAttribute("href")??"";let o="",i=!0;try{const l=new URL(a,window.location.href);o=l.hash.replace(/^#/,"");const h=window.location.pathname.replace(/\/?$/,"/"),d=l.pathname.replace(/\/?$/,"/");i=d===h||d==="/"||a.startsWith("#")}catch{o=a.includes("#")?a.split("#").pop()??"":""}if(!o||o.startsWith("/")||!i)return;const r=n.querySelector(`[id="${o}"]`);if(!r)return;t.preventDefault(),r.closest(".guidebook-section")?.classList.add("is-visible");const s=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;r.scrollIntoView({behavior:s?"auto":"smooth",block:"start"}),r.focus({preventScroll:!0})})}function Pe(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function Dr(n){const e=Rr(),t=m("article",{class:"guidebook-article"});let a=Pe(!0);t.append(a),a.append(m("p",{class:"guidebook-subtitle"},[Ar]),m("h1",{class:"guidebook-opening-title"},[io]),m("p",{class:"guidebook-opening-heading"},[e.openingHeading]),...e.openingParagraphs.map(o=>_n(o)));for(const o of e.sections){a=Pe(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title"},[o.heading]));for(const i of o.paragraphs){if(i.startsWith("**")&&i.endsWith("**")){a.append(m("p",{class:"guidebook-emphasis-line"},[i.slice(2,-2)]));continue}a.append(_n(i))}}if(e.references.length){a=Pe(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const o=m("ul",{class:"guidebook-references"});for(const i of e.references)o.append(Oa(i));a.append(o)}a=Pe(!1),t.append(a),a.append(m("p",{class:"guidebook-next"},[m("a",{href:sa,class:"guidebook-next-link",id:"guidebook-next-chapter"},[ra,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])])),n.append(t)}const Le={innerSeconds:180,midSeconds:240,outerSeconds:300,breatheSeconds:96},_=500,G=500;function v(n,e={}){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,o]of Object.entries(e))t.setAttribute(a,o);return t}function x(n,e){const t=(e-90)*Math.PI/180;return[_+n*Math.cos(t),G+n*Math.sin(t)]}function Pa(n,e,t=0){return Array.from({length:n},(a,o)=>x(e,360/n*o+t).join(",")).join(" ")}function Zt(n,e,t,a=0){const o=[];for(let i=0;i<n;i+=1)o.push(x(e,360/n*i+a).join(",")),o.push(x(t,360/n*i+180/n+a).join(","));return o.join(" ")}function xn(n,e,t,a){return[x(t,n),x((e+t)/2,n-a),x(e,n),x((e+t)/2,n+a)].map(o=>o.join(",")).join(" ")}function Cr(n,e,t,a){return[x(t,n),x(e,n-a),x(e,n+a)].map(o=>o.join(",")).join(" ")}function j(n,e,t,a,o){n.append(v("polygon",{class:o,points:Pa(e,t,a)}))}function Nr(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function Or(){const n=Nr(),e=m("figure",{class:n?"pex-attention-instrument is-static":"pex-attention-instrument"});e.setAttribute("role","img"),e.setAttribute("aria-label","Original Psychical Excursion attention instrument: nested thin-line geometry around a fixed center. Looking toward the center is optional. Seeing no afterimage or mental picture is not required.");const t=v("svg",{viewBox:"0 0 1000 1000",focusable:"false"});t.style.setProperty("--pex-attention-inner",`${Le.innerSeconds}s`),t.style.setProperty("--pex-attention-mid",`${Le.midSeconds}s`),t.style.setProperty("--pex-attention-outer",`${Le.outerSeconds}s`),t.style.setProperty("--pex-attention-breathe",`${Le.breatheSeconds}s`);const a=v("g",{class:"pex-attention-layer pex-attention-beyond"});a.append(v("polygon",{class:"pex-attention-line pex-attention-faintest",points:Pa(36,486,5)})),a.append(v("circle",{class:"pex-attention-line pex-attention-faintest",cx:String(_),cy:String(G),r:"478"}));for(let h=0;h<72;h+=1){const d=h*5,c=x(468,d),g=x(h%6===0?492:484,d);a.append(v("line",{class:"pex-attention-line pex-attention-faintest",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const o=v("g",{class:"pex-attention-layer pex-attention-spin-outer"});o.append(v("circle",{class:"pex-attention-line pex-attention-strong",cx:String(_),cy:String(G),r:"438"})),o.append(v("circle",{class:"pex-attention-line",cx:String(_),cy:String(G),r:"418"}));for(let h=0;h<144;h+=1){const d=h*2.5,c=h%12===0,g=h%4===0,b=x(c?398:g?406:410,d),f=x(438,d);o.append(v("line",{class:c?"pex-attention-line pex-attention-strong":"pex-attention-line pex-attention-fine",x1:b[0].toFixed(2),y1:b[1].toFixed(2),x2:f[0].toFixed(2),y2:f[1].toFixed(2)}))}for(let h=0;h<8;h+=1){const d=h*45;o.append(v("polygon",{class:"pex-attention-line",points:xn(d,442,462,3.2)}));const c=x(h%2===0?454:448,d);o.append(v("circle",{class:"pex-attention-fill pex-attention-fine",cx:c[0].toFixed(2),cy:c[1].toFixed(2),r:h%2===0?"2.1":"1.3"}))}for(const h of[18,138,258])o.append(v("path",{class:"pex-attention-line pex-attention-arc",d:Pr(394,h,h+84)}));const i=v("g",{class:"pex-attention-layer pex-attention-spin-mid"});for(const h of[372,344,316,288])i.append(v("circle",{class:"pex-attention-line",cx:String(_),cy:String(G),r:String(h)}));j(i,12,372,0,"pex-attention-line pex-attention-soft"),j(i,12,372,15,"pex-attention-line pex-attention-soft");for(let h=0;h<24;h+=1)i.append(v("polygon",{class:"pex-attention-line",points:xn(h*15,300,368,5.4)}));for(let h=0;h<36;h+=1)i.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:Cr(h*10,248,286,3.6)}));for(let h=0;h<24;h+=1){const d=h*15,c=x(132,d),g=x(h%2===0?368:316,d);i.append(v("line",{class:h%2===0?"pex-attention-line":"pex-attention-line pex-attention-fine",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const r=v("g",{class:"pex-attention-layer pex-attention-breathe"});j(r,8,236,0,"pex-attention-line pex-attention-strong"),j(r,8,236,22.5,"pex-attention-line"),j(r,6,204,0,"pex-attention-line"),j(r,6,204,30,"pex-attention-line"),j(r,4,176,0,"pex-attention-line pex-attention-soft"),j(r,4,176,45,"pex-attention-line pex-attention-soft"),r.append(v("polygon",{class:"pex-attention-line",points:Zt(16,220,148,0)})),r.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:Zt(12,190,126,15)}));for(const h of[168,148,128])r.append(v("circle",{class:"pex-attention-line pex-attention-fine",cx:String(_),cy:String(G),r:String(h)}));const s=v("g",{class:"pex-attention-layer pex-attention-spin-inner"});for(let h=0;h<12;h+=1){const[d,c]=x(34,h*30);s.append(v("circle",{class:"pex-attention-line",cx:d.toFixed(2),cy:c.toFixed(2),r:"34"}))}for(let h=0;h<8;h+=1)s.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:xn(h*45+22.5,58,108,8)}));j(s,16,96,0,"pex-attention-line pex-attention-fine"),j(s,8,72,0,"pex-attention-line"),s.append(v("circle",{class:"pex-attention-line",cx:String(_),cy:String(G),r:"54"}));for(let h=0;h<32;h+=1){const d=h*11.25,c=x(46,d),g=x(h%4===0?70:62,d);s.append(v("line",{class:"pex-attention-line pex-attention-fine",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const l=v("g",{class:"pex-attention-layer pex-attention-center"});return l.append(v("circle",{class:"pex-attention-line pex-attention-strong",cx:String(_),cy:String(G),r:"18"})),l.append(v("circle",{class:"pex-attention-line pex-attention-fine",cx:String(_),cy:String(G),r:"8"})),l.append(v("circle",{class:"pex-attention-fill pex-attention-center-point",cx:String(_),cy:String(G),r:"2.2"})),t.append(a,o,i,r,s,l),e.append(t),e}function Pr(n,e,t){const a=x(n,e),o=x(n,t),i=t-e>180?1:0;return`M ${a[0].toFixed(2)} ${a[1].toFixed(2)} A ${n} ${n} 0 ${i} 1 ${o[0].toFixed(2)} ${o[1].toFixed(2)}`}function ue(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function La(n,e){for(const t of e){if(t.kind==="emphasis"){n.append(m("p",{class:"guidebook-emphasis-line"},[t.text]));continue}if(t.kind==="quote"){const a=m("blockquote",{class:"guidebook-pull"});a.append(qe(t.text)),n.append(a);continue}if(t.kind==="list"){const a=m(t.ordered?"ol":"ul",{class:"guidebook-steps"});for(const o of t.items){const i=m("li",{});i.append(qe(o)),a.append(i)}n.append(a);continue}if(t.kind==="rule"){n.append(m("hr",{class:"guidebook-rule"}));continue}if(!(t.kind==="heading"||t.kind==="practice"||t.kind==="attention")){if(t.kind==="subheading"){n.append(m("h3",{class:"guidebook-practice-subheading"},[t.text]));continue}n.append(_n(t.text))}}}function Lr(n){const e=m("section",{class:"guidebook-practice-part"});return e.append(m("h2",{class:"guidebook-practice-label"},[n.label])),La(e,n.blocks),e}function Wr(n){return m("nav",{class:"guidebook-prev","aria-label":"Previous reading"},[m("a",{href:n.href,class:"guidebook-prev-link",id:n.id},[m("span",{class:"guidebook-prev-arrow","aria-hidden":"true"},["← "]),n.title])])}function C(n,e,t={}){const a=m("article",{class:"guidebook-article guidebook-chapter"});let o=ue(!0);a.append(o),t.previous&&o.append(Wr(t.previous)),o.append(m("h1",{class:"guidebook-chapter-title"},[e.title]));for(const i of e.blocks){if(i.kind==="heading"){o=ue(!1),a.append(o);const r=m("h2",{class:"guidebook-section-title"},[i.text]),s=$a(i.text);s&&(r.id=s,r.tabIndex=-1),o.append(r);continue}if(i.kind==="practice"){o=ue(!1),a.append(o);const r=m("aside",{class:"guidebook-practice","aria-label":"Summary, experiment, and intention"});for(const s of i.parts)r.append(Lr(s));o.append(r);continue}if(i.kind==="attention"){o=ue(!1),a.append(o),o.append(Or());continue}La(o,[i])}if(e.references.length){o=ue(!1),a.append(o),o.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const i=m("ul",{class:"guidebook-references"});for(const r of e.references)i.append(Oa(r));o.append(i)}t.next&&(o=ue(!1),a.append(o),o.append(m("p",{class:"guidebook-next"},[m("a",{href:t.next.href,class:"guidebook-next-link",id:t.next.id},[t.next.title,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])]))),n.append(a)}function I(n,e){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,o]of Object.entries(e))t.setAttribute(a,o);return t}function Fr(n){const e=m("div",{class:`pex-ambient pex-ambient-${n}`,"aria-hidden":"true"}),t=I("svg",{viewBox:"0 0 1200 900",focusable:"false"}),a=I("g",{class:"pex-ambient-spin pex-ambient-spin-slow"});a.append(I("circle",{class:"pex-ambient-arc",cx:"620",cy:"430",r:"318"}),I("circle",{class:"pex-ambient-arc pex-ambient-arc-faint",cx:"620",cy:"430",r:"214"}),I("circle",{class:"pex-ambient-point",cx:"938",cy:"430",r:"2.2"}),I("circle",{class:"pex-ambient-point",cx:"406",cy:"238",r:"1.6"}));const o=I("g",{class:"pex-ambient-spin pex-ambient-spin-mid"});o.append(I("ellipse",{class:"pex-ambient-arc",cx:"580",cy:"400",rx:"430",ry:"168"}),I("path",{class:"pex-ambient-arc pex-ambient-arc-partial",d:"M220 520 C 380 220, 820 180, 1040 470"}),I("circle",{class:"pex-ambient-point",cx:"1010",cy:"400",r:"1.8"}));const i=I("g",{class:"pex-ambient-axis"});if(i.append(I("line",{class:"pex-ambient-line",x1:"600",y1:"40",x2:"600",y2:"860"}),I("line",{class:"pex-ambient-line pex-ambient-line-soft",x1:"80",y1:"390",x2:"1120",y2:"510"})),t.append(i,a,o),n==="memory"){const r=I("g",{class:"pex-ambient-memory-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"430",cy:"360",rx:"92",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"760",cy:"500",rx:"70",ry:"110"}),I("circle",{class:"pex-ambient-form pex-ambient-form-c",cx:"620",cy:"280",r:"36"})),t.append(r)}if(n==="notice"||n==="recognize"){const r=I("g",{class:"pex-ambient-notice-align"});r.append(I("path",{class:"pex-ambient-arc pex-ambient-align-a",d:"M260 300 C 480 220, 700 240, 940 360"}),I("path",{class:"pex-ambient-arc pex-ambient-align-b",d:"M300 620 C 520 420, 760 380, 980 520"}),I("circle",{class:"pex-ambient-point pex-ambient-align-point",cx:"640",cy:"390",r:"2.4"})),n==="recognize"&&r.append(I("circle",{class:"pex-ambient-form pex-ambient-recognize-ring",cx:"640",cy:"390",r:"5.5"})),t.append(r)}if(n==="body"){const r=I("g",{class:"pex-ambient-body-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"620",cy:"318",rx:"36",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"620",cy:"468",rx:"58",ry:"92"}),I("circle",{class:"pex-ambient-point",cx:"620",cy:"390",r:"1.8"})),t.append(r)}return e.append(t),e}const Hr=[{name:"Aries",glyph:"♈︎"},{name:"Taurus",glyph:"♉︎"},{name:"Gemini",glyph:"♊︎"},{name:"Cancer",glyph:"♋︎"},{name:"Leo",glyph:"♌︎"},{name:"Virgo",glyph:"♍︎"},{name:"Libra",glyph:"♎︎"},{name:"Scorpio",glyph:"♏︎"},{name:"Sagittarius",glyph:"♐︎"},{name:"Capricorn",glyph:"♑︎"},{name:"Aquarius",glyph:"♒︎"},{name:"Pisces",glyph:"♓︎"}];function ea(n){const e=(n%360+360)%360;return Hr[Math.floor(e/30)%12]}function Br(n){const e=new Intl.DateTimeFormat(void 0,{month:"short",day:"numeric"}).format(n),t=new Intl.DateTimeFormat(void 0,{hour:"numeric",minute:"2-digit"}).format(n),a=new Intl.DateTimeFormat(void 0,{month:"long",day:"numeric",hour:"numeric",minute:"2-digit"}).format(n);return{date:e,time:t,spoken:a}}function na(n,e,t,a){const o=n==="sun"?`Sun in ${t}`:`Moon in ${t}`;return m("span",{class:"sky-widget-pair",tabindex:"0",role:"img","aria-label":o,"data-sky-body":n},[m("span",{class:"sky-widget-body","aria-hidden":"true"},[e]),m("span",{class:"sky-widget-sign","aria-hidden":"true"},[a]),m("span",{class:"sky-widget-tip","aria-hidden":"true"},[o])])}function Yr(){const n=new Date,e=kr(n),t=ea(e.sun.eclipticLongitudeDeg),a=ea(e.moon.eclipticLongitudeDeg),o=wr(n),i=o?Br(o):null,r=i?`Sun in ${t.name}. Moon in ${a.name}. Next full moon ${i.spoken}.`:`Sun in ${t.name}. Moon in ${a.name}.`,s=i?m("span",{class:"sky-widget-full"},[m("span",{class:"sky-widget-full-label"},["Full Moon · "]),m("span",{class:"sky-widget-full-when"},[`${i.date} · ${i.time}`])]):m("span",{class:"sky-widget-full"},["Full Moon"]);return m("div",{class:"sky-widget-compact",role:"group","aria-label":r},[na("sun",`${Da}${Jt}`,t.name,t.glyph),na("moon",`${Ca}${Jt}`,a.name,a.glyph),s])}function zr(n){return n==="chapter01"?"memory":n==="chapter02"?"notice":n==="chapter03"?"recognize":n==="chapter04"?"body":"orbit"}function qr(n){const e=lo(n);return e?ho(e.title):"Psychical Excursion"}function jr(n,e){Vn(),document.title=qr(e),n.replaceChildren();const t=m("a",{class:"skip-link",href:"#main"},["Skip to content"]),a=zr(e),o=Fr(a),i=Gr(),r=m("header",{class:"app-header guidebook-header"},[m("a",{href:te,class:"brand-link","aria-label":"Psychical Excursion home"},[m("img",{class:"brand-logo brand-logo-light",src:"/brand/pex-logo-primary.svg",alt:"",width:"220",height:"52",decoding:"async"}),m("img",{class:"brand-logo brand-logo-reverse",src:"/brand/pex-logo-primary-reverse.svg",alt:"",width:"220",height:"52",decoding:"async"})]),m("div",{class:"header-tools guidebook-tools"},[Yr(),m("span",{class:"guidebook-tools-rule","aria-hidden":"true"}),i])]),s=m("div",{id:"live-status",class:"visually-hidden","aria-live":"polite"}),l=m("main",{id:"main",class:"main-stage guidebook-main",tabindex:"-1","data-guidebook-page":e}),h=m("footer",{class:"site-footer guidebook-footer"},[m("p",{class:"attribution"},[wi("Website by "),m("a",{href:"https://hoopsnakedesigns.com/",rel:"noreferrer"},["Hoopsnake Designs"])])]),d=m("div",{class:"app-frame guidebook-frame","data-ambient":a},[r,s,l,h]);return n.append(t,o,d),{main:l}}function _r(n){Mr(n)}function Gr(){const n=Un()==="bedtime",e=m("button",{type:"button",id:"theme-light-dark",class:"theme-switch",role:"switch",dir:"ltr","aria-checked":n?"true":"false","aria-label":"Dark appearance"},[m("span",{class:"theme-switch-track","aria-hidden":"true",dir:"ltr"},[m("span",{class:"theme-switch-thumb"})])]);return e.addEventListener("click",()=>{const a=ja()==="bedtime";e.setAttribute("aria-checked",a?"true":"false")}),e}const Gn=[];function Wa(){for(;Gn.length;)Gn.pop()?.()}function Ur(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function Vr(n){Wa();const e=[...n.querySelectorAll(".pex-reveal")];if(e.length===0)return;if(Ur()||typeof IntersectionObserver!="function"){for(const a of e)a.classList.add("is-visible");return}e[0]?.classList.add("is-visible");const t=new IntersectionObserver(a=>{for(const o of a)o.isIntersecting&&(o.target.classList.add("is-visible"),t.unobserve(o.target))},{threshold:.08,rootMargin:"0px 0px -10% 0px"});for(const a of e)t.observe(a);Gn.push(()=>t.disconnect())}const ta=[];function $r(){for(;ta.length;)ta.pop()?.()}async function Kr(n){{if(ua())return;vi(),Ir(),$r(),Wa();const e=di(),t=ci(),a=ui(e);a&&!t&&vn(),mi(e);const{main:o}=jr(n,e),i={href:te,title:"Introduction",id:"guidebook-prev-home"};if(e==="chapter01"?C(o,eo(),{previous:i,next:{href:dt,title:ct,id:"guidebook-next-chapter-2"}}):e==="chapter02"?C(o,po(),{previous:{href:Qa,title:ra,id:"guidebook-prev-chapter-1"},next:{href:ut,title:Dn,id:"guidebook-next-chapter-3"}}):e==="chapter03"?C(o,bo(),{previous:{href:dt,title:ct,id:"guidebook-prev-chapter-2"},next:{href:pt,title:mt,id:"guidebook-next-chapter-4"}}):e==="chapter04"?C(o,To(),{previous:{href:ut,title:Dn,id:"guidebook-prev-chapter-3"},next:We()?{href:ht,title:Mn,id:"guidebook-next-chapter-5"}:void 0}):e==="chapter05"&&We()?C(o,ao(),{previous:{href:pt,title:mt,id:"guidebook-prev-chapter-4"},next:{href:yt,title:gt,id:"guidebook-next-chapter-6"}}):e==="chapter06"?C(o,xo(),{previous:{href:ht,title:Mn,id:"guidebook-prev-chapter-5"},next:{href:bt,title:ft,id:"guidebook-next-chapter-7"}}):e==="chapter07"?C(o,Eo(),{previous:{href:yt,title:gt,id:"guidebook-prev-chapter-6"},next:{href:vt,title:wt,id:"guidebook-next-chapter-8"}}):e==="chapter08"?C(o,Do(),{previous:{href:bt,title:ft,id:"guidebook-prev-chapter-7"},next:{href:kt,title:Tt,id:"guidebook-next-chapter-9"}}):e==="chapter09"?C(o,Oo(),{previous:{href:vt,title:wt,id:"guidebook-prev-chapter-8"},next:{href:xt,title:It,id:"guidebook-next-chapter-10"}}):e==="chapter10"?C(o,Wo(),{previous:{href:kt,title:Tt,id:"guidebook-prev-chapter-9"},next:{href:St,title:At,id:"guidebook-next-chapter-11"}}):e==="chapter11"?C(o,Bo(),{previous:{href:xt,title:It,id:"guidebook-prev-chapter-10"},next:{href:Rt,title:Et,id:"guidebook-next-chapter-12"}}):e==="chapter12"?C(o,qo(),{previous:{href:St,title:At,id:"guidebook-prev-chapter-11"},next:{href:Dt,title:Mt,id:"guidebook-next-chapter-13"}}):e==="chapter13"?C(o,Go(),{previous:{href:Rt,title:Et,id:"guidebook-prev-chapter-12"},next:{href:Nt,title:Ct,id:"guidebook-next-chapter-14"}}):e==="chapter14"?C(o,$o(),{previous:{href:Dt,title:Mt,id:"guidebook-prev-chapter-13"},next:{href:Pt,title:Ot,id:"guidebook-next-chapter-15"}}):e==="chapter15"?C(o,Qo(),{previous:{href:Nt,title:Ct,id:"guidebook-prev-chapter-14"},next:{href:Wt,title:Lt,id:"guidebook-next-chapter-16"}}):e==="chapter16"?C(o,ei(),{previous:{href:Pt,title:Ot,id:"guidebook-prev-chapter-15"},next:{href:Ht,title:Ft,id:"guidebook-next-chapter-17"}}):e==="chapter17"?C(o,ai(),{previous:{href:Wt,title:Lt,id:"guidebook-prev-chapter-16"},next:{href:si,title:ii,id:"guidebook-next-chapter-18"}}):e==="chapter18"?C(o,li(),{previous:{href:Ht,title:Ft,id:"guidebook-prev-chapter-17"}}):Dr(o),Vr(o),_r(n),yi(),t)pi(n,t);else if(a){vn();const r=navigator.userAgent??"";typeof requestAnimationFrame=="function"&&r.length>0&&!r.includes("jsdom")&&requestAnimationFrame(()=>vn())}return}}function Jr(){const n=document.getElementById("app");if(!n)throw new Error("Missing #app");return n}async function Qr(){Vn(),za();const n=Jr(),e=()=>{Kr(n)};if(!ua()){window.addEventListener("popstate",e),window.addEventListener("hashchange",()=>{window.location.hash.startsWith("#/")&&e()}),e();try{await aa.open(),await qa()}catch{}}}Qr();
