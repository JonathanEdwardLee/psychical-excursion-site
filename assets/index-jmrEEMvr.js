...
`,Ha=`# Remember Your Dreams: Dream Recall Techniques and Research
...
`,Un="nighttime-body-release",Ya="A Nighttime Body Release",In="relax-the-body",za="Relax the body",xn=`/body-scan-meditation/#${Un}`,Vn="pex:attention-instrument",Ba={[Ya]:Un,References:"references"};function qa(n){return Ba[n]}const _a={[In]:{id:In,href:xn,label:za}};function ja(n=window.location.hash){const t=n.replace(/^#/,"").split("?")[0]||"/",[a,o=""]=t.split("#");return{path:(a&&a.startsWith("/")?a:`/${a??""}`)||"/",fragment:o}}const ea="Remember Your Dreams: Dream Recall Techniques and Research",na="/dream-recall/",Ga=na,An=["Summary","Experiment","Intention"];function ta(n){return An.includes(n)}function Sn(n){const e=n.match(/^#{2,3}\s+(.*)$/);if(!e)return null;const t=e[1].trim();return ta(t)?t:null}function aa(n){const e=n.trim();return!e||e==="---"||n.startsWith("## ")||n.startsWith("### ")||n.startsWith("> ")||n.trim()===Vn||/^\d+\.\s/.test(n)||/^-\s/.test(n)}function L(n){const e=n.replace(/\r\n/g,`
`).trim().split(`
`);if(!e[0]?.startsWith("# "))throw new Error("Chapter manuscript must begin with a top-level heading");const t=e[0].slice(2).trim(),a=[],o=[];let i=1,r=!1;for(;i<e.length;){const s=e[i];if(r){const c=s.trim();c&&o.push(c),i+=1;continue}if(!s.trim()){i+=1;continue}if(s.trim()==="---"){a.push({kind:"rule"}),i+=1;continue}if(s.trim()===Vn){a.push({kind:"attention"}),i+=1;continue}if(Sn(s)){const c=Va(e,i);a.push(c.block),i=c.nextIndex,i<e.length&&e[i].trim()==="---"&&(i+=1);continue}if(s.startsWith("## ")){const c=s.slice(3).trim();if(c==="References"){r=!0,i+=1;continue}a.push({kind:"heading",text:c}),i+=1;continue}if(s.startsWith("> ")){const c=[];for(;i<e.length&&e[i].startsWith("> ");)c.push(e[i].slice(2).trim()),i+=1;a.push({kind:"quote",text:c.join(" ")});continue}if(/^\d+\.\s/.test(s)){const c=[];for(;i<e.length&&/^\d+\.\s/.test(e[i]);)c.push(e[i].replace(/^\d+\.\s+/,"").trim()),i+=1;a.push({kind:"list",items:c,ordered:!0});continue}if(/^-\s/.test(s)){const c=[];for(;i<e.length&&/^-\s/.test(e[i]);)c.push(e[i].replace(/^-\s+/,"").trim()),i+=1;a.push({kind:"list",items:c,ordered:!1});continue}const h=[];for(;i<e.length;){const c=e[i];if(aa(c)&&c.trim()!==""||!c.trim())break;h.push(c.trim()),i+=1}const d=h.join(" ");d.startsWith("**")&&d.endsWith("**")&&!d.slice(2,-2).includes("**")?a.push({kind:"emphasis",text:d.slice(2,-2)}):d&&a.push({kind:"paragraph",text:d})}return{title:t,blocks:a,references:o}}function Ua(n,e,t){const a=[];let o=e;for(;o<n.length;){const i=n[o];if(t(i))break;if(!i.trim()){o+=1;continue}if(i.trim()==="---"){a.push({kind:"rule"}),o+=1;continue}if(i.trim()===Vn){a.push({kind:"attention"}),o+=1;continue}if(i.startsWith("### ")){const l=i.slice(4).trim();if(!ta(l)){a.push({kind:"subheading",text:l}),o+=1;continue}}if(i.startsWith("> ")){const l=[];for(;o<n.length&&n[o].startsWith("> ");)l.push(n[o].slice(2).trim()),o+=1;a.push({kind:"quote",text:l.join(" ")});continue}if(/^\d+\.\s/.test(i)){const l=[];for(;o<n.length&&/^\d+\.\s/.test(n[o]);)l.push(n[o].replace(/^\d+\.\s+/,"").trim()),o+=1;a.push({kind:"list",items:l,ordered:!0});continue}if(/^-\s/.test(i)){const l=[];for(;o<n.length&&/^-\s/.test(n[o]);)l.push(n[o].replace(/^-\s+/,"").trim()),o+=1;a.push({kind:"list",items:l,ordered:!1});continue}const r=[];for(;o<n.length;){const l=n[o];if(t(l)||!l.trim()||aa(l)&&l.trim()!=="")break;r.push(l.trim()),o+=1}const s=r.join(" ");s.startsWith("**")&&s.endsWith("**")&&!s.slice(2,-2).includes("**")?a.push({kind:"emphasis",text:s.slice(2,-2)}):s&&a.push({kind:"paragraph",text:s})}return{blocks:a,nextIndex:o}}function Va(n,e){const t=[];let a=e;for(const o of An){if(Sn(n[a]??"")!==o)throw new Error(`Practice component must use ${An.join(", then ")} headings`);a+=1;const r=Ua(n,a,s=>!!(s.startsWith("## ")||Sn(s)||s.trim()==="---"));t.push({label:o,blocks:r.blocks}),a=r.nextIndex}return{block:{kind:"practice",parts:t},nextIndex:a}}let nn=null;function $a(){return nn||(nn=L(Ha)),nn}const En="Move Your Attention: Focused Attention and Body Awareness Meditation",Ka="/attention-body-awareness/",st=Ka,Ja="PEX-IMPLEMENTATION-PLACEHOLDER";let tn=null;function Fe(){const n=Zt.trim();return!n||n.includes(Ja)?!1:n.startsWith(`# ${En}`)}function Qa(){if(!Fe())throw new Error("Chapter 5 manuscript has not been supplied by PEX_PRIMARY");return tn||(tn=L(Zt)),tn}const Xa=[{id:"home",title:"Psychical Excursion: Lucid Dreaming, Meditation, Visualization & OBE Research",path:"/psychical-excursion/",legacyHashes:["/","#/","#"],description:"A free researched practice guide for lucid dreaming, meditation, visualization, and out-of-body experience. Public pages use aggregate analytics. Journal entries stay on this device."},{id:"chapter01",title:"Remember Your Dreams: Dream Recall Techniques and Research",path:"/dream-recall/",legacyHashes:["#/you-are-dreaming-remember"],description:"Dream recall techniques and research for noticing and remembering more of the night’s dreams."},{id:"chapter02",title:"Notice Your Dreams: Dream Awareness, Patterns and Dream Signs",path:"/dream-awareness-signs/",legacyHashes:["#/you-are-dreaming-notice"],description:"How to notice dream awareness, repeating patterns, and dream signs without forcing a story onto the night."},{id:"chapter03",title:"Recognize the Dream: Lucid Dreaming, Reality Checks and Dream Signs",path:"/lucid-dreaming-reality-checks/",legacyHashes:["#/you-are-dreaming-recognize"],description:"Lucid dreaming practice that uses reality checks and dream signs to recognize the dream while it is happening."},{id:"chapter04",title:"Feel the Body: Body Scan Meditation and Deep Relaxation",path:"/body-scan-meditation/",legacyHashes:["#/feel-the-body"],description:"Body scan meditation and deep relaxation as a way to feel the body before sleep-edge practice."},{id:"chapter05",title:"Move Your Attention: Focused Attention and Body Awareness Meditation",path:"/attention-body-awareness/",legacyHashes:["#/move-your-attention"],description:"Focused attention and body-awareness meditation for moving attention through the body without forcing results."},{id:"chapter06",title:"Build the Current: Tingling, Energy Sensations and Focused Attention",path:"/energy-sensations-meditation/",legacyHashes:["#/build-the-current"],description:"Tingling and energy-like sensations as objects of focused attention, kept distinct from interpretation."},{id:"chapter07",title:"Quiet the Mind: Meditation Techniques for Sleep and Dream Awareness",path:"/meditation-for-lucid-dreaming/",legacyHashes:["#/quiet-the-mind"],description:"Meditation techniques that quiet mental noise around sleep so dream awareness can be noticed."},{id:"chapter08",title:"See the Image: Visualization, Mental Imagery and Hypnagogic Imagery",path:"/visualization-hypnagogic-imagery/",legacyHashes:["#/see-the-image"],description:"Visualization and hypnagogic imagery as they appear near sleep, including how images form and fade."},{id:"chapter09",title:"Watch the Edge: Hypnagogia and the Transition Into Lucid Dreaming",path:"/hypnagogia-lucid-dreaming/",legacyHashes:["#/watch-the-edge"],description:"Watching hypnagogia and the sleep-edge transition without treating every image as a finished dream."},{id:"chapter10",title:"Let the Body Sleep: The “Mind Awake, Body Asleep” Route to Lucid Dreaming",path:"/mind-awake-body-asleep/",legacyHashes:["#/let-the-body-sleep"],description:"The mind-awake, body-asleep route to lucid dreaming, including how awareness and the sleeping body can diverge."},{id:"chapter11",title:"Move Without Moving: Motor Imagery, Dream Movement and Sleep-Onset Practice",path:"/motor-imagery-lucid-dreaming/",legacyHashes:["#/move-without-moving"],description:"Motor imagery and imagined movement while the physical body stays still, as sleep-onset practice."},{id:"chapter12",title:"Feel the Shift: Vibrations, Floating and Out-of-Body Sensations Near Sleep",path:"/out-of-body-sensations-sleep/",legacyHashes:["#/feel-the-shift"],description:"Vibrations, floating, and out-of-body sensations near sleep, tracked as changes in bodily self rather than a single event."},{id:"chapter13",title:"Know the Threshold: How to Recognize When Waking Imagery Becomes a Dream",path:"/entering-a-lucid-dream/",legacyHashes:["#/know-the-threshold"],description:"How to notice the threshold where waking imagery begins to continue as a dream, without diagnosing a sleep stage."},{id:"chapter14",title:"Stabilize the Dream: Lucid Dream Stabilization Techniques and Research",path:"/lucid-dream-stabilization/",legacyHashes:["#/stabilize-the-dream"],description:"Lucid dream stabilization techniques and research, keeping lucidity, stability, and control distinct."},{id:"chapter15",title:"Explore the Dream: Lucid Dream Experiments, Dream Control and Research",path:"/lucid-dream-experiments/",legacyHashes:["#/explore-the-dream"],description:"Lucid dream experiments that keep exploration, dream control, and externally verified facts distinct."},{id:"chapter16",title:"Loosen the Body: Body Ownership, Self-Location and Out-of-Body Experience",path:"/out-of-body-experience-body-ownership/",legacyHashes:["#/loosen-the-body"],description:"How body ownership, self-location, perspective, and vestibular-motor experience can loosen independently near sleep, without treating an OBE as proven literal separation."}],_e={pages:Xa},$n=_e.pages,Za=_e.pages[0].title,te=_e.pages[0].path;_e.pages[0].description;function je(n){if(!n)return te;const e=n.split("?")[0]||"/",t=e.startsWith("/")?e:`/${e}`;return t==="/"?te:t.endsWith("/")?t:`${t}/`}function eo(n){return $n.find(e=>e.id===n)}function no(n){const e=je(n);return $n.find(t=>t.path===e)}function to(n){return eo(n==="home"?"home":n)}function ao(n){return`${n} | Psychical Excursion`}function oo(n){const e=n.trim();return!e||e==="#"||e==="#/"?"#/":e}function oa(n){if(!n.startsWith("#/"))return null;const e=n.replace(/^#/,"");if(!e||e==="/")return{path:te,fragment:""};if(!e.startsWith("/"))return null;const t=e.split("?")[0]||"/",[a,o=""]=t.split("#"),i=oo(`#${a||"/"}`),r=$n.find(s=>s.legacyHashes.includes(i));return r?{path:r.path,fragment:o}:null}const io=`# Notice Your Dreams: Dream Awareness, Patterns and Dream Signs
...
`,lt="Notice Your Dreams: Dream Awareness, Patterns and Dream Signs",ro="/dream-awareness-signs/",ht=ro;let an=null;function so(){return an||(an=L(io)),an}const ia=`# Recognize the Dream: Lucid Dreaming, Reality Checks and Dream Signs
...
`,Rn="Recognize the Dream: Lucid Dreaming, Reality Checks and Dream Signs",lo="/lucid-dreaming-reality-checks/",ct=lo,ho="PEX-IMPLEMENTATION-PLACEHOLDER";let on=null;function co(){const n=ia.trim();return!n||n.includes(ho)?!1:n.startsWith(`# ${Rn}`)}function uo(){if(!co())throw new Error("Chapter 3 manuscript has not been supplied by PEX_PRIMARY");return on||(on=L(ia)),on}const mo=`# Feel the Body: Body Scan Meditation and Deep Relaxation
...
`,dt="Feel the Body: Body Scan Meditation and Deep Relaxation",po="/body-scan-meditation/",ut=po;let rn=null;function go(){return rn||(rn=L(mo)),rn}const yo=`# Build the Current: Tingling, Energy Sensations and Focused Attention
...
`,mt="Build the Current: Tingling, Energy Sensations and Focused Attention",fo="/energy-sensations-meditation/",pt=fo;let sn=null;function bo(){return sn||(sn=L(yo)),sn}const wo=`# Quiet the Mind: Meditation Techniques for Sleep and Dream Awareness
...
`,gt="Quiet the Mind: Meditation Techniques for Sleep and Dream Awareness",vo="/meditation-for-lucid-dreaming/",yt=vo;let ln=null;function To(){return ln||(ln=L(wo)),ln}const ko=`# See the Image: Visualization, Mental Imagery and Hypnagogic Imagery
...
`,ft="See the Image: Visualization, Mental Imagery and Hypnagogic Imagery",Io="/visualization-hypnagogic-imagery/",bt=Io;let hn=null;function xo(){return hn||(hn=L(ko)),hn}const Ao=`# Watch the Edge: Hypnagogia and the Transition Into Lucid Dreaming
...
`,wt="Watch the Edge: Hypnagogia and the Transition Into Lucid Dreaming",So="/hypnagogia-lucid-dreaming/",vt=So;let cn=null;function Eo(){return cn||(cn=L(Ao)),cn}const Ro=`# Let the Body Sleep: The “Mind Awake, Body Asleep” Route to Lucid Dreaming
...
`,Tt="Let the Body Sleep: The “Mind Awake, Body Asleep” Route to Lucid Dreaming",Mo="/mind-awake-body-asleep/",kt=Mo;let dn=null;function Do(){return dn||(dn=L(Ro)),dn}const No=`# Move Without Moving: Motor Imagery, Dream Movement and Sleep-Onset Practice
...
`,It="Move Without Moving: Motor Imagery, Dream Movement and Sleep-Onset Practice",Co="/motor-imagery-lucid-dreaming/",xt=Co;let un=null;function Oo(){return un||(un=L(No)),un}const Po=`# Feel the Shift: Vibrations, Floating and Out-of-Body Sensations Near Sleep
...
`,At="Feel the Shift: Vibrations, Floating and Out-of-Body Sensations Near Sleep",Lo="/out-of-body-sensations-sleep/",St=Lo;let mn=null;function Fo(){return mn||(mn=L(Po)),mn}const Wo=`# Know the Threshold: How to Recognize When Waking Imagery Becomes a Dream
...
`,Et="Know the Threshold: How to Recognize When Waking Imagery Becomes a Dream",Ho="/entering-a-lucid-dream/",Rt=Ho;let pn=null;function Yo(){return pn||(pn=L(Wo)),pn}const zo=`# Stabilize the Dream: Lucid Dream Stabilization Techniques and Research
...
`,Mt="Stabilize the Dream: Lucid Dream Stabilization Techniques and Research",Bo="/lucid-dream-stabilization/",Dt=Bo;let gn=null;function qo(){return gn||(gn=L(zo)),gn}const _o=`# Explore the Dream: Lucid Dream Experiments, Dream Control and Research
...
`,Nt="Explore the Dream: Lucid Dream Experiments, Dream Control and Research",jo="/lucid-dream-experiments/",Ct=jo;let yn=null;function Go(){return yn||(yn=L(_o)),yn}const Uo=`# Loosen the Body: Body Ownership, Self-Location and Out-of-Body Experience
...
`,mr="An experiment in dreams, consciousness, energy, and out-of-body experience";function pr(n){const e=n.trim(),t=e.match(/^#\s+(.+)\n+/);if(!t)throw new Error("Guidebook manuscript must begin with a top-level heading");const a=t[1].trim(),i=e.slice(t[0].length).split(/\n## /),r=Gt(i[0]??""),s=[];let l=[];for(let h=1;h<i.length;h+=1){const d=i[h],c=d.indexOf(`
`),g=(c===-1?d:d.slice(0,c)).trim(),b=c===-1?"":d.slice(c+1);if(g==="References"){l=gr(b);continue}s.push({heading:g,paragraphs:Gt(b)})}return{openingHeading:a,openingParagraphs:r,sections:s,references:l}}function Gt(n){return n.split(/\n---\n/).flatMap(e=>e.split(/\n\n+/)).map(e=>e.replace(/\n/g," ").trim()).filter(Boolean)}function gr(n){return n.split(/\n+/).map(e=>e.trim()).filter(Boolean)}let Tn=null;function yr(){return Tn||(Tn=pr(ur)),Tn}function Ut(n){return n.replace(/[.,;:!?)]+$/u,"")}function qe(n,e={}){const{linkCitations:t=!0}=e,a=m("span",{class:"guidebook-rich-text"}),o=t?/(\[[^\]]+\]\((?:pex:[a-z0-9-]+|#\/[^)\s]+)\)|\*\*[^*]+\*\*|\*[^*]+\*|\[\d+\]|https:\/\/\S+|doi:10\.\S+)/gi:/(\[[^\]]+\]\((?:pex:[a-z0-9-]+|#\/[^)\s]+)\)|\*\*[^*]+\*\*|\*[^*]+\*|https:\/\/\S+|doi:10\.\S+)/gi;let i=0;for(const r of n.matchAll(o)){const s=r.index??0;s>i&&a.append(n.slice(i,s));const l=r[0],h=l.match(/^\[([^\]]+)\]\((pex:[a-z0-9-]+|#\/[^)\s]+)\)$/i);if(h){const d=h[1],c=h[2],g=c.toLowerCase().startsWith("pex:")?c.slice(4).toLowerCase():"",b=g?_a[g]:void 0,f=b?.href??(c===xn||c.endsWith(`#${Un}`)?xn:c),T=m("a",{href:f,class:"guidebook-pex-link","data-pex-link":b?.id??In}),M=d.match(/^\*\*(.+)\*\*$/);M?T.append(m("strong",{},[M[1]])):T.append(d),a.append(T)}else if(l.startsWith("**"))a.append(m("strong",{},[l.slice(2,-2)]));else if(l.startsWith("*"))a.append(m("em",{},[l.slice(1,-1)]));else if(/^\[\d+\]$/.test(l)&&t){const d=l.slice(1,-1);a.append(m("a",{href:`#ref-${d}`,class:"guidebook-citation"},[l]))}else if(l.toLowerCase().startsWith("doi:10.")){const d=Ut(l.slice(4));a.append(m("a",{href:`https://doi.org/${d}`,class:"guidebook-doi",rel:"noreferrer"},[l]))}else if(l.toLowerCase().startsWith("https://")){const d=Ut(l);a.append(m("a",{href:d,class:"guidebook-external",rel:"noreferrer"},[l]))}else a.append(l);i=s+l.length}return i<n.length&&a.append(n.slice(i)),a.childNodes.length||a.append(n),a}function qn(n,e){const t=m("p",{});return t.append(qe(n,e)),t}function Ea(n){const e=n.match(/^(?:\*\*)?\[(\d+)\](?:\*\*)?\s*(.*)$/),t=e?.[1]??"0",a=e?.[2]??n,o=m("li",{id:`ref-${t}`,class:"guidebook-reference-item"}),i=m("span",{class:"guidebook-ref-num"},[`[${t}] `]);return o.append(i,qe(a,{linkCitations:!1})),o.tabIndex=-1,o}function fr(n){for(const e of n.querySelectorAll("a.guidebook-citation"))e.addEventListener("click",t=>{const o=(e.getAttribute("href")??"").match(/^#ref-(\d+)$/);if(!o)return;const i=n.querySelector(`#ref-${o[1]}`);if(!i)return;t.preventDefault();const r=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;i.scrollIntoView({behavior:r?"auto":"smooth",block:"start"}),i.focus({preventScroll:!0})});for(const e of n.querySelectorAll("a.guidebook-pex-link"))e.addEventListener("click",t=>{const a=e.getAttribute("href")??"";let o="",i=!0;try{const l=new URL(a,window.location.href);o=l.hash.replace(/^#/,"");const h=window.location.pathname.replace(/\/?$/,"/"),d=l.pathname.replace(/\/?$/,"/");i=d===h||d==="/"||a.startsWith("#")}catch{o=a.includes("#")?a.split("#").pop()??"":""}if(!o||o.startsWith("/")||!i)return;const r=n.querySelector(`[id="${o}"]`);if(!r)return;t.preventDefault(),r.closest(".guidebook-section")?.classList.add("is-visible");const s=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;r.scrollIntoView({behavior:s?"auto":"smooth",block:"start"}),r.focus({preventScroll:!0})})}function Pe(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function br(n){const e=yr(),t=m("article",{class:"guidebook-article"});let a=Pe(!0);t.append(a),a.append(m("p",{class:"guidebook-subtitle"},[mr]),m("h1",{class:"guidebook-opening-title"},[Za]),m("p",{class:"guidebook-opening-heading"},[e.openingHeading]),...e.openingParagraphs.map(o=>qn(o)));for(const o of e.sections){a=Pe(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title"},[o.heading]));for(const i of o.paragraphs){if(i.startsWith("**")&&i.endsWith("**")){a.append(m("p",{class:"guidebook-emphasis-line"},[i.slice(2,-2)]));continue}a.append(qn(i))}}if(e.references.length){a=Pe(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const o=m("ul",{class:"guidebook-references"});for(const i of e.references)o.append(Ea(i));a.append(o)}a=Pe(!1),t.append(a),a.append(m("p",{class:"guidebook-next"},[m("a",{href:na,class:"guidebook-next-link",id:"guidebook-next-chapter"},[ea,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])])),n.append(t)}const Le={innerSeconds:180,midSeconds:240,outerSeconds:300,breatheSeconds:96},j=500,G=500;function v(n,e={}){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,o]of Object.entries(e))t.setAttribute(a,o);return t}function x(n,e){const t=(e-90)*Math.PI/180;return[j+n*Math.cos(t),G+n*Math.sin(t)]}function Ra(n,e,t=0){return Array.from({length:n},(a,o)=>x(e,360/n*o+t).join(",")).join(" ")}function Vt(n,e,t,a=0){const o=[];for(let i=0;i<n;i+=1)o.push(x(e,360/n*i+a).join(",")),o.push(x(t,360/n*i+180/n+a).join(","));return o.join(" ")}function kn(n,e,t,a){return[x(t,n),x((e+t)/2,n-a),x(e,n),x((e+t)/2,n+a)].map(o=>o.join(",")).join(" ")}function wr(n,e,t,a){return[x(t,n),x(e,n-a),x(e,n+a)].map(o=>o.join(",")).join(" ")}function _(n,e,t,a,o){n.append(v("polygon",{class:o,points:Ra(e,t,a)}))}function vr(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function Tr(){const n=vr(),e=m("figure",{class:n?"pex-attention-instrument is-static":"pex-attention-instrument"});e.setAttribute("role","img"),e.setAttribute("aria-label","Original Psychical Excursion attention instrument: nested thin-line geometry around a fixed center. Looking toward the center is optional. Seeing no afterimage or mental picture is not required.");const t=v("svg",{viewBox:"0 0 1000 1000",focusable:"false"});t.style.setProperty("--pex-attention-inner",`${Le.innerSeconds}s`),t.style.setProperty("--pex-attention-mid",`${Le.midSeconds}s`),t.style.setProperty("--pex-attention-outer",`${Le.outerSeconds}s`),t.style.setProperty("--pex-attention-breathe",`${Le.breatheSeconds}s`);const a=v("g",{class:"pex-attention-layer pex-attention-beyond"});a.append(v("polygon",{class:"pex-attention-line pex-attention-faintest",points:Ra(36,486,5)})),a.append(v("circle",{class:"pex-attention-line pex-attention-faintest",cx:String(j),cy:String(G),r:"478"}));for(let h=0;h<72;h+=1){const d=h*5,c=x(468,d),g=x(h%6===0?492:484,d);a.append(v("line",{class:"pex-attention-line pex-attention-faintest",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const o=v("g",{class:"pex-attention-layer pex-attention-spin-outer"});o.append(v("circle",{class:"pex-attention-line pex-attention-strong",cx:String(j),cy:String(G),r:"438"})),o.append(v("circle",{class:"pex-attention-line",cx:String(j),cy:String(G),r:"418"}));for(let h=0;h<144;h+=1){const d=h*2.5,c=h%12===0,g=h%4===0,b=x(c?398:g?406:410,d),f=x(438,d);o.append(v("line",{class:c?"pex-attention-line pex-attention-strong":"pex-attention-line pex-attention-fine",x1:b[0].toFixed(2),y1:b[1].toFixed(2),x2:f[0].toFixed(2),y2:f[1].toFixed(2)}))}for(let h=0;h<8;h+=1){const d=h*45;o.append(v("polygon",{class:"pex-attention-line",points:kn(d,442,462,3.2)}));const c=x(h%2===0?454:448,d);o.append(v("circle",{class:"pex-attention-fill pex-attention-fine",cx:c[0].toFixed(2),cy:c[1].toFixed(2),r:h%2===0?"2.1":"1.3"}))}for(const h of[18,138,258])o.append(v("path",{class:"pex-attention-line pex-attention-arc",d:kr(394,h,h+84)}));const i=v("g",{class:"pex-attention-layer pex-attention-spin-mid"});for(const h of[372,344,316,288])i.append(v("circle",{class:"pex-attention-line",cx:String(j),cy:String(G),r:String(h)}));_(i,12,372,0,"pex-attention-line pex-attention-soft"),_(i,12,372,15,"pex-attention-line pex-attention-soft");for(let h=0;h<24;h+=1)i.append(v("polygon",{class:"pex-attention-line",points:kn(h*15,300,368,5.4)}));for(let h=0;h<36;h+=1)i.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:wr(h*10,248,286,3.6)}));for(let h=0;h<24;h+=1){const d=h*15,c=x(132,d),g=x(h%2===0?368:316,d);i.append(v("line",{class:h%2===0?"pex-attention-line":"pex-attention-line pex-attention-fine",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const r=v("g",{class:"pex-attention-layer pex-attention-breathe"});_(r,8,236,0,"pex-attention-line pex-attention-strong"),_(r,8,236,22.5,"pex-attention-line"),_(r,6,204,0,"pex-attention-line"),_(r,6,204,30,"pex-attention-line"),_(r,4,176,0,"pex-attention-line pex-attention-soft"),_(r,4,176,45,"pex-attention-line pex-attention-soft"),r.append(v("polygon",{class:"pex-attention-line",points:Vt(16,220,148,0)})),r.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:Vt(12,190,126,15)}));for(const h of[168,148,128])r.append(v("circle",{class:"pex-attention-line pex-attention-fine",cx:String(j),cy:String(G),r:String(h)}));const s=v("g",{class:"pex-attention-layer pex-attention-spin-inner"});for(let h=0;h<12;h+=1){const[d,c]=x(34,h*30);s.append(v("circle",{class:"pex-attention-line",cx:d.toFixed(2),cy:c.toFixed(2),r:"34"}))}for(let h=0;h<8;h+=1)s.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:kn(h*45+22.5,58,108,8)}));_(s,16,96,0,"pex-attention-line pex-attention-fine"),_(s,8,72,0,"pex-attention-line"),s.append(v("circle",{class:"pex-attention-line",cx:String(j),cy:String(G),r:"54"}));for(let h=0;h<32;h+=1){const d=h*11.25,c=x(46,d),g=x(h%4===0?70:62,d);s.append(v("line",{class:"pex-attention-line pex-attention-fine",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const l=v("g",{class:"pex-attention-layer pex-attention-center"});return l.append(v("circle",{class:"pex-attention-line pex-attention-strong",cx:String(j),cy:String(G),r:"18"})),l.append(v("circle",{class:"pex-attention-line pex-attention-fine",cx:String(j),cy:String(G),r:"8"})),l.append(v("circle",{class:"pex-attention-fill pex-attention-center-point",cx:String(j),cy:String(G),r:"2.2"})),t.append(a,o,i,r,s,l),e.append(t),e}function kr(n,e,t){const a=x(n,e),o=x(n,t),i=t-e>180?1:0;return`M ${a[0].toFixed(2)} ${a[1].toFixed(2)} A ${n} ${n} 0 ${i} 1 ${o[0].toFixed(2)} ${o[1].toFixed(2)}`}function ue(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function Ma(n,e){for(const t of e){if(t.kind==="emphasis"){n.append(m("p",{class:"guidebook-emphasis-line"},[t.text]));continue}if(t.kind==="quote"){const a=m("blockquote",{class:"guidebook-pull"});a.append(qe(t.text)),n.append(a);continue}if(t.kind==="list"){const a=m(t.ordered?"ol":"ul",{class:"guidebook-steps"});for(const o of t.items){const i=m("li",{});i.append(qe(o)),a.append(i)}n.append(a);continue}if(t.kind==="rule"){n.append(m("hr",{class:"guidebook-rule"}));continue}if(!(t.kind==="heading"||t.kind==="practice"||t.kind==="attention")){if(t.kind==="subheading"){n.append(m("h3",{class:"guidebook-practice-subheading"},[t.text]));continue}n.append(qn(t.text))}}}function Ir(n){const e=m("section",{class:"guidebook-practice-part"});return e.append(m("h2",{class:"guidebook-practice-label"},[n.label])),Ma(e,n.blocks),e}function xr(n){return m("nav",{class:"guidebook-prev","aria-label":"Previous reading"},[m("a",{href:n.href,class:"guidebook-prev-link",id:n.id},[m("span",{class:"guidebook-prev-arrow","aria-hidden":"true"},["← "]),n.title])])}function O(n,e,t={}){const a=m("article",{class:"guidebook-article guidebook-chapter"});let o=ue(!0);a.append(o),t.previous&&o.append(xr(t.previous)),o.append(m("h1",{class:"guidebook-chapter-title"},[e.title]));for(const i of e.blocks){if(i.kind==="heading"){o=ue(!1),a.append(o);const r=m("h2",{class:"guidebook-section-title"},[i.text]),s=qa(i.text);s&&(r.id=s,r.tabIndex=-1),o.append(r);continue}if(i.kind==="practice"){o=ue(!1),a.append(o);const r=m("aside",{class:"guidebook-practice","aria-label":"Summary, experiment, and intention"});for(const s of i.parts)r.append(Ir(s));o.append(r);continue}if(i.kind==="attention"){o=ue(!1),a.append(o),o.append(Tr());continue}Ma(o,[i])}if(e.references.length){o=ue(!1),a.append(o),o.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const i=m("ul",{class:"guidebook-references"});for(const r of e.references)i.append(Ea(r));o.append(i)}t.next&&(o=ue(!1),a.append(o),o.append(m("p",{class:"guidebook-next"},[m("a",{href:t.next.href,class:"guidebook-next-link",id:t.next.id},[t.next.title,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])]))),n.append(a)}function I(n,e){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,o]of Object.entries(e))t.setAttribute(a,o);return t}function Ar(n){const e=m("div",{class:`pex-ambient pex-ambient-${n}`,"aria-hidden":"true"}),t=I("svg",{viewBox:"0 0 1200 900",focusable:"false"}),a=I("g",{class:"pex-ambient-spin pex-ambient-spin-slow"});a.append(I("circle",{class:"pex-ambient-arc",cx:"620",cy:"430",r:"318"}),I("circle",{class:"pex-ambient-arc pex-ambient-arc-faint",cx:"620",cy:"430",r:"214"}),I("circle",{class:"pex-ambient-point",cx:"938",cy:"430",r:"2.2"}),I("circle",{class:"pex-ambient-point",cx:"406",cy:"238",r:"1.6"}));const o=I("g",{class:"pex-ambient-spin pex-ambient-spin-mid"});o.append(I("ellipse",{class:"pex-ambient-arc",cx:"580",cy:"400",rx:"430",ry:"168"}),I("path",{class:"pex-ambient-arc pex-ambient-arc-partial",d:"M220 520 C 380 220, 820 180, 1040 470"}),I("circle",{class:"pex-ambient-point",cx:"1010",cy:"400",r:"1.8"}));const i=I("g",{class:"pex-ambient-axis"});if(i.append(I("line",{class:"pex-ambient-line",x1:"600",y1:"40",x2:"600",y2:"860"}),I("line",{class:"pex-ambient-line pex-ambient-line-soft",x1:"80",y1:"390",x2:"1120",y2:"510"})),t.append(i,a,o),n==="memory"){const r=I("g",{class:"pex-ambient-memory-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"430",cy:"360",rx:"92",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"760",cy:"500",rx:"70",ry:"110"}),I("circle",{class:"pex-ambient-form pex-ambient-form-c",cx:"620",cy:"280",r:"36"})),t.append(r)}if(n==="notice"||n==="recognize"){const r=I("g",{class:"pex-ambient-notice-align"});r.append(I("path",{class:"pex-ambient-arc pex-ambient-align-a",d:"M260 300 C 480 220, 700 240, 940 360"}),I("path",{class:"pex-ambient-arc pex-ambient-align-b",d:"M300 620 C 520 420, 760 380, 980 520"}),I("circle",{class:"pex-ambient-point pex-ambient-align-point",cx:"640",cy:"390",r:"2.4"})),n==="recognize"&&r.append(I("circle",{class:"pex-ambient-form pex-ambient-recognize-ring",cx:"640",cy:"390",r:"5.5"})),t.append(r)}if(n==="body"){const r=I("g",{class:"pex-ambient-body-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"620",cy:"318",rx:"36",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"620",cy:"468",rx:"58",ry:"92"}),I("circle",{class:"pex-ambient-point",cx:"620",cy:"390",r:"1.8"})),t.append(r)}return e.append(t),e}const Sr=[{name:"Aries",glyph:"♈︎"},{name:"Taurus",glyph:"♉︎"},{name:"Gemini",glyph:"♊︎"},{name:"Cancer",glyph:"♋︎"},{name:"Leo",glyph:"♌︎"},{name:"Virgo",glyph:"♍︎"},{name:"Libra",glyph:"♎︎"},{name:"Scorpio",glyph:"♏︎"},{name:"Sagittarius",glyph:"♐︎"},{name:"Capricorn",glyph:"♑︎"},{name:"Aquarius",glyph:"♒︎"},{name:"Pisces",glyph:"♓︎"}];function $t(n){const e=(n%360+360)%360;return Sr[Math.floor(e/30)%12]}function Er(n){const e=new Intl.DateTimeFormat(void 0,{month:"short",day:"numeric"}).format(n),t=new Intl.DateTimeFormat(void 0,{hour:"numeric",minute:"2-digit"}).format(n),a=new Intl.DateTimeFormat(void 0,{month:"long",day:"numeric",hour:"numeric",minute:"2-digit"}).format(n);return{date:e,time:t,spoken:a}}function Kt(n,e,t,a){const o=n==="sun"?`Sun in ${t}`:`Moon in ${t}`;return m("span",{class:"sky-widget-pair",tabindex:"0",role:"img","aria-label":o,"data-sky-body":n},[m("span",{class:"sky-widget-body","aria-hidden":"true"},[e]),m("span",{class:"sky-widget-sign","aria-hidden":"true"},[a]),m("span",{class:"sky-widget-tip","aria-hidden":"true"},[o])])}function Rr(){const n=new Date,e=cr(n),t=$t(e.sun.eclipticLongitudeDeg),a=$t(e.moon.eclipticLongitudeDeg),o=sr(n),i=o?Er(o):null,r=i?`Sun in ${t.name}. Moon in ${a.name}. Next full moon ${i.spoken}.`:`Sun in ${t.name}. Moon in ${a.name}.`,s=i?m("span",{class:"sky-widget-full"},[m("span",{class:"sky-widget-full-label"},["Full Moon · "]),m("span",{class:"sky-widget-full-when"},[`${i.date} · ${i.time}`])]):m("span",{class:"sky-widget-full"},["Full Moon"]);return m("div",{class:"sky-widget-compact",role:"group","aria-label":r},[Kt("sun",`${xa}${jt}`,t.name,t.glyph),Kt("moon",`${Aa}${jt}`,a.name,a.glyph),s])}function Mr(n){return n==="chapter01"?"memory":n==="chapter02"?"notice":n==="chapter03"?"recognize":n==="chapter04"?"body":"orbit"}function Dr(n){const e=to(n);return e?ao(e.title):"Psychical Excursion"}function Nr(n,e){Gn(),document.title=Dr(e),n.replaceChildren();const t=m("a",{class:"skip-link",href:"#main"},["Skip to content"]),a=Mr(e),o=Ar(a),i=Or(),r=m("header",{class:"app-header guidebook-header"},[m("a",{href:te,class:"brand-link","aria-label":"Psychical Excursion home"},[m("img",{class:"brand-logo brand-logo-light",src:"/brand/pex-logo-primary.svg",alt:"",width:"220",height:"52",decoding:"async"}),m("img",{class:"brand-logo brand-logo-reverse",src:"/brand/pex-logo-primary-reverse.svg",alt:"",width:"220",height:"52",decoding:"async"})]),m("div",{class:"header-tools guidebook-tools"},[Rr(),m("span",{class:"guidebook-tools-rule","aria-hidden":"true"}),i])]),s=m("div",{id:"live-status",class:"visually-hidden","aria-live":"polite"}),l=m("main",{id:"main",class:"main-stage guidebook-main",tabindex:"-1","data-guidebook-page":e}),h=m("footer",{class:"site-footer guidebook-footer"},[m("p",{class:"attribution"},[si("Website by "),m("a",{href:"https://hoopsnakedesigns.com/",rel:"noreferrer"},["Hoopsnake Designs"])])]),d=m("div",{class:"app-frame guidebook-frame","data-ambient":a},[r,s,l,h]);return n.append(t,o,d),{main:l}}function Cr(n){fr(n)}function Or(){const n=jn()==="bedtime",e=m("button",{type:"button",id:"theme-light-dark",class:"theme-switch",role:"switch",dir:"ltr","aria-checked":n?"true":"false","aria-label":"Dark appearance"},[m("span",{class:"theme-switch-track","aria-hidden":"true",dir:"ltr"},[m("span",{class:"theme-switch-thumb"})])]);return e.addEventListener("click",()=>{const a=Wa()==="bedtime";e.setAttribute("aria-checked",a?"true":"false")}),e}const _n=[];function Da(){for(;_n.length;)_n.pop()?.()}function Pr(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function Lr(n){Da();const e=[...n.querySelectorAll(".pex-reveal")];if(e.length===0)return;if(Pr()||typeof IntersectionObserver!="function"){for(const a of e)a.classList.add("is-visible");return}e[0]?.classList.add("is-visible");const t=new IntersectionObserver(a=>{for(const o of a)o.isIntersecting&&(o.target.classList.add("is-visible"),t.unobserve(o.target))},{threshold:.08,rootMargin:"0px 0px -10% 0px"});for(const a of e)t.observe(a);_n.push(()=>t.disconnect())}const Jt=[];function Fr(){for(;Jt.length;)Jt.pop()?.()}async function Wr(n){{if(ra())return;li(),dr(),Fr(),Da();const e=Zo(),t=Xo(),a=ei(e);a&&!t&&bn(),ni(e);const{main:o}=Nr(n,e),i={href:te,title:"Introduction",id:"guidebook-prev-home"};if(e==="chapter01"?O(o,$a(),{previous:i,next:{href:ht,title:lt,id:"guidebook-next-chapter-2"}}):e==="chapter02"?O(o,so(),{previous:{href:Ga,title:ea,id:"guidebook-prev-chapter-1"},next:{href:ct,title:Rn,id:"guidebook-next-chapter-3"}}):e==="chapter03"?O(o,uo(),{previous:{href:ht,title:lt,id:"guidebook-prev-chapter-2"},next:{href:ut,title:dt,id:"guidebook-next-chapter-4"}}):e==="chapter04"?O(o,go(),{previous:{href:ct,title:Rn,id:"guidebook-prev-chapter-3"},next:Fe()?{href:st,title:En,id:"guidebook-next-chapter-5"}:void 0}):e==="chapter05"&&Fe()?O(o,Qa(),{previous:{href:ut,title:dt,id:"guidebook-prev-chapter-4"},next:{href:pt,title:mt,id:"guidebook-next-chapter-6"}}):e==="chapter06"?O(o,bo(),{previous:{href:st,title:En,id:"guidebook-prev-chapter-5"},next:{href:yt,title:gt,id:"guidebook-next-chapter-7"}}):e==="chapter07"?O(o,To(),{previous:{href:pt,title:mt,id:"guidebook-prev-chapter-6"},next:{href:bt,title:ft,id:"guidebook-next-chapter-8"}}):e==="chapter08"?O(o,xo(),{previous:{href:yt,title:gt,id:"guidebook-prev-chapter-7"},next:{href:vt,title:wt,id:"guidebook-next-chapter-9"}}):e==="chapter09"?O(o,Eo(),{previous:{href:bt,title:ft,id:"guidebook-prev-chapter-8"},next:{href:kt,title:Tt,id:"guidebook-next-chapter-10"}}):e==="chapter10"?O(o,Do(),{previous:{href:vt,title:wt,id:"guidebook-prev-chapter-9"},next:{href:xt,title:It,id:"guidebook-next-chapter-11"}}):e==="chapter11"?O(o,Oo(),{previous:{href:kt,title:Tt,id:"guidebook-prev-chapter-10"},next:{href:St,title:At,id:"guidebook-next-chapter-12"}}):e==="chapter12"?O(o,Fo(),{previous:{href:xt,title:It,id:"guidebook-prev-chapter-11"},next:{href:Rt,title:Et,id:"guidebook-next-chapter-13"}}):e==="chapter13"?O(o,Yo(),{previous:{href:St,title:At,id:"guidebook-prev-chapter-12"},next:{href:Dt,title:Mt,id:"guidebook-next-chapter-14"}}):e==="chapter14"?O(o,qo(),{previous:{href:Rt,title:Et,id:"guidebook-prev-chapter-13"},next:{href:Ct,title:Nt,id:"guidebook-next-chapter-15"}}):e==="chapter15"?O(o,Go(),{previous:{href:Dt,title:Mt,id:"guidebook-prev-chapter-14"},next:{href:Ko,title:Vo,id:"guidebook-next-chapter-16"}}):e==="chapter16"?O(o,Jo(),{previous:{href:Ct,title:Nt,id:"guidebook-prev-chapter-15"}}):br(o),Lr(o),Cr(n),oi(),t)ti(n,t);else if(a){bn();const r=navigator.userAgent??"";typeof requestAnimationFrame=="function"&&r.length>0&&!r.includes("jsdom")&&requestAnimationFrame(()=>bn())}return}}function Hr(){const n=document.getElementById("app");if(!n)throw new Error("Missing #app");return n}async function Yr(){Gn(),La();const n=Hr(),e=()=>{Wr(n)};if(!ra()){window.addEventListener("popstate",e),window.addEventListener("hashchange",()=>{window.location.hash.startsWith("#/")&&e()}),e();try{await Qt.open(),await Fa()}catch{}}}Yr();
