const ut="0.6.0";const Qa="pex-local";const y={entries:"entries",media:"media",progress:"progress",settings:"settings"};function oe(n){const e=typeof n.updatedAt=="number"?n.updatedAt:n.createdAt;return{id:n.id,type:n.type,createdAt:n.createdAt,updatedAt:e,note:typeof n.note=="string"?n.note:"",audioId:n.audioId??null,audioMimeType:n.audioMimeType??null,audioByteLength:n.audioByteLength??null,syncState:n.syncState??"LOCAL",localSafeAt:n.localSafeAt??e,syncVersion:typeof n.syncVersion=="number"?n.syncVersion:1,remoteVersion:n.remoteVersion??null,remoteFileId:n.remoteFileId??null,remoteMediaFileId:n.remoteMediaFileId??null,syncErrorCode:n.syncErrorCode??null,pexDay:n.pexDay??null,phaseId:n.phaseId??null}}class U extends Error{code;recoverable;constructor(e,t,a=!0){super(t),this.name="AppError",this.code=e,this.recoverable=a}}function en(n){return{day:n,unlocked:!0,visitedAt:null,completedAt:null}}function De(n){return{day:n.day,unlocked:!0,visitedAt:typeof n.visitedAt=="number"?n.visitedAt:null,completedAt:typeof n.completedAt=="number"?n.completedAt:null}}function N(n){return new Promise((e,t)=>{n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error??new Error("indexeddb-request-failed"))})}function P(n){return new Promise((e,t)=>{n.oncomplete=()=>e(),n.onerror=()=>t(n.error??new Error("indexeddb-transaction-failed")),n.onabort=()=>t(n.error??new Error("indexeddb-transaction-aborted"))})}function we(n){const e=n instanceof DOMException?n.name:"",t=n instanceof Error?n.message:String(n);return e==="QuotaExceededError"||/quota/i.test(t)?new U("quota-exceeded","This browser does not have enough space to save. Export existing entries if you can, then free space and retry."):new U("save-failed","The local save did not complete. Your entry was not discarded silently; retry the save.")}class Xa{constructor(e=globalThis.indexedDB??null){this.factory=e}factory;assertAvailable(){if(!this.factory)throw new U("indexeddb-unavailable","IndexedDB is not available in this browser, so journal entries cannot be stored locally.",!1)}async open(){this.assertAvailable();try{const e=this.factory.open(Qa,3);e.onupgradeneeded=i=>{const o=e.result,r=e.transaction;if(o.objectStoreNames.contains(y.entries)){if(i.oldVersion<2){const s=r.objectStore(y.entries);s.indexNames.contains("syncState")||s.createIndex("syncState","syncState")}}else{const s=o.createObjectStore(y.entries,{keyPath:"id"});s.createIndex("createdAt","createdAt"),s.createIndex("type","type"),s.createIndex("syncState","syncState")}o.objectStoreNames.contains(y.media)||o.createObjectStore(y.media,{keyPath:"id"}).createIndex("entryId","entryId",{unique:!0}),o.objectStoreNames.contains(y.progress)||o.createObjectStore(y.progress,{keyPath:"day"}),o.objectStoreNames.contains(y.settings)||o.createObjectStore(y.settings,{keyPath:"key"})};const t=await N(e);return await this.ensureSeed(t)&&(await this.migrateLegacyEntries(t),await this.bumpSchemaRecord(t)),t}catch(e){throw e instanceof U?e:new U("indexeddb-open-failed","The local database could not be opened.",!1)}}async ensureSeed(e){const t=e.transaction([y.progress,y.settings],"readwrite"),a=t.objectStore(y.progress),i=t.objectStore(y.settings);for(let s=1;s<=60;s+=1){const l=await N(a.get(s));l?a.put(De(l)):a.put(en(s))}const o=await N(i.get("schema"));let r=!1;return o?o.schemaVersion<3&&(r=!0):i.put({key:"schema",schemaVersion:3,appVersion:ut,createdAt:Date.now()}),await P(t),r}async bumpSchemaRecord(e){const t=e.transaction(y.settings,"readwrite"),a=t.objectStore(y.settings),i=await N(a.get("schema"));i&&a.put({...i,schemaVersion:3,appVersion:ut}),await P(t)}async migrateLegacyEntries(e){const t=e.transaction(y.entries,"readwrite"),a=t.objectStore(y.entries),i=await N(a.getAll());for(const o of i)o.syncState===void 0&&a.put(oe(o));await P(t)}async listEntries(){const e=await this.open();try{const t=e.transaction(y.entries,"readonly"),a=t.objectStore(y.entries),i=await N(a.getAll());return await P(t),i.map(o=>oe(o)).sort((o,r)=>r.createdAt-o.createdAt)}finally{e.close()}}async getEntry(e){const t=await this.open();try{const a=t.transaction([y.entries,y.media],"readonly"),i=await N(a.objectStore(y.entries).get(e));if(!i)return await P(a),null;const o=oe(i);let r=null;return o.audioId&&(r=await N(a.objectStore(y.media).get(o.audioId))??null),await P(a),{entry:o,media:r}}finally{t.close()}}async saveCapture(e){const t=await this.open(),a=e.createdAt,i=oe({id:e.id,type:e.type,createdAt:a,updatedAt:a,note:e.note.trim(),audioId:e.audio?.id??null,audioMimeType:e.audio?.mimeType??null,audioByteLength:e.audio?e.audio.blob.size:null,syncState:e.syncState??"LOCAL",localSafeAt:a,syncVersion:e.syncVersion??1,remoteVersion:e.remoteVersion??null,remoteFileId:e.remoteFileId??null,remoteMediaFileId:e.remoteMediaFileId??null,syncErrorCode:null,pexDay:e.pexDay??null,phaseId:e.phaseId??null});try{const o=t.transaction(e.audio?[y.entries,y.media]:y.entries,"readwrite");if(o.objectStore(y.entries).put(i),e.audio){const r={id:e.audio.id,entryId:i.id,mimeType:e.audio.mimeType,blob:e.audio.blob,byteLength:e.audio.blob.size,createdAt:e.createdAt};o.objectStore(y.media).put(r)}return await P(o),i}catch(o){throw we(o)}finally{t.close()}}async updateNote(e,t){const a=await this.open();try{const i=a.transaction(y.entries,"readwrite"),o=i.objectStore(y.entries),r=await N(o.get(e));if(!r)throw new U("save-failed","That entry is no longer in local storage.");const s=oe(r),l=oe({...s,note:t.trim(),updatedAt:Date.now(),syncVersion:s.syncVersion+1,syncState:s.syncState==="SYNCED"?"PENDING_SYNC":s.syncState});return o.put(l),await P(i),l}catch(i){throw i instanceof U?i:we(i)}finally{a.close()}}async updateSyncState(e,t){const a=await this.open();try{const i=a.transaction(y.entries,"readwrite"),o=i.objectStore(y.entries),r=await N(o.get(e));if(!r)throw new U("save-failed","That entry is no longer in local storage.");const s=oe({...r,...t,updatedAt:Date.now()});return o.put(s),await P(i),s}catch(i){throw i instanceof U?i:we(i)}finally{a.close()}}async deleteEntry(e){const t=await this.open();try{const a=t.transaction([y.entries,y.media],"readwrite"),i=await N(a.objectStore(y.entries).get(e));i?.audioId&&a.objectStore(y.media).delete(i.audioId),a.objectStore(y.entries).delete(e),await P(a)}catch{throw new U("delete-failed","The entry could not be deleted. It should still be present in the journal.")}finally{t.close()}}async listProgress(){const e=await this.open();try{const t=e.transaction(y.progress,"readonly"),a=await N(t.objectStore(y.progress).getAll());return await P(t),a.map(i=>De(i)).sort((i,o)=>i.day-o.day)}finally{e.close()}}async getDayProgress(e){const t=await this.open();try{const a=t.transaction(y.progress,"readonly"),i=await N(a.objectStore(y.progress).get(e));return await P(a),i?De(i):en(e)}finally{t.close()}}async writeDayProgress(e,t){const a=await this.open();try{const i=a.transaction([y.progress,y.settings],"readwrite"),o=i.objectStore(y.progress),r=De(await N(o.get(e))??en(e)),s=t(r);return o.put(s),i.objectStore(y.settings).put({key:"resumeDay",value:e}),await P(i),s}catch(i){throw we(i)}finally{a.close()}}async markDayVisited(e){return this.writeDayProgress(e,t=>({...t,unlocked:!0,visitedAt:Date.now()}))}async completeDay(e){return this.writeDayProgress(e,t=>({...t,unlocked:!0,visitedAt:t.visitedAt??Date.now(),completedAt:Date.now()}))}async undoDayCompletion(e){return this.writeDayProgress(e,t=>({...t,unlocked:!0,completedAt:null}))}async loadResumeDay(){return this.getSetting("resumeDay")}async getSetting(e){const t=await this.open();try{const a=t.transaction(y.settings,"readonly"),i=await N(a.objectStore(y.settings).get(e));return await P(a),i?.value}finally{t.close()}}async setSetting(e,t){const a=await this.open();try{const i=a.transaction(y.settings,"readwrite");i.objectStore(y.settings).put({key:e,value:t}),await P(i)}catch(i){throw we(i)}finally{a.close()}}async getSchemaInfo(){const e=await this.open();try{const t=e.transaction(y.settings,"readonly"),a=await N(t.objectStore(y.settings).get("schema"));return await P(t),a}finally{e.close()}}async savePersistenceReport(e){await this.setSetting("persistenceReport",e)}async loadPersistenceReport(){return this.getSetting("persistenceReport")}async exportBundle(){const e=await this.open();try{const t=e.transaction([y.entries,y.media],"readonly"),a=await N(t.objectStore(y.entries).getAll()),i=await N(t.objectStore(y.media).getAll());return await P(t),{entries:a.sort((o,r)=>o.createdAt-r.createdAt),media:i}}finally{e.close()}}}const ga=new Xa;function Za(){typeof window>"u"||"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").then(n=>{n.addEventListener("updatefound",()=>{const e=n.installing;e&&e.addEventListener("statechange",()=>{e.state==="installed"&&navigator.serviceWorker.controller&&document.dispatchEvent(new CustomEvent("pex-sw-update"))})})})})}async function ei(n=navigator.storage){const e=[],t=!!n,a=typeof n?.persist=="function";let i=null,o=!1,r=null,s=null,l=null;if(!t)e.push("navigator.storage is not present. Persistence cannot be requested in this browser.");else{if(typeof n?.persisted=="function")try{i=await n.persisted()}catch{e.push("storage.persisted() threw; treated as unknown.")}else e.push("storage.persisted() is not present.");if(typeof n?.estimate=="function")try{const d=await n.estimate();s=typeof d.usage=="number"?d.usage:null,l=typeof d.quota=="number"?d.quota:null}catch{e.push("storage.estimate() threw.")}else e.push("storage.estimate() is not present.");if(a&&i!==!0){o=!0;try{r=await n.persist(),r!==!0&&e.push("persist() returned a non-true result. The UI must not claim persistence was granted.")}catch{r=!1,e.push("persist() threw. Persistence is not claimed.")}}else a?(r=!0,e.push("Storage was already marked persisted before this request.")):e.push("storage.persist() is not present.")}const h={inspectedAt:Date.now(),storageApiPresent:t,persistApiPresent:a,persistedBeforeRequest:i,persistRequestAttempted:o,persistGranted:r,estimateUsageBytes:s,estimateQuotaBytes:l,notes:e};try{await ga.savePersistenceReport(h)}catch{e.push("The persistence report could not be stored in IndexedDB.")}return h}const ya="pex-theme";function $n(){try{const n=localStorage.getItem(ya);if(n==="bedtime"||n==="light")return n}catch{}return"light"}function Qn(n=$n()){const e=document.documentElement;e.dataset.theme=n;const t=n==="bedtime"?"#0c0c0b":"#f7f5ef",a=document.querySelector('meta[name="theme-color"]');a&&a.setAttribute("content",t);try{localStorage.setItem(ya,n)}catch{}return n}function ni(){const n=$n()==="bedtime"?"light":"bedtime";return Qn(n)}const fa=`# Move Your Attention: Focused Attention and Body Awareness Meditation
...
`,ti=`# Remember Your Dreams: Dream Recall Techniques and Research
...
`,pt="Notice Your Dreams: Dream Awareness, Patterns and Dream Signs",xi="/dream-awareness-signs/",gt=xi;let an=null;function Ai(){return an||(an=S(Ii)),an}const Ia=`# Recognize the Dream: Lucid Dreaming, Reality Checks and Dream Signs
...
`,Pn="Recognize the Dream: Lucid Dreaming, Reality Checks and Dream Signs",Si="/lucid-dreaming-reality-checks/",yt=Si,Ri="PEX-IMPLEMENTATION-PLACEHOLDER";let on=null;function Ei(){const n=Ia.trim();return!n||n.includes(Ri)?!1:n.startsWith(`# ${Pn}`)}function Mi(){if(!Ei())throw new Error("Chapter 3 manuscript has not been supplied by PEX_PRIMARY");return on||(on=S(Ia)),on}const Di=`# Feel the Body: Body Scan Meditation and Deep Relaxation
...
`,ft="Feel the Body: Body Scan Meditation and Deep Relaxation",Ci="/body-scan-meditation/",bt=Ci;let rn=null;function Ni(){return rn||(rn=S(Di)),rn}const Oi=`# Build the Current: Tingling, Energy Sensations and Focused Attention
...
`,wt="Build the Current: Tingling, Energy Sensations and Focused Attention",Pi="/energy-sensations-meditation/",vt=Pi;let sn=null;function Li(){return sn||(sn=S(Oi)),sn}const Wi=`# Quiet the Mind: Meditation Techniques for Sleep and Dream Awareness
...
`,Tt="Quiet the Mind: Meditation Techniques for Sleep and Dream Awareness",Bi="/meditation-for-lucid-dreaming/",kt=Bi;let ln=null;function Fi(){return ln||(ln=S(Wi)),ln}const zi=`# See the Image: Visualization, Mental Imagery and Hypnagogic Imagery
...
`,It="See the Image: Visualization, Mental Imagery and Hypnagogic Imagery",Hi="/visualization-hypnagogic-imagery/",xt=Hi;let hn=null;function Yi(){return hn||(hn=S(zi)),hn}const qi=`# Watch the Edge: Hypnagogia and the Transition Into Lucid Dreaming
...
`,At="Watch the Edge: Hypnagogia and the Transition Into Lucid Dreaming",ji="/hypnagogia-lucid-dreaming/",St=ji;let cn=null;function _i(){return cn||(cn=S(qi)),cn}const Gi=`# Let the Body Sleep: The “Mind Awake, Body Asleep” Route to Lucid Dreaming
...
`,Rt="Let the Body Sleep: The “Mind Awake, Body Asleep” Route to Lucid Dreaming",Ui="/mind-awake-body-asleep/",Et=Ui;let dn=null;function Vi(){return dn||(dn=S(Gi)),dn}const Ji=`# Move Without Moving: Motor Imagery, Dream Movement and Sleep-Onset Practice
...
`,Mt="Move Without Moving: Motor Imagery, Dream Movement and Sleep-Onset Practice",Ki="/motor-imagery-lucid-dreaming/",Dt=Ki;let un=null;function $i(){return un||(un=S(Ji)),un}const Qi=`# Feel the Shift: Vibrations, Floating and Out-of-Body Sensations Near Sleep
...
The previous chapter practiced intentional movement while the physical body remained still. Here we begin listening for movement that appears on its own.
...
`,Ct="Feel the Shift: Vibrations, Floating and Out-of-Body Sensations Near Sleep",Xi="/out-of-body-sensations-sleep/",Nt=Xi;let mn=null;function Zi(){return mn||(mn=S(Qi)),mn}const eo=`# Know the Threshold: How to Recognize When Waking Imagery Becomes a Dream
...
`,Ot="Know the Threshold: How to Recognize When Waking Imagery Becomes a Dream",no="/entering-a-lucid-dream/",Pt=no;let pn=null;function to(){return pn||(pn=S(eo)),pn}const ao=`# Stabilize the Dream: Lucid Dream Stabilization Techniques and Research
...
`,Lt="Stabilize the Dream: Lucid Dream Stabilization Techniques and Research",io="/lucid-dream-stabilization/",Wt=io;let gn=null;function oo(){return gn||(gn=S(ao)),gn}const ro=`# Explore the Dream: Lucid Dream Experiments, Dream Control and Research
...
`,Bt="Explore the Dream: Lucid Dream Experiments, Dream Control and Research",so="/lucid-dream-experiments/",Ft=so;let yn=null;function lo(){return yn||(yn=S(ro)),yn}const ho=`# Loosen the Body: Body Ownership, Self-Location and Out-of-Body Experience
...
`,zt="Loosen the Body: Body Ownership, Self-Location and Out-of-Body Experience",co="/out-of-body-experience-body-ownership/",Ht=co;let fn=null;function uo(){return fn||(fn=S(ho)),fn}const mo=`# Cross the Threshold: Astral Projection, OBE Techniques and Lucid Dreaming
...
`,Yt="Cross the Threshold: Astral Projection, OBE Techniques and Lucid Dreaming",po="/astral-projection-obe-techniques/",qt=po;let bn=null;function go(){return bn||(bn=S(mo)),bn}const yo=`# Test the Experience: Can Lucid Dreams and Out-of-Body Experiences Be Verified?
...
`,jt="Test the Experience: Can Lucid Dreams and Out-of-Body Experiences Be Verified?",fo="/testing-out-of-body-experiences/",_t=fo;let wn=null;function bo(){return wn||(wn=S(yo)),wn}const wo=`# Compare the Maps: Lucid Dreaming vs. Astral Projection, OBE and Sleep Paralysis
...
`,Gt="Compare the Maps: Lucid Dreaming vs. Astral Projection, OBE and Sleep Paralysis",vo="/lucid-dreaming-vs-astral-projection/",Ut=vo;let vn=null;function To(){return vn||(vn=S(wo)),vn}const ko=`# Floating in Space: Sun, Moon, Planets and the Science of Sleep & Dreams
...
`,Vt="Floating in Space: Sun, Moon, Planets and the Science of Sleep & Dreams",Io="/sun-moon-planets-sleep-dreams/",Jt=Io;let Tn=null;function xo(){return Tn||(Tn=S(ko)),Tn}const Ao=`# Notice the Coincidence: Synchronicity, Recurring Dreams, Shared Dreams and Pattern Recognition
...
`,Kt="Notice the Coincidence: Synchronicity, Recurring Dreams, Shared Dreams and Pattern Recognition",So="/synchronicity-recurring-shared-dreams/",$t=So;let kn=null;function Ro(){return kn||(kn=S(Ao)),kn}const Eo=`# Return. Record. Repeat: The Best Bedtime Routine for Lucid Dreaming and Astral Projection
...
`,Kr="An experiment in dreams, consciousness, energy, and out-of-body experience";function $r(n){const e=n.trim(),t=e.match(/^#\s+(.+)\n+/);if(!t)throw new Error("Guidebook manuscript must begin with a top-level heading");const a=t[1].trim(),o=e.slice(t[0].length).split(/\n## /),r=ha(o[0]??""),s=[];let l=[];for(let h=1;h<o.length;h+=1){const d=o[h],c=d.indexOf(`
`),g=(c===-1?d:d.slice(0,c)).trim(),b=c===-1?"":d.slice(c+1);if(g==="References"){l=Qr(b);continue}s.push({heading:g,paragraphs:ha(b)})}return{openingHeading:a,openingParagraphs:r,sections:s,references:l}}function ha(n){return n.split(/\n---\n/).flatMap(e=>e.split(/\n\n+/)).map(e=>e.replace(/\n/g," ").trim()).filter(Boolean)}function Qr(n){return n.split(/\n+/).map(e=>e.trim()).filter(Boolean)}let Rn=null;function Xr(){return Rn||(Rn=$r(Jr)),Rn}function ca(n){return n.replace(/[.,;:!?)]+$/u,"")}function qe(n,e={}){const{linkCitations:t=!0}=e,a=m("span",{class:"guidebook-rich-text"}),i=t?/(\[[^\]]+\]\((?:pex:[a-z0-9-]+|#\/[^)\s]+)\)|\*\*[^*]+\*\*|\*[^*]+\*|\[\d+\]|https:\/\/\S+|doi:10\.\S+)/gi:/(\[[^\]]+\]\((?:pex:[a-z0-9-]+|#\/[^)\s]+)\)|\*\*[^*]+\*\*|\*[^*]+\*|https:\/\/\S+|doi:10\.\S+)/gi;let o=0;for(const r of n.matchAll(i)){const s=r.index??0;s>o&&a.append(n.slice(o,s));const l=r[0],h=l.match(/^\[([^\]]+)\]\((pex:[a-z0-9-]+|#\/[^)\s]+)\)$/i);if(h){const d=h[1],c=h[2],g=c.toLowerCase().startsWith("pex:")?c.slice(4).toLowerCase():"",b=g?si[g]:void 0,f=b?.href??(c===Dn||c.endsWith(`#${Xn}`)?Dn:c),T=m("a",{href:f,class:"guidebook-pex-link","data-pex-link":b?.id??Mn}),C=d.match(/^\*\*(.+)\*\*$/);C?T.append(m("strong",{},[C[1]])):T.append(d),a.append(T)}else if(l.startsWith("**"))a.append(m("strong",{},[l.slice(2,-2)]));else if(l.startsWith("*"))a.append(m("em",{},[l.slice(1,-1)]));else if(/^\[\d+\]$/.test(l)&&t){const d=l.slice(1,-1);a.append(m("a",{href:`#ref-${d}`,class:"guidebook-citation"},[l]))}else if(l.toLowerCase().startsWith("doi:10.")){const d=ca(l.slice(4));a.append(m("a",{href:`https://doi.org/${d}`,class:"guidebook-doi",rel:"noreferrer"},[l]))}else if(l.toLowerCase().startsWith("https://")){const d=ca(l);a.append(m("a",{href:d,class:"guidebook-external",rel:"noreferrer"},[l]))}else a.append(l);o=s+l.length}return o<n.length&&a.append(n.slice(o)),a.childNodes.length||a.append(n),a}function Jn(n,e){const t=m("p",{});return t.append(qe(n,e)),t}function Ga(n){const e=n.match(/^(?:\*\*)?\[(\d+)\](?:\*\*)?\s*(.*)$/),t=e?.[1]??"0",a=e?.[2]??n,i=m("li",{id:`ref-${t}`,class:"guidebook-reference-item"}),o=m("span",{class:"guidebook-ref-num"},[`[${t}] `]);return i.append(o,qe(a,{linkCitations:!1})),i.tabIndex=-1,i}function Zr(n){for(const e of n.querySelectorAll("a.guidebook-citation"))e.addEventListener("click",t=>{const i=(e.getAttribute("href")??"").match(/^#ref-(\d+)$/);if(!i)return;const o=n.querySelector(`#ref-${i[1]}`);if(!o)return;t.preventDefault();const r=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;o.scrollIntoView({behavior:r?"auto":"smooth",block:"start"}),o.focus({preventScroll:!0})});for(const e of n.querySelectorAll("a.guidebook-pex-link"))e.addEventListener("click",t=>{const a=e.getAttribute("href")??"";let i="",o=!0;try{const l=new URL(a,window.location.href);i=l.hash.replace(/^#/,"");const h=window.location.pathname.replace(/\/?$/,"/"),d=l.pathname.replace(/\/?$/,"/");o=d===h||d==="/"||a.startsWith("#")}catch{i=a.includes("#")?a.split("#").pop()??"":""}if(!i||i.startsWith("/")||!o)return;const r=n.querySelector(`[id="${i}"]`);if(!r)return;t.preventDefault(),r.closest(".guidebook-section")?.classList.add("is-visible");const s=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;r.scrollIntoView({behavior:s?"auto":"smooth",block:"start"}),r.focus({preventScroll:!0})})}function Pe(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function es(n){const e=Xr(),t=m("article",{class:"guidebook-article"});let a=Pe(!0);t.append(a),a.append(m("p",{class:"guidebook-subtitle"},[Kr]),m("h1",{class:"guidebook-opening-title"},[fi]),m("p",{class:"guidebook-opening-heading"},[e.openingHeading]),...e.openingParagraphs.map(i=>Jn(i)));for(const i of e.sections){a=Pe(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title"},[i.heading]));for(const o of i.paragraphs){if(o.startsWith("**")&&o.endsWith("**")){a.append(m("p",{class:"guidebook-emphasis-line"},[o.slice(2,-2)]));continue}a.append(Jn(o))}}if(e.references.length){a=Pe(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const i=m("ul",{class:"guidebook-references"});for(const o of e.references)i.append(Ga(o));a.append(i)}a=Pe(!1),t.append(a),a.append(m("p",{class:"guidebook-next"},[m("a",{href:wa,class:"guidebook-next-link",id:"guidebook-next-chapter"},[ba,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])])),n.append(t)}const Le={innerSeconds:180,midSeconds:240,outerSeconds:300,breatheSeconds:96},_=500,G=500;function v(n,e={}){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,i]of Object.entries(e))t.setAttribute(a,i);return t}function A(n,e){const t=(e-90)*Math.PI/180;return[_+n*Math.cos(t),G+n*Math.sin(t)]}function Ua(n,e,t=0){return Array.from({length:n},(a,i)=>A(e,360/n*i+t).join(",")).join(" ")}function da(n,e,t,a=0){const i=[];for(let o=0;o<n;o+=1)i.push(A(e,360/n*o+a).join(",")),i.push(A(t,360/n*o+180/n+a).join(","));return i.join(" ")}function En(n,e,t,a){return[A(t,n),A((e+t)/2,n-a),A(e,n),A((e+t)/2,n+a)].map(i=>i.join(",")).join(" ")}function ns(n,e,t,a){return[A(t,n),A(e,n-a),A(e,n+a)].map(i=>i.join(",")).join(" ")}function j(n,e,t,a,i){n.append(v("polygon",{class:i,points:Ua(e,t,a)}))}function ts(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function as(){const n=ts(),e=m("figure",{class:n?"pex-attention-instrument is-static":"pex-attention-instrument"});e.setAttribute("role","img"),e.setAttribute("aria-label","Original Psychical Excursion attention instrument: nested thin-line geometry around a fixed center. Looking toward the center is optional. Seeing no afterimage or mental picture is not required.");const t=v("svg",{viewBox:"0 0 1000 1000",focusable:"false"});t.style.setProperty("--pex-attention-inner",`${Le.innerSeconds}s`),t.style.setProperty("--pex-attention-mid",`${Le.midSeconds}s`),t.style.setProperty("--pex-attention-outer",`${Le.outerSeconds}s`),t.style.setProperty("--pex-attention-breathe",`${Le.breatheSeconds}s`);const a=v("g",{class:"pex-attention-layer pex-attention-beyond"});a.append(v("polygon",{class:"pex-attention-line pex-attention-faintest",points:Ua(36,486,5)})),a.append(v("circle",{class:"pex-attention-line pex-attention-faintest",cx:String(_),cy:String(G),r:"478"}));for(let h=0;h<72;h+=1){const d=h*5,c=A(468,d),g=A(h%6===0?492:484,d);a.append(v("line",{class:"pex-attention-line pex-attention-faintest",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const i=v("g",{class:"pex-attention-layer pex-attention-spin-outer"});i.append(v("circle",{class:"pex-attention-line pex-attention-strong",cx:String(_),cy:String(G),r:"438"})),i.append(v("circle",{class:"pex-attention-line",cx:String(_),cy:String(G),r:"418"}));for(let h=0;h<144;h+=1){const d=h*2.5,c=h%12===0,g=h%4===0,b=A(c?398:g?406:410,d),f=A(438,d);i.append(v("line",{class:c?"pex-attention-line pex-attention-strong":"pex-attention-line pex-attention-fine",x1:b[0].toFixed(2),y1:b[1].toFixed(2),x2:f[0].toFixed(2),y2:f[1].toFixed(2)}))}for(let h=0;h<8;h+=1){const d=h*45;i.append(v("polygon",{class:"pex-attention-line",points:En(d,442,462,3.2)}));const c=A(h%2===0?454:448,d);i.append(v("circle",{class:"pex-attention-fill pex-attention-fine",cx:c[0].toFixed(2),cy:c[1].toFixed(2),r:h%2===0?"2.1":"1.3"}))}for(const h of[18,138,258])i.append(v("path",{class:"pex-attention-line pex-attention-arc",d:is(394,h,h+84)}));const o=v("g",{class:"pex-attention-layer pex-attention-spin-mid"});for(const h of[372,344,316,288])o.append(v("circle",{class:"pex-attention-line",cx:String(_),cy:String(G),r:String(h)}));j(o,12,372,0,"pex-attention-line pex-attention-soft"),j(o,12,372,15,"pex-attention-line pex-attention-soft");for(let h=0;h<24;h+=1)o.append(v("polygon",{class:"pex-attention-line",points:En(h*15,300,368,5.4)}));for(let h=0;h<36;h+=1)o.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:ns(h*10,248,286,3.6)}));for(let h=0;h<24;h+=1){const d=h*15,c=A(132,d),g=A(h%2===0?368:316,d);o.append(v("line",{class:h%2===0?"pex-attention-line":"pex-attention-line pex-attention-fine",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const r=v("g",{class:"pex-attention-layer pex-attention-breathe"});j(r,8,236,0,"pex-attention-line pex-attention-strong"),j(r,8,236,22.5,"pex-attention-line"),j(r,6,204,0,"pex-attention-line"),j(r,6,204,30,"pex-attention-line"),j(r,4,176,0,"pex-attention-line pex-attention-soft"),j(r,4,176,45,"pex-attention-line pex-attention-soft"),r.append(v("polygon",{class:"pex-attention-line",points:da(16,220,148,0)})),r.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:da(12,190,126,15)}));for(const h of[168,148,128])r.append(v("circle",{class:"pex-attention-line pex-attention-fine",cx:String(_),cy:String(G),r:String(h)}));const s=v("g",{class:"pex-attention-layer pex-attention-spin-inner"});for(let h=0;h<12;h+=1){const[d,c]=A(34,h*30);s.append(v("circle",{class:"pex-attention-line",cx:d.toFixed(2),cy:c.toFixed(2),r:"34"}))}for(let h=0;h<8;h+=1)s.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:En(h*45+22.5,58,108,8)}));j(s,16,96,0,"pex-attention-line pex-attention-fine"),j(s,8,72,0,"pex-attention-line"),s.append(v("circle",{class:"pex-attention-line",cx:String(_),cy:String(G),r:"54"}));for(let h=0;h<32;h+=1){const d=h*11.25,c=A(46,d),g=A(h%4===0?70:62,d);s.append(v("line",{class:"pex-attention-line pex-attention-fine",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:g[0].toFixed(2),y2:g[1].toFixed(2)}))}const l=v("g",{class:"pex-attention-layer pex-attention-center"});return l.append(v("circle",{class:"pex-attention-line pex-attention-strong",cx:String(_),cy:String(G),r:"18"})),l.append(v("circle",{class:"pex-attention-line pex-attention-fine",cx:String(_),cy:String(G),r:"8"})),l.append(v("circle",{class:"pex-attention-fill pex-attention-center-point",cx:String(_),cy:String(G),r:"2.2"})),t.append(a,i,o,r,s,l),e.append(t),e}function is(n,e,t){const a=A(n,e),i=A(n,t),o=t-e>180?1:0;return`M ${a[0].toFixed(2)} ${a[1].toFixed(2)} A ${n} ${n} 0 ${o} 1 ${i[0].toFixed(2)} ${i[1].toFixed(2)}`}function ue(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function Va(n,e,t="guidebook-section-subheading"){for(const a of e){if(a.kind==="emphasis"){n.append(m("p",{class:"guidebook-emphasis-line"},[a.text]));continue}if(a.kind==="quote"){const i=m("blockquote",{class:"guidebook-pull"});i.append(qe(a.text)),n.append(i);continue}if(a.kind==="list"){const i=m(a.ordered?"ol":"ul",{class:"guidebook-steps"});for(const o of a.items){const r=m("li",{});r.append(qe(o)),i.append(r)}n.append(i);continue}if(a.kind==="rule"){n.append(m("hr",{class:"guidebook-rule"}));continue}if(!(a.kind==="heading"||a.kind==="practice"||a.kind==="attention")){if(a.kind==="subheading"){n.append(m("h3",{class:t},[a.text]));continue}n.append(Jn(a.text))}}}function os(n){const e=m("section",{class:"guidebook-practice-part"});return e.append(m("h2",{class:"guidebook-practice-label"},[n.label])),Va(e,n.blocks,"guidebook-practice-subheading"),e}function rs(n){return m("nav",{class:"guidebook-prev","aria-label":"Previous reading"},[m("a",{href:n.href,class:"guidebook-prev-link",id:n.id},[m("span",{class:"guidebook-prev-arrow","aria-hidden":"true"},["← "]),n.title])])}function x(n,e,t={}){const a=m("article",{class:"guidebook-article guidebook-chapter"});let i=ue(!0);a.append(i),t.previous&&i.append(rs(t.previous)),i.append(m("h1",{class:"guidebook-chapter-title"},[e.title]));for(const o of e.blocks){if(o.kind==="heading"){i=ue(!1),a.append(i);const r=m("h2",{class:"guidebook-section-title"},[o.text]),s=ri(o.text);s&&(r.id=s,r.tabIndex=-1),i.append(r);continue}if(o.kind==="practice"){i=ue(!1),a.append(i);const r=m("aside",{class:"guidebook-practice","aria-label":"Summary, experiment, and intention"});for(const s of o.parts)r.append(os(s));i.append(r);continue}if(o.kind==="attention"){i=ue(!1),a.append(i),i.append(as());continue}Va(i,[o])}if(e.references.length){i=ue(!1),a.append(i),i.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const o=m("ul",{class:"guidebook-references"});for(const r of e.references)o.append(Ga(r));i.append(o)}t.next&&(i=ue(!1),a.append(i),i.append(m("p",{class:"guidebook-next"},[m("a",{href:t.next.href,class:"guidebook-next-link",id:t.next.id},[t.next.title,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])]))),n.append(a)}function I(n,e){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,i]of Object.entries(e))t.setAttribute(a,i);return t}function ss(n){const e=m("div",{class:`pex-ambient pex-ambient-${n}`,"aria-hidden":"true"}),t=I("svg",{viewBox:"0 0 1200 900",focusable:"false"}),a=I("g",{class:"pex-ambient-spin pex-ambient-spin-slow"});a.append(I("circle",{class:"pex-ambient-arc",cx:"620",cy:"430",r:"318"}),I("circle",{class:"pex-ambient-arc pex-ambient-arc-faint",cx:"620",cy:"430",r:"214"}),I("circle",{class:"pex-ambient-point",cx:"938",cy:"430",r:"2.2"}),I("circle",{class:"pex-ambient-point",cx:"406",cy:"238",r:"1.6"}));const i=I("g",{class:"pex-ambient-spin pex-ambient-spin-mid"});i.append(I("ellipse",{class:"pex-ambient-arc",cx:"580",cy:"400",rx:"430",ry:"168"}),I("path",{class:"pex-ambient-arc pex-ambient-arc-partial",d:"M220 520 C 380 220, 820 180, 1040 470"}),I("circle",{class:"pex-ambient-point",cx:"1010",cy:"400",r:"1.8"}));const o=I("g",{class:"pex-ambient-axis"});if(o.append(I("line",{class:"pex-ambient-line",x1:"600",y1:"40",x2:"600",y2:"860"}),I("line",{class:"pex-ambient-line pex-ambient-line-soft",x1:"80",y1:"390",x2:"1120",y2:"510"})),t.append(o,a,i),n==="memory"){const r=I("g",{class:"pex-ambient-memory-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"430",cy:"360",rx:"92",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"760",cy:"500",rx:"70",ry:"110"}),I("circle",{class:"pex-ambient-form pex-ambient-form-c",cx:"620",cy:"280",r:"36"})),t.append(r)}if(n==="notice"||n==="recognize"){const r=I("g",{class:"pex-ambient-notice-align"});r.append(I("path",{class:"pex-ambient-arc pex-ambient-align-a",d:"M260 300 C 480 220, 700 240, 940 360"}),I("path",{class:"pex-ambient-arc pex-ambient-align-b",d:"M300 620 C 520 420, 760 380, 980 520"}),I("circle",{class:"pex-ambient-point pex-ambient-align-point",cx:"640",cy:"390",r:"2.4"})),n==="recognize"&&r.append(I("circle",{class:"pex-ambient-form pex-ambient-recognize-ring",cx:"640",cy:"390",r:"5.5"})),t.append(r)}if(n==="body"){const r=I("g",{class:"pex-ambient-body-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"620",cy:"318",rx:"36",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"620",cy:"468",rx:"58",ry:"92"}),I("circle",{class:"pex-ambient-point",cx:"620",cy:"390",r:"1.8"})),t.append(r)}return e.append(t),e}const ls=[{name:"Aries",glyph:"♈︎"},{name:"Taurus",glyph:"♉︎"},{name:"Gemini",glyph:"♊︎"},{name:"Cancer",glyph:"♋︎"},{name:"Leo",glyph:"♌︎"},{name:"Virgo",glyph:"♍︎"},{name:"Libra",glyph:"♎︎"},{name:"Scorpio",glyph:"♏︎"},{name:"Sagittarius",glyph:"♐︎"},{name:"Capricorn",glyph:"♑︎"},{name:"Aquarius",glyph:"♒︎"},{name:"Pisces",glyph:"♓︎"}];function ua(n){const e=(n%360+360)%360;return ls[Math.floor(e/30)%12]}function hs(n){const e=new Intl.DateTimeFormat(void 0,{month:"short",day:"numeric"}).format(n),t=new Intl.DateTimeFormat(void 0,{hour:"numeric",minute:"2-digit"}).format(n),a=new Intl.DateTimeFormat(void 0,{month:"long",day:"numeric",hour:"numeric",minute:"2-digit"}).format(n);return{date:e,time:t,spoken:a}}function ma(n,e,t,a){const i=n==="sun"?`Sun in ${t}`:`Moon in ${t}`;return m("span",{class:"sky-widget-pair",tabindex:"0",role:"img","aria-label":i,"data-sky-body":n},[m("span",{class:"sky-widget-body","aria-hidden":"true"},[e]),m("span",{class:"sky-widget-sign","aria-hidden":"true"},[a]),m("span",{class:"sky-widget-tip","aria-hidden":"true"},[i])])}function cs(){const n=new Date,e=Ur(n),t=ua(e.sun.eclipticLongitudeDeg),a=ua(e.moon.eclipticLongitudeDeg),i=jr(n),o=i?hs(i):null,r=o?`Sun in ${t.name}. Moon in ${a.name}. Next full moon ${o.spoken}.`:`Sun in ${t.name}. Moon in ${a.name}.`,s=o?m("span",{class:"sky-widget-full"},[m("span",{class:"sky-widget-full-label"},["Full Moon · "]),m("span",{class:"sky-widget-full-when"},[`${o.date} · ${o.time}`])]):m("span",{class:"sky-widget-full"},["Full Moon"]);return m("div",{class:"sky-widget-compact",role:"group","aria-label":r},[ma("sun",`${qa}${la}`,t.name,t.glyph),ma("moon",`${ja}${la}`,a.name,a.glyph),s])}function ds(n){return n==="chapter01"?"memory":n==="chapter02"?"notice":n==="chapter03"?"recognize":n==="chapter04"?"body":"orbit"}function us(n){const e=vi(n);return e?Ti(e.title):"Psychical Excursion"}function ms(n,e){Qn(),document.title=us(e),n.replaceChildren();const t=m("a",{class:"skip-link",href:"#main"},["Skip to content"]),a=ds(e),i=ss(a),o=gs(),r=m("header",{class:"app-header guidebook-header"},[m("a",{href:te,class:"brand-link","aria-label":"Psychical Excursion home"},[m("img",{class:"brand-logo brand-logo-light",src:"/brand/pex-logo-primary.svg",alt:"",width:"220",height:"52",decoding:"async"}),m("img",{class:"brand-logo brand-logo-reverse",src:"/brand/pex-logo-primary-reverse.svg",alt:"",width:"220",height:"52",decoding:"async"})]),m("div",{class:"header-tools guidebook-tools"},[cs(),m("span",{class:"guidebook-tools-rule","aria-hidden":"true"}),o])]),s=m("div",{id:"live-status",class:"visually-hidden","aria-live":"polite"}),l=m("main",{id:"main",class:"main-stage guidebook-main",tabindex:"-1","data-guidebook-page":e}),h=m("footer",{class:"site-footer guidebook-footer"},[m("p",{class:"attribution"},[jo("Website by "),m("a",{href:"https://hoopsnakedesigns.com/",rel:"noreferrer"},["Hoopsnake Designs"])])]),d=m("div",{class:"app-frame guidebook-frame","data-ambient":a},[r,s,l,h]);return n.append(t,i,d),{main:l}}function ps(n){Zr(n)}function gs(){const n=$n()==="bedtime",e=m("button",{type:"button",id:"theme-light-dark",class:"theme-switch",role:"switch",dir:"ltr","aria-checked":n?"true":"false","aria-label":"Dark appearance"},[m("span",{class:"theme-switch-track","aria-hidden":"true",dir:"ltr"},[m("span",{class:"theme-switch-thumb"})])]);return e.addEventListener("click",()=>{const a=ni()==="bedtime";e.setAttribute("aria-checked",a?"true":"false")}),e}const Kn=[];function Ja(){for(;Kn.length;)Kn.pop()?.()}function ys(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function fs(n){Ja();const e=[...n.querySelectorAll(".pex-reveal")];if(e.length===0)return;if(ys()||typeof IntersectionObserver!="function"){for(const a of e)a.classList.add("is-visible");return}e[0]?.classList.add("is-visible");const t=new IntersectionObserver(a=>{for(const i of a)i.isIntersecting&&(i.target.classList.add("is-visible"),t.unobserve(i.target))},{threshold:.08,rootMargin:"0px 0px -10% 0px"});for(const a of e)t.observe(a);Kn.push(()=>t.disconnect())}const pa=[];function bs(){for(;pa.length;)pa.pop()?.()}async function ws(n){{if(xa())return;_o(),Vr(),bs(),Ja();const e=Lo(),t=Po(),a=Wo(e);a&&!t&&xn(),Bo(e);const{main:i}=ms(n,e),o={href:te,title:"Introduction",id:"guidebook-prev-home"};if(e==="chapter01"?x(i,ui(),{previous:o,next:{href:gt,title:pt,id:"guidebook-next-chapter-2"}}):e==="chapter02"?x(i,Ai(),{previous:{href:hi,title:ba,id:"guidebook-prev-chapter-1"},next:{href:yt,title:Pn,id:"guidebook-next-chapter-3"}}):e==="chapter03"?x(i,Mi(),{previous:{href:gt,title:pt,id:"guidebook-prev-chapter-2"},next:{href:bt,title:ft,id:"guidebook-next-chapter-4"}}):e==="chapter04"?x(i,Ni(),{previous:{href:yt,title:Pn,id:"guidebook-prev-chapter-3"},next:We()?{href:mt,title:On,id:"guidebook-next-chapter-5"}:void 0}):e==="chapter05"&&We()?x(i,gi(),{previous:{href:bt,title:ft,id:"guidebook-prev-chapter-4"},next:{href:vt,title:wt,id:"guidebook-next-chapter-6"}}):e==="chapter06"?x(i,Li(),{previous:{href:mt,title:On,id:"guidebook-prev-chapter-5"},next:{href:kt,title:Tt,id:"guidebook-next-chapter-7"}}):e==="chapter07"?x(i,Fi(),{previous:{href:vt,title:wt,id:"guidebook-prev-chapter-6"},next:{href:xt,title:It,id:"guidebook-next-chapter-8"}}):e==="chapter08"?x(i,Yi(),{previous:{href:kt,title:Tt,id:"guidebook-prev-chapter-7"},next:{href:St,title:At,id:"guidebook-next-chapter-9"}}):e==="chapter09"?x(i,_i(),{previous:{href:xt,title:It,id:"guidebook-prev-chapter-8"},next:{href:Et,title:Rt,id:"guidebook-next-chapter-10"}}):e==="chapter10"?x(i,Vi(),{previous:{href:St,title:At,id:"guidebook-prev-chapter-9"},next:{href:Dt,title:Mt,id:"guidebook-next-chapter-11"}}):e==="chapter11"?x(i,$i(),{previous:{href:Et,title:Rt,id:"guidebook-prev-chapter-10"},next:{href:Nt,title:Ct,id:"guidebook-next-chapter-12"}}):e==="chapter12"?x(i,Zi(),{previous:{href:Dt,title:Mt,id:"guidebook-prev-chapter-11"},next:{href:Pt,title:Ot,id:"guidebook-next-chapter-13"}}):e==="chapter13"?x(i,to(),{previous:{href:Nt,title:Ct,id:"guidebook-prev-chapter-12"},next:{href:Wt,title:Lt,id:"guidebook-next-chapter-14"}}):e==="chapter14"?x(i,oo(),{previous:{href:Pt,title:Ot,id:"guidebook-prev-chapter-13"},next:{href:Ft,title:Bt,id:"guidebook-next-chapter-15"}}):e==="chapter15"?x(i,lo(),{previous:{href:Wt,title:Lt,id:"guidebook-prev-chapter-14"},next:{href:Ht,title:zt,id:"guidebook-next-chapter-16"}}):e==="chapter16"?x(i,uo(),{previous:{href:Ft,title:Bt,id:"guidebook-prev-chapter-15"},next:{href:qt,title:Yt,id:"guidebook-next-chapter-17"}}):e==="chapter17"?x(i,go(),{previous:{href:Ht,title:zt,id:"guidebook-prev-chapter-16"},next:{href:_t,title:jt,id:"guidebook-next-chapter-18"}}):e==="chapter18"?x(i,bo(),{previous:{href:qt,title:Yt,id:"guidebook-prev-chapter-17"},next:{href:Ut,title:Gt,id:"guidebook-next-chapter-19"}}):e==="chapter19"?x(i,To(),{previous:{href:_t,title:jt,id:"guidebook-prev-chapter-18"},next:{href:Jt,title:Vt,id:"guidebook-next-chapter-20"}}):e==="chapter20"?x(i,xo(),{previous:{href:Ut,title:Gt,id:"guidebook-prev-chapter-19"},next:{href:$t,title:Kt,id:"guidebook-next-chapter-21"}}):e==="chapter21"?x(i,Ro(),{previous:{href:Jt,title:Vt,id:"guidebook-prev-chapter-20"},next:{href:Co,title:Mo,id:"guidebook-next-chapter-22"}}):e==="chapter22"?x(i,No(),{previous:{href:$t,title:Kt,id:"guidebook-prev-chapter-21"}}):es(i),fs(i),ps(n),Ho(),t)Fo(n,t);else if(a){xn();const r=navigator.userAgent??"";typeof requestAnimationFrame=="function"&&r.length>0&&!r.includes("jsdom")&&requestAnimationFrame(()=>xn())}return}}function vs(){const n=document.getElementById("app");if(!n)throw new Error("Missing #app");return n}async function Ts(){Qn(),Za();const n=vs(),e=()=>{ws(n)};if(!xa()){window.addEventListener("popstate",e),window.addEventListener("hashchange",()=>{window.location.hash.startsWith("#/")&&e()}),e();try{await ga.open(),await ei()}catch{}}}Ts();
