...
`,gt="You Are Dreaming. Remember.",vn="/you-are-dreaming-remember",Ya=`#${vn}`,un=["Summary","Experiment","Intention"];function _a(n){return un.includes(n)}function mn(n){if(!n.startsWith("### "))return null;const e=n.slice(4).trim();return _a(e)?e:null}function yt(n){const e=n.trim();return!e||e==="---"||n.startsWith("## ")||n.startsWith("### ")||n.startsWith("> ")||/^\d+\.\s/.test(n)||/^-\s/.test(n)}function Fe(n){const e=n.replace(/\r\n/g,`
`).trim().split(`
`);if(!e[0]?.startsWith("# "))throw new Error("Chapter manuscript must begin with a top-level heading");const t=e[0].slice(2).trim(),a=[],o=[];let i=1,r=!1;for(;i<e.length;){const l=e[i];if(r){const c=l.trim();c&&o.push(c),i+=1;continue}if(!l.trim()){i+=1;continue}if(l.trim()==="---"){a.push({kind:"rule"}),i+=1;continue}if(l.startsWith("## ")){const c=l.slice(3).trim();if(c==="References"){r=!0,i+=1;continue}a.push({kind:"heading",text:c}),i+=1;continue}if(mn(l)){const c=qa(e,i);a.push(c.block),i=c.nextIndex,i<e.length&&e[i].trim()==="---"&&(i+=1);continue}if(l.startsWith("> ")){const c=[];for(;i<e.length&&e[i].startsWith("> ");)c.push(e[i].slice(2).trim()),i+=1;a.push({kind:"quote",text:c.join(" ")});continue}if(/^\d+\.\s/.test(l)){const c=[];for(;i<e.length&&/^\d+\.\s/.test(e[i]);)c.push(e[i].replace(/^\d+\.\s+/,"").trim()),i+=1;a.push({kind:"list",items:c,ordered:!0});continue}if(/^-\s/.test(l)){const c=[];for(;i<e.length&&/^-\s/.test(e[i]);)c.push(e[i].replace(/^-\s+/,"").trim()),i+=1;a.push({kind:"list",items:c,ordered:!1});continue}const d=[];for(;i<e.length;){const c=e[i];if(yt(c)&&c.trim()!==""||!c.trim())break;d.push(c.trim()),i+=1}const u=d.join(" ");u.startsWith("**")&&u.endsWith("**")&&!u.slice(2,-2).includes("**")?a.push({kind:"emphasis",text:u.slice(2,-2)}):u&&a.push({kind:"paragraph",text:u})}return{title:t,blocks:a,references:o}}function Ba(n,e,t){const a=[];let o=e;for(;o<n.length;){const i=n[o];if(t(i))break;if(!i.trim()){o+=1;continue}if(i.trim()==="---"){a.push({kind:"rule"}),o+=1;continue}if(i.startsWith("> ")){const s=[];for(;o<n.length&&n[o].startsWith("> ");)s.push(n[o].slice(2).trim()),o+=1;a.push({kind:"quote",text:s.join(" ")});continue}if(/^\d+\.\s/.test(i)){const s=[];for(;o<n.length&&/^\d+\.\s/.test(n[o]);)s.push(n[o].replace(/^\d+\.\s+/,"").trim()),o+=1;a.push({kind:"list",items:s,ordered:!0});continue}if(/^-\s/.test(i)){const s=[];for(;o<n.length&&/^-\s/.test(n[o]);)s.push(n[o].replace(/^-\s+/,"").trim()),o+=1;a.push({kind:"list",items:s,ordered:!1});continue}const r=[];for(;o<n.length;){const s=n[o];if(t(s)||!s.trim()||yt(s)&&s.trim()!=="")break;r.push(s.trim()),o+=1}const l=r.join(" ");l.startsWith("**")&&l.endsWith("**")&&!l.slice(2,-2).includes("**")?a.push({kind:"emphasis",text:l.slice(2,-2)}):l&&a.push({kind:"paragraph",text:l})}return{blocks:a,nextIndex:o}}function qa(n,e){const t=[];let a=e;for(const o of un){if(mn(n[a]??"")!==o)throw new Error(`Practice component must use ### ${un.join(", then ### ")}`);a+=1;const r=Ba(n,a,l=>!!(l.startsWith("## ")||mn(l)||l.trim()==="---"));t.push({label:o,blocks:r.blocks}),a=r.nextIndex}return{block:{kind:"practice",parts:t},nextIndex:a}}let Ue=null;function Ga(){return Ue||(Ue=Fe(za)),Ue}const ja=`
...
`,Ua="An experiment in dreams, consciousness, energy, and out-of-body experience";function Va(n){const e=n.trim(),t=e.match(/^#\s+(.+)\n+/);if(!t)throw new Error("Guidebook manuscript must begin with a top-level heading");const a=t[1].trim(),i=e.slice(t[0].length).split(/\n## /),r=qn(i[0]??""),l=[];let s=[];for(let d=1;d<i.length;d+=1){const u=i[d],c=u.indexOf(`
`),y=(c===-1?u:u.slice(0,c)).trim(),w=c===-1?"":u.slice(c+1);if(y==="References"){s=$a(w);continue}l.push({heading:y,paragraphs:qn(w)})}return{openingHeading:a,openingParagraphs:r,sections:l,references:s}}function qn(n){return n.split(/\n---\n/).flatMap(e=>e.split(/\n\n+/)).map(e=>e.replace(/\n/g," ").trim()).filter(Boolean)}function $a(n){return n.split(/\n+/).map(e=>e.trim()).filter(Boolean)}let Ve=null;function Ja(){return Ve||(Ve=Va(ja)),Ve}function Gn(n){return n.replace(/[.,;:!?)]+$/u,"")}function De(n,e={}){const{linkCitations:t=!0}=e,a=p("span",{class:"guidebook-rich-text"}),o=t?/(\*\*[^*]+\*\*|\*[^*]+\*|\[\d+\]|https:\/\/\S+|doi:10\.\S+)/gi:/(\*\*[^*]+\*\*|\*[^*]+\*|https:\/\/\S+|doi:10\.\S+)/gi;let i=0;for(const r of n.matchAll(o)){const l=r.index??0;l>i&&a.append(n.slice(i,l));const s=r[0];if(s.startsWith("**"))a.append(p("strong",{},[s.slice(2,-2)]));else if(s.startsWith("*"))a.append(p("em",{},[s.slice(1,-1)]));else if(/^\[\d+\]$/.test(s)&&t){const d=s.slice(1,-1);a.append(p("a",{href:`#ref-${d}`,class:"guidebook-citation"},[s]))}else if(s.toLowerCase().startsWith("doi:10.")){const d=Gn(s.slice(4));a.append(p("a",{href:`https://doi.org/${d}`,class:"guidebook-doi",rel:"noreferrer"},[s]))}else if(s.toLowerCase().startsWith("https://")){const d=Gn(s);a.append(p("a",{href:d,class:"guidebook-external",rel:"noreferrer"},[s]))}else a.append(s);i=l+s.length}return i<n.length&&a.append(n.slice(i)),a.childNodes.length||a.append(n),a}function pn(n,e){const t=p("p",{});return t.append(De(n,e)),t}function ft(n){const e=n.match(/^\[(\d+)\]\s*(.*)$/),t=e?.[1]??"0",a=e?.[2]??n,o=p("li",{id:`ref-${t}`,class:"guidebook-reference-item"}),i=p("span",{class:"guidebook-ref-num"},[`[${t}] `]);return o.append(i,De(a,{linkCitations:!1})),o.tabIndex=-1,o}function Ka(n){for(const e of n.querySelectorAll("a.guidebook-citation"))e.addEventListener("click",t=>{const o=(e.getAttribute("href")??"").match(/^#ref-(\d+)$/);if(!o)return;const i=n.querySelector(`#ref-${o[1]}`);if(!i)return;t.preventDefault();const r=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;i.scrollIntoView({behavior:r?"auto":"smooth",block:"start"}),i.focus({preventScroll:!0})})}function Ee(n){return p("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function Qa(n){const e=Ja(),t=p("article",{class:"guidebook-article"});let a=Ee(!0);t.append(a),a.append(p("p",{class:"guidebook-subtitle"},[Ua]),p("h1",{class:"guidebook-opening-title"},[e.openingHeading]),...e.openingParagraphs.map(o=>pn(o)));for(const o of e.sections){a=Ee(!1),t.append(a),a.append(p("h2",{class:"guidebook-section-title"},[o.heading]));for(const i of o.paragraphs){if(i.startsWith("**")&&i.endsWith("**")){a.append(p("p",{class:"guidebook-emphasis-line"},[i.slice(2,-2)]));continue}a.append(pn(i))}}if(e.references.length){a=Ee(!1),t.append(a),a.append(p("h2",{class:"guidebook-section-title",id:"references"},["References"]));const o=p("ul",{class:"guidebook-references"});for(const i of e.references)o.append(ft(i));a.append(o)}a=Ee(!1),t.append(a),a.append(p("p",{class:"guidebook-next"},[p("a",{href:Ya,class:"guidebook-next-link",id:"guidebook-next-chapter"},[gt,p("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])])),n.append(t)}function me(n){return p("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function bt(n,e){for(const t of e){if(t.kind==="emphasis"){n.append(p("p",{class:"guidebook-emphasis-line"},[t.text]));continue}if(t.kind==="quote"){const a=p("blockquote",{class:"guidebook-pull"});a.append(De(t.text)),n.append(a);continue}if(t.kind==="list"){const a=p(t.ordered?"ol":"ul",{class:"guidebook-steps"});for(const o of t.items){const i=p("li",{});i.append(De(o)),a.append(i)}n.append(a);continue}if(t.kind==="rule"){n.append(p("hr",{class:"guidebook-rule"}));continue}t.kind==="heading"||t.kind==="practice"||n.append(pn(t.text))}}function Xa(n){const e=p("section",{class:"guidebook-practice-part"});return e.append(p("h2",{class:"guidebook-practice-label"},[n.label])),bt(e,n.blocks),e}function Re(n,e,t){const a=p("article",{class:"guidebook-article guidebook-chapter"});let o=me(!0);a.append(o),o.append(p("h1",{class:"guidebook-chapter-title"},[e.title]));for(const i of e.blocks){if(i.kind==="heading"){o=me(!1),a.append(o);const r=p("h2",{class:"guidebook-section-title"},[i.text]);i.text==="References"&&(r.id="references"),o.append(r);continue}if(i.kind==="practice"){o=me(!1),a.append(o);const r=p("aside",{class:"guidebook-practice","aria-label":"Summary, experiment, and intention"});for(const l of i.parts)r.append(Xa(l));o.append(r);continue}bt(o,[i])}if(e.references.length){o=me(!1),a.append(o),o.append(p("h2",{class:"guidebook-section-title",id:"references"},["References"]));const i=p("ul",{class:"guidebook-references"});for(const r of e.references)i.append(ft(r));o.append(i)}t&&(o=me(!1),a.append(o),o.append(p("p",{class:"guidebook-next"},[p("a",{href:t.href,class:"guidebook-next-link",id:t.id},[t.title,p("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])]))),n.append(a)}const Za=`# You Are Dreaming. Notice.
...
`,wt="You Are Dreaming. Notice.",In="/you-are-dreaming-notice",eo=`#${In}`;let $e=null;function no(){return $e||($e=Fe(Za)),$e}const vt=`# You Are Dreaming. Recognize.
...
`,Tn="You Are Dreaming. Recognize.",kn="/you-are-dreaming-recognize",to=`#${kn}`,ao="PEX-IMPLEMENTATION-PLACEHOLDER";let Je=null;function oo(){const n=vt.trim();return!n||n.includes(ao)?!1:n.startsWith(`# ${Tn}`)}function io(){if(!oo())throw new Error("Chapter 3 manuscript has not been supplied by PEX_PRIMARY");return Je||(Je=Fe(vt)),Je}const ro=`# Feel the Body.
...
`,It="Feel the Body.",An="/feel-the-body",so=`#${An}`;let Ke=null;function lo(){return Ke||(Ke=Fe(ro)),Ke}function I(n,e){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,o]of Object.entries(e))t.setAttribute(a,o);return t}function ho(n){const e=p("div",{class:`pex-ambient pex-ambient-${n}`,"aria-hidden":"true"}),t=I("svg",{viewBox:"0 0 1200 900",focusable:"false"}),a=I("g",{class:"pex-ambient-spin pex-ambient-spin-slow"});a.append(I("circle",{class:"pex-ambient-arc",cx:"620",cy:"430",r:"318"}),I("circle",{class:"pex-ambient-arc pex-ambient-arc-faint",cx:"620",cy:"430",r:"214"}),I("circle",{class:"pex-ambient-point",cx:"938",cy:"430",r:"2.2"}),I("circle",{class:"pex-ambient-point",cx:"406",cy:"238",r:"1.6"}));const o=I("g",{class:"pex-ambient-spin pex-ambient-spin-mid"});o.append(I("ellipse",{class:"pex-ambient-arc",cx:"580",cy:"400",rx:"430",ry:"168"}),I("path",{class:"pex-ambient-arc pex-ambient-arc-partial",d:"M220 520 C 380 220, 820 180, 1040 470"}),I("circle",{class:"pex-ambient-point",cx:"1010",cy:"400",r:"1.8"}));const i=I("g",{class:"pex-ambient-axis"});if(i.append(I("line",{class:"pex-ambient-line",x1:"600",y1:"40",x2:"600",y2:"860"}),I("line",{class:"pex-ambient-line pex-ambient-line-soft",x1:"80",y1:"390",x2:"1120",y2:"510"})),t.append(i,a,o),n==="memory"){const r=I("g",{class:"pex-ambient-memory-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"430",cy:"360",rx:"92",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"760",cy:"500",rx:"70",ry:"110"}),I("circle",{class:"pex-ambient-form pex-ambient-form-c",cx:"620",cy:"280",r:"36"})),t.append(r)}if(n==="notice"||n==="recognize"){const r=I("g",{class:"pex-ambient-notice-align"});r.append(I("path",{class:"pex-ambient-arc pex-ambient-align-a",d:"M260 300 C 480 220, 700 240, 940 360"}),I("path",{class:"pex-ambient-arc pex-ambient-align-b",d:"M300 620 C 520 420, 760 380, 980 520"}),I("circle",{class:"pex-ambient-point pex-ambient-align-point",cx:"640",cy:"390",r:"2.4"})),n==="recognize"&&r.append(I("circle",{class:"pex-ambient-form pex-ambient-recognize-ring",cx:"640",cy:"390",r:"5.5"})),t.append(r)}if(n==="body"){const r=I("g",{class:"pex-ambient-body-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"620",cy:"318",rx:"36",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"620",cy:"468",rx:"58",ry:"92"}),I("circle",{class:"pex-ambient-point",cx:"620",cy:"390",r:"1.8"})),t.append(r)}return e.append(t),e}const co=[{name:"Aries",glyph:"♈︎"},{name:"Taurus",glyph:"♉︎"},{name:"Gemini",glyph:"♊︎"},{name:"Cancer",glyph:"♋︎"},{name:"Leo",glyph:"♌︎"},{name:"Virgo",glyph:"♍︎"},{name:"Libra",glyph:"♎︎"},{name:"Scorpio",glyph:"♏︎"},{name:"Sagittarius",glyph:"♐︎"},{name:"Capricorn",glyph:"♑︎"},{name:"Aquarius",glyph:"♒︎"},{name:"Pisces",glyph:"♓︎"}];function jn(n){const e=(n%360+360)%360;return co[Math.floor(e/30)%12]}function uo(n){const e=new Intl.DateTimeFormat(void 0,{month:"short",day:"numeric"}).format(n),t=new Intl.DateTimeFormat(void 0,{hour:"numeric",minute:"2-digit"}).format(n),a=new Intl.DateTimeFormat(void 0,{month:"long",day:"numeric",hour:"numeric",minute:"2-digit"}).format(n);return{date:e,time:t,spoken:a}}function Un(n,e,t,a){const o=n==="sun"?`Sun in ${t}`:`Moon in ${t}`;return p("span",{class:"sky-widget-pair",tabindex:"0",role:"img","aria-label":o,"data-sky-body":n},[p("span",{class:"sky-widget-body","aria-hidden":"true"},[e]),p("span",{class:"sky-widget-sign","aria-hidden":"true"},[a]),p("span",{class:"sky-widget-tip","aria-hidden":"true"},[o])])}function mo(){const n=new Date,e=Ha(n),t=jn(e.sun.eclipticLongitudeDeg),a=jn(e.moon.eclipticLongitudeDeg),o=Pa(n),i=o?uo(o):null,r=i?`Sun in ${t.name}. Moon in ${a.name}. Next full moon ${i.spoken}.`:`Sun in ${t.name}. Moon in ${a.name}.`,l=i?p("span",{class:"sky-widget-full"},[p("span",{class:"sky-widget-full-label"},["Full Moon · "]),p("span",{class:"sky-widget-full-when"},[`${i.date} · ${i.time}`])]):p("span",{class:"sky-widget-full"},["Full Moon"]);return p("div",{class:"sky-widget-compact",role:"group","aria-label":r},[Un("sun",`${ut}${Bn}`,t.name,t.glyph),Un("moon",`${mt}${Bn}`,a.name,a.glyph),l])}function po(n){return n==="chapter01"?"memory":n==="chapter02"?"notice":n==="chapter03"?"recognize":n==="chapter04"?"body":"orbit"}function go(n){return n==="chapter01"?`${gt} · Psychical Excursion`:n==="chapter02"?`${wt} · Psychical Excursion`:n==="chapter03"?`${Tn} · Psychical Excursion`:n==="chapter04"?`${It} · Psychical Excursion`:"Psychical Excursion"}function yo(n,e){fn(),document.title=go(e),n.replaceChildren();const t=p("a",{class:"skip-link",href:"#main"},["Skip to content"]),a=po(e),o=ho(a),i=bo(),r=p("header",{class:"app-header guidebook-header"},[p("a",{href:"#/",class:"brand-link","aria-label":"Psychical Excursion home"},[p("img",{class:"brand-logo brand-logo-light",src:"/brand/pex-logo-primary.svg",alt:"",width:"220",height:"52",decoding:"async"}),p("img",{class:"brand-logo brand-logo-reverse",src:"/brand/pex-logo-primary-reverse.svg",alt:"",width:"220",height:"52",decoding:"async"})]),p("div",{class:"header-tools guidebook-tools"},[mo(),p("span",{class:"guidebook-tools-rule","aria-hidden":"true"}),i])]),l=p("div",{id:"live-status",class:"visually-hidden","aria-live":"polite"}),s=p("main",{id:"main",class:"main-stage guidebook-main",tabindex:"-1","data-guidebook-page":e}),d=p("footer",{class:"site-footer guidebook-footer"},[p("p",{class:"attribution"},[Pt("Website by "),p("a",{href:"https://hoopsnakedesigns.com/",rel:"noreferrer"},["Hoopsnake Designs"])])]),u=p("div",{class:"app-frame guidebook-frame","data-ambient":a},[r,l,s,d]);return n.append(t,o,u),{main:s}}function fo(n){Ka(n)}function bo(){const n=yn()==="bedtime",e=p("button",{type:"button",id:"theme-light-dark",class:"theme-switch",role:"switch",dir:"ltr","aria-checked":n?"true":"false","aria-label":"Dark appearance"},[p("span",{class:"theme-switch-track","aria-hidden":"true",dir:"ltr"},[p("span",{class:"theme-switch-thumb"})])]);return e.addEventListener("click",()=>{const a=Ct()==="bedtime";e.setAttribute("aria-checked",a?"true":"false")}),e}function Tt(n=window.location.hash){const e=n.replace(/^#/,"").split("?")[0]||"/";return e.startsWith("/")?e:`/${e}`}function wo(n=window.location.hash){const e=Tt(n);return e===vn?"chapter01":e===In?"chapter02":e===kn?"chapter03":e===An?"chapter04":"home"}function vo(){const n=Tt();n==="/"||n===vn||n===In||n===kn||n===An||window.history.replaceState(null,"",`${window.location.pathname}${window.location.search}#/`)}let kt=null;function Io(n){return kt!==n}function To(n){kt=n}function Qe(){typeof history<"u"&&"scrollRestoration"in history&&(history.scrollRestoration="manual"),document.documentElement.scrollTop=0,document.body.scrollTop=0;const n=navigator.userAgent??"";typeof window.scrollTo=="function"&&n.length>0&&!n.includes("jsdom")&&window.scrollTo(0,0)}const gn=[];function At(){for(;gn.length;)gn.pop()?.()}function ko(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function Ao(n){At();const e=[...n.querySelectorAll(".pex-reveal")];if(e.length===0)return;if(ko()||typeof IntersectionObserver!="function"){for(const a of e)a.classList.add("is-visible");return}e[0]?.classList.add("is-visible");const t=new IntersectionObserver(a=>{for(const o of a)o.isIntersecting&&(o.target.classList.add("is-visible"),t.unobserve(o.target))},{threshold:.08,rootMargin:"0px 0px -10% 0px"});for(const a of e)t.observe(a);gn.push(()=>t.disconnect())}const Vn=[];function So(){for(;Vn.length;)Vn.pop()?.()}async function Eo(n){{vo(),Ft(),Wa(),So(),At();const e=wo(),t=Io(e);t&&Qe(),To(e);const{main:a}=yo(n,e);e==="chapter01"?Re(a,Ga(),{href:eo,title:wt,id:"guidebook-next-chapter-2"}):e==="chapter02"?Re(a,no(),{href:to,title:Tn,id:"guidebook-next-chapter-3"}):e==="chapter03"?Re(a,io(),{href:so,title:It,id:"guidebook-next-chapter-4"}):e==="chapter04"?Re(a,lo()):Qa(a),Ao(a),fo(n),t&&(Qe(),requestAnimationFrame(()=>Qe()));return}}function Ro(){const n=document.getElementById("app");if(!n)throw new Error("Missing #app");return n}async function xo(){fn(),Mt();const n=Ro(),e=()=>{Eo(n)};window.addEventListener("hashchange",e),e();try{await $n.open(),await Nt()}catch{}}xo();
