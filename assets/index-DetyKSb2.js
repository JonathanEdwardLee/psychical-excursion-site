...
`,st="You Are Dreaming. Remember.",mn="/you-are-dreaming-remember",Na=`#${mn}`;function lt(n){const e=n.replace(/\r\n/g,`
`).trim().split(`
`);if(!e[0]?.startsWith("# "))throw new Error("Chapter manuscript must begin with a top-level heading");const t=e[0].slice(2).trim(),a=[],o=[];let i=1,r=!1;for(;i<e.length;){const l=e[i];if(r){const h=l.trim();h&&o.push(h),i+=1;continue}if(!l.trim()){i+=1;continue}if(l.trim()==="---"){a.push({kind:"rule"}),i+=1;continue}if(l.startsWith("## ")){const h=l.slice(3).trim();if(h==="References"){r=!0,i+=1;continue}a.push({kind:"heading",text:h}),i+=1;continue}if(l.startsWith("> ")){const h=[];for(;i<e.length&&e[i].startsWith("> ");)h.push(e[i].slice(2).trim()),i+=1;a.push({kind:"quote",text:h.join(" ")});continue}if(/^\d+\.\s/.test(l)){const h=[];for(;i<e.length&&/^\d+\.\s/.test(e[i]);)h.push(e[i].replace(/^\d+\.\s+/,"").trim()),i+=1;a.push({kind:"list",items:h});continue}const s=[];for(;i<e.length;){const h=e[i];if(!h.trim()||h.trim()==="---"||h.startsWith("## ")||h.startsWith("> ")||/^\d+\.\s/.test(h))break;s.push(h.trim()),i+=1}const d=s.join(" ");d.startsWith("**")&&d.endsWith("**")&&!d.slice(2,-2).includes("**")?a.push({kind:"emphasis",text:d.slice(2,-2)}):d&&a.push({kind:"paragraph",text:d})}return{title:t,blocks:a,references:o}}let Be=null;function xa(){return Be||(Be=lt(Ma)),Be}const Ca=`
...
`,Oa="An experiment in dreams, consciousness, energy, and out-of-body experience";function Da(n){const e=n.trim(),t=e.match(/^#\s+(.+)\n+/);if(!t)throw new Error("Guidebook manuscript must begin with a top-level heading");const a=t[1].trim(),i=e.slice(t[0].length).split(/\n## /),r=Pn(i[0]??""),l=[];let s=[];for(let d=1;d<i.length;d+=1){const h=i[d],u=h.indexOf(`
`),y=(u===-1?h:h.slice(0,u)).trim(),b=u===-1?"":h.slice(u+1);if(y==="References"){s=Pa(b);continue}l.push({heading:y,paragraphs:Pn(b)})}return{openingHeading:a,openingParagraphs:r,sections:l,references:s}}function Pn(n){return n.split(/\n---\n/).flatMap(e=>e.split(/\n\n+/)).map(e=>e.replace(/\n/g," ").trim()).filter(Boolean)}function Pa(n){return n.split(/\n+/).map(e=>e.trim()).filter(Boolean)}let Ue=null;function Fa(){return Ue||(Ue=Da(Ca)),Ue}function Fn(n){return n.replace(/[.,;:!?)]+$/u,"")}function Ce(n,e={}){const{linkCitations:t=!0}=e,a=m("span",{class:"guidebook-rich-text"}),o=t?/(\*\*[^*]+\*\*|\*[^*]+\*|\[\d+\]|https:\/\/\S+|doi:10\.\S+)/gi:/(\*\*[^*]+\*\*|\*[^*]+\*|https:\/\/\S+|doi:10\.\S+)/gi;let i=0;for(const r of n.matchAll(o)){const l=r.index??0;l>i&&a.append(n.slice(i,l));const s=r[0];if(s.startsWith("**"))a.append(m("strong",{},[s.slice(2,-2)]));else if(s.startsWith("*"))a.append(m("em",{},[s.slice(1,-1)]));else if(/^\[\d+\]$/.test(s)&&t){const d=s.slice(1,-1);a.append(m("a",{href:`#ref-${d}`,class:"guidebook-citation"},[s]))}else if(s.toLowerCase().startsWith("doi:10.")){const d=Fn(s.slice(4));a.append(m("a",{href:`https://doi.org/${d}`,class:"guidebook-doi",rel:"noreferrer"},[s]))}else if(s.toLowerCase().startsWith("https://")){const d=Fn(s);a.append(m("a",{href:d,class:"guidebook-external",rel:"noreferrer"},[s]))}else a.append(s);i=l+s.length}return i<n.length&&a.append(n.slice(i)),a.childNodes.length||a.append(n),a}function ln(n,e){const t=m("p",{});return t.append(Ce(n,e)),t}function ct(n){const e=n.match(/^\[(\d+)\]\s*(.*)$/),t=e?.[1]??"0",a=e?.[2]??n,o=m("li",{id:`ref-${t}`,class:"guidebook-reference-item"}),i=m("span",{class:"guidebook-ref-num"},[`[${t}] `]);return o.append(i,Ce(a,{linkCitations:!1})),o.tabIndex=-1,o}function La(n){for(const e of n.querySelectorAll("a.guidebook-citation"))e.addEventListener("click",t=>{const o=(e.getAttribute("href")??"").match(/^#ref-(\d+)$/);if(!o)return;const i=n.querySelector(`#ref-${o[1]}`);if(!i)return;t.preventDefault();const r=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;i.scrollIntoView({behavior:r?"auto":"smooth",block:"start"}),i.focus({preventScroll:!0})})}function Ee(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function Ha(n){const e=Fa(),t=m("article",{class:"guidebook-article"});let a=Ee(!0);t.append(a),a.append(m("p",{class:"guidebook-subtitle"},[Oa]),m("h1",{class:"guidebook-opening-title"},[e.openingHeading]),...e.openingParagraphs.map(o=>ln(o)));for(const o of e.sections){a=Ee(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title"},[o.heading]));for(const i of o.paragraphs){if(i.startsWith("**")&&i.endsWith("**")){a.append(m("p",{class:"guidebook-emphasis-line"},[i.slice(2,-2)]));continue}a.append(ln(i))}}if(e.references.length){a=Ee(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const o=m("ul",{class:"guidebook-references"});for(const i of e.references)o.append(ct(i));a.append(o)}a=Ee(!1),t.append(a),a.append(m("p",{class:"guidebook-next"},[m("a",{href:Na,class:"guidebook-next-link",id:"guidebook-next-chapter"},[st,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])])),n.append(t)}function Se(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function Ln(n,e,t){const a=m("article",{class:"guidebook-article guidebook-chapter"});let o=Se(!0);a.append(o),o.append(m("h1",{class:"guidebook-chapter-title"},[e.title]));for(const i of e.blocks){if(i.kind==="heading"){o=Se(!1),a.append(o);const r=m("h2",{class:"guidebook-section-title"},[i.text]);i.text==="References"&&(r.id="references"),o.append(r);continue}if(i.kind==="emphasis"){o.append(m("p",{class:"guidebook-emphasis-line"},[i.text]));continue}if(i.kind==="quote"){const r=m("blockquote",{class:"guidebook-pull"});r.append(Ce(i.text)),o.append(r);continue}if(i.kind==="list"){const r=m("ol",{class:"guidebook-steps"});for(const l of i.items){const s=m("li",{});s.append(Ce(l)),r.append(s)}o.append(r);continue}if(i.kind==="rule"){o.append(m("hr",{class:"guidebook-rule"}));continue}o.append(ln(i.text))}if(e.references.length){o=Se(!1),a.append(o),o.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const i=m("ul",{class:"guidebook-references"});for(const r of e.references)i.append(ct(r));o.append(i)}t&&(o=Se(!1),a.append(o),o.append(m("p",{class:"guidebook-next"},[m("a",{href:t.href,class:"guidebook-next-link",id:t.id},[t.title,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])]))),n.append(a)}const Wa=`# You Are Dreaming. Notice.
...
`,ht="You Are Dreaming. Notice.",gn="/you-are-dreaming-notice",za=`#${gn}`;let je=null;function Ya(){return je||(je=lt(Wa)),je}function k(n,e){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,o]of Object.entries(e))t.setAttribute(a,o);return t}function _a(n){const e=m("div",{class:`pex-ambient pex-ambient-${n}`,"aria-hidden":"true"}),t=k("svg",{viewBox:"0 0 1200 900",focusable:"false"}),a=k("g",{class:"pex-ambient-spin pex-ambient-spin-slow"});a.append(k("circle",{class:"pex-ambient-arc",cx:"620",cy:"430",r:"318"}),k("circle",{class:"pex-ambient-arc pex-ambient-arc-faint",cx:"620",cy:"430",r:"214"}),k("circle",{class:"pex-ambient-point",cx:"938",cy:"430",r:"2.2"}),k("circle",{class:"pex-ambient-point",cx:"406",cy:"238",r:"1.6"}));const o=k("g",{class:"pex-ambient-spin pex-ambient-spin-mid"});o.append(k("ellipse",{class:"pex-ambient-arc",cx:"580",cy:"400",rx:"430",ry:"168"}),k("path",{class:"pex-ambient-arc pex-ambient-arc-partial",d:"M220 520 C 380 220, 820 180, 1040 470"}),k("circle",{class:"pex-ambient-point",cx:"1010",cy:"400",r:"1.8"}));const i=k("g",{class:"pex-ambient-axis"});if(i.append(k("line",{class:"pex-ambient-line",x1:"600",y1:"40",x2:"600",y2:"860"}),k("line",{class:"pex-ambient-line pex-ambient-line-soft",x1:"80",y1:"390",x2:"1120",y2:"510"})),t.append(i,a,o),n==="memory"){const r=k("g",{class:"pex-ambient-memory-forms"});r.append(k("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"430",cy:"360",rx:"92",ry:"48"}),k("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"760",cy:"500",rx:"70",ry:"110"}),k("circle",{class:"pex-ambient-form pex-ambient-form-c",cx:"620",cy:"280",r:"36"})),t.append(r)}if(n==="notice"){const r=k("g",{class:"pex-ambient-notice-align"});r.append(k("path",{class:"pex-ambient-arc pex-ambient-align-a",d:"M260 300 C 480 220, 700 240, 940 360"}),k("path",{class:"pex-ambient-arc pex-ambient-align-b",d:"M300 620 C 520 420, 760 380, 980 520"}),k("circle",{class:"pex-ambient-point pex-ambient-align-point",cx:"640",cy:"390",r:"2.4"})),t.append(r)}return e.append(t),e}const Ga=[{name:"Aries",glyph:"♈︎"},{name:"Taurus",glyph:"♉︎"},{name:"Gemini",glyph:"♊︎"},{name:"Cancer",glyph:"♋︎"},{name:"Leo",glyph:"♌︎"},{name:"Virgo",glyph:"♍︎"},{name:"Libra",glyph:"♎︎"},{name:"Scorpio",glyph:"♏︎"},{name:"Sagittarius",glyph:"♐︎"},{name:"Capricorn",glyph:"♑︎"},{name:"Aquarius",glyph:"♒︎"},{name:"Pisces",glyph:"♓︎"}];function Hn(n){const e=(n%360+360)%360;return Ga[Math.floor(e/30)%12]}function qa(n){const e=new Intl.DateTimeFormat(void 0,{month:"short",day:"numeric"}).format(n),t=new Intl.DateTimeFormat(void 0,{hour:"numeric",minute:"2-digit"}).format(n),a=new Intl.DateTimeFormat(void 0,{month:"long",day:"numeric",hour:"numeric",minute:"2-digit"}).format(n);return{date:e,time:t,spoken:a}}function Wn(n,e,t,a){const o=n==="sun"?`Sun in ${t}`:`Moon in ${t}`;return m("span",{class:"sky-widget-pair",tabindex:"0",role:"img","aria-label":o,"data-sky-body":n},[m("span",{class:"sky-widget-body","aria-hidden":"true"},[e]),m("span",{class:"sky-widget-sign","aria-hidden":"true"},[a]),m("span",{class:"sky-widget-tip","aria-hidden":"true"},[o])])}function Ba(){const n=new Date,e=Sa(n),t=Hn(e.sun.eclipticLongitudeDeg),a=Hn(e.moon.eclipticLongitudeDeg),o=Aa(n),i=o?qa(o):null,r=i?`Sun in ${t.name}. Moon in ${a.name}. Next full moon ${i.spoken}.`:`Sun in ${t.name}. Moon in ${a.name}.`,l=i?m("span",{class:"sky-widget-full"},[m("span",{class:"sky-widget-full-label"},["Full Moon · "]),m("span",{class:"sky-widget-full-when"},[`${i.date} · ${i.time}`])]):m("span",{class:"sky-widget-full"},["Full Moon"]);return m("div",{class:"sky-widget-compact",role:"group","aria-label":r},[Wn("sun",`${ot}${Dn}`,t.name,t.glyph),Wn("moon",`${it}${Dn}`,a.name,a.glyph),l])}function Ua(n){return n==="chapter01"?"memory":n==="chapter02"?"notice":"orbit"}function ja(n){return n==="chapter01"?`${st} · Psychical Excursion`:n==="chapter02"?`${ht} · Psychical Excursion`:"Psychical Excursion"}function Va(n,e){dn(),document.title=ja(e),n.replaceChildren();const t=m("a",{class:"skip-link",href:"#main"},["Skip to content"]),a=Ua(e),o=_a(a),i=Ja(),r=m("header",{class:"app-header guidebook-header"},[m("a",{href:"#/",class:"brand-link","aria-label":"Psychical Excursion home"},[m("img",{class:"brand-logo brand-logo-light",src:"/brand/pex-logo-primary.svg",alt:"",width:"220",height:"52",decoding:"async"}),m("img",{class:"brand-logo brand-logo-reverse",src:"/brand/pex-logo-primary-reverse.svg",alt:"",width:"220",height:"52",decoding:"async"})]),m("div",{class:"header-tools guidebook-tools"},[Ba(),m("span",{class:"guidebook-tools-rule","aria-hidden":"true"}),i])]),l=m("div",{id:"live-status",class:"visually-hidden","aria-live":"polite"}),s=m("main",{id:"main",class:"main-stage guidebook-main",tabindex:"-1","data-guidebook-page":e}),d=m("footer",{class:"site-footer guidebook-footer"},[m("p",{class:"attribution"},[At("Website by "),m("a",{href:"https://hoopsnakedesigns.com/",rel:"noreferrer"},["Hoopsnake Designs"])])]),h=m("div",{class:"app-frame guidebook-frame","data-ambient":a},[r,l,s,d]);return n.append(t,o,h),{main:s}}function $a(n){La(n)}function Ja(){const n=hn()==="bedtime",e=m("button",{type:"button",id:"theme-light-dark",class:"theme-switch",role:"switch",dir:"ltr","aria-checked":n?"true":"false","aria-label":"Dark appearance"},[m("span",{class:"theme-switch-track","aria-hidden":"true",dir:"ltr"},[m("span",{class:"theme-switch-thumb"})])]);return e.addEventListener("click",()=>{const a=vt()==="bedtime";e.setAttribute("aria-checked",a?"true":"false")}),e}function dt(n=window.location.hash){const e=n.replace(/^#/,"").split("?")[0]||"/";return e.startsWith("/")?e:`/${e}`}function Ka(n=window.location.hash){const e=dt(n);return e===mn?"chapter01":e===gn?"chapter02":"home"}function Qa(){const n=dt();n==="/"||n===mn||n===gn||window.history.replaceState(null,"",`${window.location.pathname}${window.location.search}#/`)}let ut=null;function Xa(n){return ut!==n}function Za(n){ut=n}function Ve(){typeof history<"u"&&"scrollRestoration"in history&&(history.scrollRestoration="manual"),document.documentElement.scrollTop=0,document.body.scrollTop=0,typeof window.scrollTo=="function"&&!navigator.userAgent?.includes("jsdom")&&window.scrollTo(0,0)}const cn=[];function pt(){for(;cn.length;)cn.pop()?.()}function e0(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function n0(n){pt();const e=[...n.querySelectorAll(".pex-reveal")];if(e.length===0)return;if(e0()||typeof IntersectionObserver!="function"){for(const a of e)a.classList.add("is-visible");return}e[0]?.classList.add("is-visible");const t=new IntersectionObserver(a=>{for(const o of a)o.isIntersecting&&(o.target.classList.add("is-visible"),t.unobserve(o.target))},{threshold:.08,rootMargin:"0px 0px -10% 0px"});for(const a of e)t.observe(a);cn.push(()=>t.disconnect())}const zn=[];function t0(){for(;zn.length;)zn.pop()?.()}async function a0(n){{Qa(),kt(),Ra(),t0(),pt();const e=Ka(),t=Xa(e);t&&Ve(),Za(e);const{main:a}=Va(n,e);e==="chapter01"?Ln(a,xa(),{href:za,title:ht,id:"guidebook-next-chapter-2"}):e==="chapter02"?Ln(a,Ya()):Ha(a),n0(a),$a(n),t&&(Ve(),requestAnimationFrame(()=>Ve()));return}}function o0(){const n=document.getElementById("app");if(!n)throw new Error("Missing #app");return n}async function i0(){dn(),wt();const n=o0(),e=()=>{a0(n)};window.addEventListener("hashchange",e),e();try{await Yn.open(),await bt()}catch{}}i0();
