const yt="0.6.0";const ro="pex-local";const y={entries:"entries",media:"media",progress:"progress",settings:"settings"};function oe(n){const e=typeof n.updatedAt=="number"?n.updatedAt:n.createdAt;return{id:n.id,type:n.type,createdAt:n.createdAt,updatedAt:e,note:typeof n.note=="string"?n.note:"",audioId:n.audioId??null,audioMimeType:n.audioMimeType??null,audioByteLength:n.audioByteLength??null,syncState:n.syncState??"LOCAL",localSafeAt:n.localSafeAt??e,syncVersion:typeof n.syncVersion=="number"?n.syncVersion:1,remoteVersion:n.remoteVersion??null,remoteFileId:n.remoteFileId??null,remoteMediaFileId:n.remoteMediaFileId??null,syncErrorCode:n.syncErrorCode??null,pexDay:n.pexDay??null,phaseId:n.phaseId??null}}class U extends Error{code;recoverable;constructor(e,t,a=!0){super(t),this.name="AppError",this.code=e,this.recoverable=a}}function tn(n){return{day:n,unlocked:!0,visitedAt:null,completedAt:null}}function Me(n){return{day:n.day,unlocked:!0,visitedAt:typeof n.visitedAt=="number"?n.visitedAt:null,completedAt:typeof n.completedAt=="number"?n.completedAt:null}}function O(n){return new Promise((e,t)=>{n.onsuccess=()=>e(n.result),n.onerror=()=>t(n.error??new Error("indexeddb-request-failed"))})}function L(n){return new Promise((e,t)=>{n.oncomplete=()=>e(),n.onerror=()=>t(n.error??new Error("indexeddb-transaction-failed")),n.onabort=()=>t(n.error??new Error("indexeddb-transaction-aborted"))})}function be(n){const e=n instanceof DOMException?n.name:"",t=n instanceof Error?n.message:String(n);return e==="QuotaExceededError"||/quota/i.test(t)?new U("quota-exceeded","This browser does not have enough space to save. Export existing entries if you can, then free space and retry."):new U("save-failed","The local save did not complete. Your entry was not discarded silently; retry the save.")}class so{constructor(e=globalThis.indexedDB??null){this.factory=e}factory;assertAvailable(){if(!this.factory)throw new U("indexeddb-unavailable","IndexedDB is not available in this browser, so journal entries cannot be stored locally.",!1)}async open(){this.assertAvailable();try{const e=this.factory.open(ro,3);e.onupgradeneeded=o=>{const i=e.result,r=e.transaction;if(i.objectStoreNames.contains(y.entries)){if(o.oldVersion<2){const s=r.objectStore(y.entries);s.indexNames.contains("syncState")||s.createIndex("syncState","syncState")}}else{const s=i.createObjectStore(y.entries,{keyPath:"id"});s.createIndex("createdAt","createdAt"),s.createIndex("type","type"),s.createIndex("syncState","syncState")}i.objectStoreNames.contains(y.media)||i.createObjectStore(y.media,{keyPath:"id"}).createIndex("entryId","entryId",{unique:!0}),i.objectStoreNames.contains(y.progress)||i.createObjectStore(y.progress,{keyPath:"day"}),i.objectStoreNames.contains(y.settings)||i.createObjectStore(y.settings,{keyPath:"key"})};const t=await O(e);return await this.ensureSeed(t)&&(await this.migrateLegacyEntries(t),await this.bumpSchemaRecord(t)),t}catch(e){throw e instanceof U?e:new U("indexeddb-open-failed","The local database could not be opened.",!1)}}async ensureSeed(e){const t=e.transaction([y.progress,y.settings],"readwrite"),a=t.objectStore(y.progress),o=t.objectStore(y.settings);for(let s=1;s<=60;s+=1){const l=await O(a.get(s));l?a.put(Me(l)):a.put(tn(s))}const i=await O(o.get("schema"));let r=!1;return i?i.schemaVersion<3&&(r=!0):o.put({key:"schema",schemaVersion:3,appVersion:yt,createdAt:Date.now()}),await L(t),r}async bumpSchemaRecord(e){const t=e.transaction(y.settings,"readwrite"),a=t.objectStore(y.settings),o=await O(a.get("schema"));o&&a.put({...o,schemaVersion:3,appVersion:yt}),await L(t)}async migrateLegacyEntries(e){const t=e.transaction(y.entries,"readwrite"),a=t.objectStore(y.entries),o=await O(a.getAll());for(const i of o)i.syncState===void 0&&a.put(oe(i));await L(t)}async listEntries(){const e=await this.open();try{const t=e.transaction(y.entries,"readonly"),a=t.objectStore(y.entries),o=await O(a.getAll());return await L(t),o.map(i=>oe(i)).sort((i,r)=>r.createdAt-i.createdAt)}finally{e.close()}}async getEntry(e){const t=await this.open();try{const a=t.transaction([y.entries,y.media],"readonly"),o=await O(a.objectStore(y.entries).get(e));if(!o)return await L(a),null;const i=oe(o);let r=null;return i.audioId&&(r=await O(a.objectStore(y.media).get(i.audioId))??null),await L(a),{entry:i,media:r}}finally{t.close()}}async saveCapture(e){const t=await this.open(),a=e.createdAt,o=oe({id:e.id,type:e.type,createdAt:a,updatedAt:a,note:e.note.trim(),audioId:e.audio?.id??null,audioMimeType:e.audio?.mimeType??null,audioByteLength:e.audio?e.audio.blob.size:null,syncState:e.syncState??"LOCAL",localSafeAt:a,syncVersion:e.syncVersion??1,remoteVersion:e.remoteVersion??null,remoteFileId:e.remoteFileId??null,remoteMediaFileId:e.remoteMediaFileId??null,syncErrorCode:null,pexDay:e.pexDay??null,phaseId:e.phaseId??null});try{const i=t.transaction(e.audio?[y.entries,y.media]:y.entries,"readwrite");if(i.objectStore(y.entries).put(o),e.audio){const r={id:e.audio.id,entryId:o.id,mimeType:e.audio.mimeType,blob:e.audio.blob,byteLength:e.audio.blob.size,createdAt:e.createdAt};i.objectStore(y.media).put(r)}return await L(i),o}catch(i){throw be(i)}finally{t.close()}}async updateNote(e,t){const a=await this.open();try{const o=a.transaction(y.entries,"readwrite"),i=o.objectStore(y.entries),r=await O(i.get(e));if(!r)throw new U("save-failed","That entry is no longer in local storage.");const s=oe(r),l=oe({...s,note:t.trim(),updatedAt:Date.now(),syncVersion:s.syncVersion+1,syncState:s.syncState==="SYNCED"?"PENDING_SYNC":s.syncState});return i.put(l),await L(o),l}catch(o){throw o instanceof U?o:be(o)}finally{a.close()}}async updateSyncState(e,t){const a=await this.open();try{const o=a.transaction(y.entries,"readwrite"),i=o.objectStore(y.entries),r=await O(i.get(e));if(!r)throw new U("save-failed","That entry is no longer in local storage.");const s=oe({...r,...t,updatedAt:Date.now()});return i.put(s),await L(o),s}catch(o){throw o instanceof U?o:be(o)}finally{a.close()}}async deleteEntry(e){const t=await this.open();try{const a=t.transaction([y.entries,y.media],"readwrite"),o=await O(a.objectStore(y.entries).get(e));o?.audioId&&a.objectStore(y.media).delete(o.audioId),a.objectStore(y.entries).delete(e),await L(a)}catch{throw new U("delete-failed","The entry could not be deleted. It should still be present in the journal.")}finally{t.close()}}async listProgress(){const e=await this.open();try{const t=e.transaction(y.progress,"readonly"),a=await O(t.objectStore(y.progress).getAll());return await L(t),a.map(o=>Me(o)).sort((o,i)=>o.day-i.day)}finally{e.close()}}async getDayProgress(e){const t=await this.open();try{const a=t.transaction(y.progress,"readonly"),o=await O(a.objectStore(y.progress).get(e));return await L(a),o?Me(o):tn(e)}finally{t.close()}}async writeDayProgress(e,t){const a=await this.open();try{const o=a.transaction([y.progress,y.settings],"readwrite"),i=o.objectStore(y.progress),r=Me(await O(i.get(e))??tn(e)),s=t(r);return i.put(s),o.objectStore(y.settings).put({key:"resumeDay",value:e}),await L(o),s}catch(o){throw be(o)}finally{a.close()}}async markDayVisited(e){return this.writeDayProgress(e,t=>({...t,unlocked:!0,visitedAt:Date.now()}))}async completeDay(e){return this.writeDayProgress(e,t=>({...t,unlocked:!0,visitedAt:t.visitedAt??Date.now(),completedAt:Date.now()}))}async undoDayCompletion(e){return this.writeDayProgress(e,t=>({...t,unlocked:!0,completedAt:null}))}async loadResumeDay(){return this.getSetting("resumeDay")}async getSetting(e){const t=await this.open();try{const a=t.transaction(y.settings,"readonly"),o=await O(a.objectStore(y.settings).get(e));return await L(a),o?.value}finally{t.close()}}async setSetting(e,t){const a=await this.open();try{const o=a.transaction(y.settings,"readwrite");o.objectStore(y.settings).put({key:e,value:t}),await L(o)}catch(o){throw be(o)}finally{a.close()}}async getSchemaInfo(){const e=await this.open();try{const t=e.transaction(y.settings,"readonly"),a=await O(t.objectStore(y.settings).get("schema"));return await L(t),a}finally{e.close()}}async savePersistenceReport(e){await this.setSetting("persistenceReport",e)}async loadPersistenceReport(){return this.getSetting("persistenceReport")}async exportBundle(){const e=await this.open();try{const t=e.transaction([y.entries,y.media],"readonly"),a=await O(t.objectStore(y.entries).getAll()),o=await O(t.objectStore(y.media).getAll());return await L(t),{entries:a.sort((i,r)=>i.createdAt-r.createdAt),media:o}}finally{e.close()}}}const va=new so;function lo(){typeof window>"u"||"serviceWorker"in navigator&&window.addEventListener("load",()=>{navigator.serviceWorker.register("/sw.js").then(n=>{n.addEventListener("updatefound",()=>{const e=n.installing;e&&e.addEventListener("statechange",()=>{e.state==="installed"&&navigator.serviceWorker.controller&&document.dispatchEvent(new CustomEvent("pex-sw-update"))})})})})}async function ho(n=navigator.storage){const e=[],t=!!n,a=typeof n?.persist=="function";let o=null,i=!1,r=null,s=null,l=null;if(!t)e.push("navigator.storage is not present. Persistence cannot be requested in this browser.");else{if(typeof n?.persisted=="function")try{o=await n.persisted()}catch{e.push("storage.persisted() threw; treated as unknown.")}else e.push("storage.persisted() is not present.");if(typeof n?.estimate=="function")try{const d=await n.estimate();s=typeof d.usage=="number"?d.usage:null,l=typeof d.quota=="number"?d.quota:null}catch{e.push("storage.estimate() threw.")}else e.push("storage.estimate() is not present.");if(a&&o!==!0){i=!0;try{r=await n.persist(),r!==!0&&e.push("persist() returned a non-true result. The UI must not claim persistence was granted.")}catch{r=!1,e.push("persist() threw. Persistence is not claimed.")}}else a?(r=!0,e.push("Storage was already marked persisted before this request.")):e.push("storage.persist() is not present.")}const h={inspectedAt:Date.now(),storageApiPresent:t,persistApiPresent:a,persistedBeforeRequest:o,persistRequestAttempted:i,persistGranted:r,estimateUsageBytes:s,estimateQuotaBytes:l,notes:e};try{await va.savePersistenceReport(h)}catch{e.push("The persistence report could not be stored in IndexedDB.")}return h}const Ta="pex-theme";function Xn(){try{const n=localStorage.getItem(Ta);if(n==="bedtime"||n==="light")return n}catch{}return"light"}function Zn(n=Xn()){const e=document.documentElement;e.dataset.theme=n;const t=n==="bedtime"?"#0c0c0b":"#f7f5ef",a=document.querySelector('meta[name="theme-color"]');a&&a.setAttribute("content",t);try{localStorage.setItem(Ta,n)}catch{}return n}function co(){const n=Xn()==="bedtime"?"light":"bedtime";return Zn(n)}const ka=`# Move Your Attention: Focused Attention and Body Awareness Meditation
...
`,uo=`# Remember Your Dreams: Dream Recall Techniques and Research
...
`,wt="Notice Your Dreams: Dream Awareness, Patterns and Dream Signs",Oo="/dream-awareness-signs/",vt=Oo;let rn=null;function Po(){return rn||(rn=S(No)),rn}const Ma=`# Recognize the Dream: Lucid Dreaming, Reality Checks and Dream Signs
...
A 2023 systematic review of empirical lucid-dream induction research found MILD to be the most effective technique among the methods it reviewed.[4]
...
Research protocols have used awakenings of thirty or sixty minutes.[3]
...
`,Wn="Recognize the Dream: Lucid Dreaming, Reality Checks and Dream Signs",Lo="/lucid-dreaming-reality-checks/",Tt=Lo,Wo="PEX-IMPLEMENTATION-PLACEHOLDER";let sn=null;function Bo(){const n=Ma.trim();return!n||n.includes(Wo)?!1:n.startsWith(`# ${Wn}`)}function Fo(){if(!Bo())throw new Error("Chapter 3 manuscript has not been supplied by PEX_PRIMARY");return sn||(sn=S(Ma)),sn}const zo=`# Feel the Body: Body Scan Meditation and Deep Relaxation
...
`,kt="Feel the Body: Body Scan Meditation and Deep Relaxation",Ho="/body-scan-meditation/",It=Ho;let ln=null;function Yo(){return ln||(ln=S(zo)),ln}const qo=`# Build the Current: Tingling, Energy Sensations and Focused Attention
...
`,xt="Build the Current: Tingling, Energy Sensations and Focused Attention",jo="/energy-sensations-meditation/",At=jo;let hn=null;function _o(){return hn||(hn=S(qo)),hn}const Go=`# Quiet the Mind: Meditation Techniques for Sleep and Dream Awareness
...
`,St="Quiet the Mind: Meditation Techniques for Sleep and Dream Awareness",Uo="/meditation-for-lucid-dreaming/",Et=Uo;let cn=null;function Vo(){return cn||(cn=S(Go)),cn}const Jo=`# See the Image: Visualization, Mental Imagery and Hypnagogic Imagery
...
`,Rt="See the Image: Visualization, Mental Imagery and Hypnagogic Imagery",Ko="/visualization-hypnagogic-imagery/",Mt=Ko;let dn=null;function $o(){return dn||(dn=S(Jo)),dn}const Qo=`# Watch the Edge: Hypnagogia and the Transition Into Lucid Dreaming
...
`,Dt="Watch the Edge: Hypnagogia and the Transition Into Lucid Dreaming",Xo="/hypnagogia-lucid-dreaming/",Ct=Xo;let un=null;function Zo(){return un||(un=S(Qo)),un}const ei=`# Let the Body Sleep: The “Mind Awake, Body Asleep” Route to Lucid Dreaming
...
`,Nt="Let the Body Sleep: The “Mind Awake, Body Asleep” Route to Lucid Dreaming",ni="/mind-awake-body-asleep/",Ot=ni;let mn=null;function ti(){return mn||(mn=S(ei)),mn}const ai=`# Move Without Moving: Motor Imagery, Dream Movement and Sleep-Onset Practice
...
`,Pt="Move Without Moving: Motor Imagery, Dream Movement and Sleep-Onset Practice",oi="/motor-imagery-lucid-dreaming/",Lt=oi;let pn=null;function ii(){return pn||(pn=S(ai)),pn}const ri=`# Feel the Shift: Vibrations, Floating and Out-of-Body Sensations Near Sleep
...
`,Wt="Feel the Shift: Vibrations, Floating and Out-of-Body Sensations Near Sleep",si="/out-of-body-sensations-sleep/",Bt=si;let gn=null;function li(){return gn||(gn=S(ri)),gn}const hi=`# Know the Threshold: How to Recognize When Waking Imagery Becomes a Dream
...
`,Ft="Know the Threshold: How to Recognize When Waking Imagery Becomes a Dream",ci="/entering-a-lucid-dream/",zt=ci;let yn=null;function di(){return yn||(yn=S(hi)),yn}const ui=`# Stabilize the Dream: Lucid Dream Stabilization Techniques and Research
...
`,Ht="Stabilize the Dream: Lucid Dream Stabilization Techniques and Research",mi="/lucid-dream-stabilization/",Yt=mi;let fn=null;function pi(){return fn||(fn=S(ui)),fn}const gi=`# Explore the Dream: Lucid Dream Experiments, Dream Control and Research
...
`,qt="Explore the Dream: Lucid Dream Experiments, Dream Control and Research",yi="/lucid-dream-experiments/",jt=yi;let bn=null;function fi(){return bn||(bn=S(gi)),bn}const bi=`# Loosen the Body: Body Ownership, Self-Location and Out-of-Body Experience
...
`,_t="Loosen the Body: Body Ownership, Self-Location and Out-of-Body Experience",wi="/out-of-body-experience-body-ownership/",Gt=wi;let wn=null;function vi(){return wn||(wn=S(bi)),wn}const Ti=`# Cross the Threshold: Astral Projection, OBE Techniques and Lucid Dreaming
...
`,Ut="Cross the Threshold: Astral Projection, OBE Techniques and Lucid Dreaming",ki="/astral-projection-obe-techniques/",Vt=ki;let vn=null;function Ii(){return vn||(vn=S(Ti)),vn}const xi=`# Test the Experience: Can Lucid Dreams and Out-of-Body Experiences Be Verified?
...
`,Jt="Test the Experience: Can Lucid Dreams and Out-of-Body Experiences Be Verified?",Ai="/testing-out-of-body-experiences/",Kt=Ai;let Tn=null;function Si(){return Tn||(Tn=S(xi)),Tn}const Ei=`# Compare the Maps: Lucid Dreaming vs. Astral Projection, OBE and Sleep Paralysis
...
`,$t="Compare the Maps: Lucid Dreaming vs. Astral Projection, OBE and Sleep Paralysis",Ri="/lucid-dreaming-vs-astral-projection/",Qt=Ri;let kn=null;function Mi(){return kn||(kn=S(Ei)),kn}const Di=`# Floating in Space: Sun, Moon, Planets and the Science of Sleep & Dreams
...
`,Xt="Floating in Space: Sun, Moon, Planets and the Science of Sleep & Dreams",Ci="/sun-moon-planets-sleep-dreams/",Zt=Ci;let In=null;function Ni(){return In||(In=S(Di)),In}const Oi=`# Notice the Coincidence: Synchronicity, Recurring Dreams, Shared Dreams and Pattern Recognition
...
`,ea="Notice the Coincidence: Synchronicity, Recurring Dreams, Shared Dreams and Pattern Recognition",Pi="/synchronicity-recurring-shared-dreams/",na=Pi;let xn=null;function Li(){return xn||(xn=S(Oi)),xn}const Wi=`# Return. Record. Repeat: The Best Bedtime Routine for Lucid Dreaming and Astral Projection
...
`,os="An experiment in dreams, consciousness, energy, and out-of-body experience";function is(n){const e=n.trim(),t=e.match(/^#\s+(.+)\n+/);if(!t)throw new Error("Guidebook manuscript must begin with a top-level heading");const a=t[1].trim(),i=e.slice(t[0].length).split(/\n## /),r=ma(i[0]??""),s=[];let l=[];for(let h=1;h<i.length;h+=1){const d=i[h],c=d.indexOf(`
`),p=(c===-1?d:d.slice(0,c)).trim(),b=c===-1?"":d.slice(c+1);if(p==="References"){l=rs(b);continue}s.push({heading:p,paragraphs:ma(b)})}return{openingHeading:a,openingParagraphs:r,sections:s,references:l}}function ma(n){return n.split(/\n---\n/).flatMap(e=>e.split(/\n\n+/)).map(e=>e.replace(/\n/g," ").trim()).filter(Boolean)}function rs(n){return n.split(/\n+/).map(e=>e.trim()).filter(Boolean)}let Mn=null;function Qa(){return Mn||(Mn=is(as)),Mn}function Dn(n){return n.replace(/[.,;:!?)]+$/u,"")}function je(n,e={}){const{linkCitations:t=!0}=e,a=m("span",{class:"guidebook-rich-text"}),o=String.raw`\[(?:[^\]]+)\]\((?:pex:[a-z0-9-]+|#\/[^)\s]+|\/[a-z0-9-/#]+)\)`,i=t?new RegExp(`(${o}|\\*\\*[^*]+\\*\\*|\\*[^*]+\\*|\\[\\d+\\]|https:\\/\\/\\S+|doi:10\\.\\S+)`,"gi"):new RegExp(`(${o}|\\*\\*[^*]+\\*\\*|\\*[^*]+\\*|https:\\/\\/\\S+|doi:10\\.\\S+)`,"gi");let r=0;for(const s of n.matchAll(i)){const l=s.index??0;l>r&&a.append(n.slice(r,l));const h=s[0],d=h.match(/^\[([^\]]+)\]\((pex:[a-z0-9-]+|#\/[^)\s]+|\/[a-z0-9-/#]+)\)$/i);if(d){const c=d[1],p=d[2],b=p.toLowerCase().startsWith("pex:")?p.slice(4).toLowerCase():"",f=b?fo[b]:void 0,T=f?.href??(p===Nn||p.endsWith(`#${et}`)?Nn:p),D=m("a",{href:T,class:"guidebook-pex-link","data-pex-link":f?.id}),N=c.match(/^\*\*(.+)\*\*$/);N?D.append(m("strong",{},[N[1]])):D.append(c),a.append(D)}else if(h.startsWith("**"))a.append(m("strong",{},[h.slice(2,-2)]));else if(h.startsWith("*"))a.append(m("em",{},[h.slice(1,-1)]));else if(/^\[\d+\]$/.test(h)&&t){const c=h.slice(1,-1);a.append(m("a",{href:`#ref-${c}`,class:"guidebook-citation"},[h]))}else if(h.toLowerCase().startsWith("doi:10.")){const c=Dn(h.slice(4));a.append(m("a",{href:`https://doi.org/${c}`,class:"guidebook-doi",rel:"noreferrer"},[h]))}else if(h.toLowerCase().startsWith("https://doi.org/")){const c=Dn(h);a.append(m("a",{href:c,class:"guidebook-doi",rel:"noreferrer"},[h]))}else if(h.toLowerCase().startsWith("https://")){const c=Dn(h);a.append(m("a",{href:c,class:"guidebook-external",rel:"noreferrer"},[h]))}else a.append(h);r=l+h.length}return r<n.length&&a.append(n.slice(r)),a.childNodes.length||a.append(n),a}function $n(n,e){const t=m("p",{});return t.append(je(n,e)),t}function Xa(n){const e=n.match(/^(?:\*\*)?\[(\d+)\](?:\*\*)?\s*(.*)$/),t=e?.[1]??"0",a=e?.[2]??n,o=m("li",{id:`ref-${t}`,class:"guidebook-reference-item"}),i=m("span",{class:"guidebook-ref-num"},[`[${t}] `]);return o.append(i,je(a,{linkCitations:!1})),o.tabIndex=-1,o}function ss(n){for(const e of n.querySelectorAll("a.guidebook-citation"))e.addEventListener("click",t=>{const o=(e.getAttribute("href")??"").match(/^#ref-(\d+)$/);if(!o)return;const i=n.querySelector(`#ref-${o[1]}`);if(!i)return;t.preventDefault();const r=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;i.scrollIntoView({behavior:r?"auto":"smooth",block:"start"}),i.focus({preventScroll:!0})});for(const e of n.querySelectorAll("a.guidebook-pex-link"))e.addEventListener("click",t=>{const a=e.getAttribute("href")??"";let o="",i=!0;try{const l=new URL(a,window.location.href);o=l.hash.replace(/^#/,"");const h=window.location.pathname.replace(/\/?$/,"/"),d=l.pathname.replace(/\/?$/,"/");i=d===h||d==="/"||a.startsWith("#")}catch{o=a.includes("#")?a.split("#").pop()??"":""}if(!o||o.startsWith("/")||!i)return;const r=n.querySelector(`[id="${o}"]`);if(!r)return;t.preventDefault(),r.closest(".guidebook-section")?.classList.add("is-visible");const s=typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches;r.scrollIntoView({behavior:s?"auto":"smooth",block:"start"}),r.focus({preventScroll:!0})})}function Oe(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function ls(n){const e=Qa(),t=m("article",{class:"guidebook-article"});let a=Oe(!0);t.append(a),a.append(m("p",{class:"guidebook-subtitle"},[os]),m("h1",{class:"guidebook-opening-title"},[Eo]),m("p",{class:"guidebook-opening-heading"},[e.openingHeading]),...e.openingParagraphs.map(o=>$n(o)));for(const o of e.sections){a=Oe(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title"},[o.heading]));for(const i of o.paragraphs){if(i.startsWith("**")&&i.endsWith("**")){a.append(m("p",{class:"guidebook-emphasis-line"},[i.slice(2,-2)]));continue}a.append($n(i))}}if(e.references.length){a=Oe(!1),t.append(a),a.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const o=m("ul",{class:"guidebook-references"});for(const i of e.references)o.append(Xa(i));a.append(o)}a=Oe(!1),t.append(a),a.append(m("p",{class:"guidebook-next"},[m("a",{href:xa,class:"guidebook-next-link",id:"guidebook-next-chapter"},[Ia,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])])),n.append(t)}const hs="My body sleeps. I remain aware. I recognize the transition and calmly enter.",cs="A researched, practical journey through dream recall, attention, lucid dreaming, sleep-edge states, out-of-body experience, verification, strange coincidences, and the question of how far consciousness can go.",ds="Read Psychical Excursion",Pe={innerSeconds:180,midSeconds:240,outerSeconds:300,breatheSeconds:96},_=500,G=500;function v(n,e={}){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,o]of Object.entries(e))t.setAttribute(a,o);return t}function A(n,e){const t=(e-90)*Math.PI/180;return[_+n*Math.cos(t),G+n*Math.sin(t)]}function Za(n,e,t=0){return Array.from({length:n},(a,o)=>A(e,360/n*o+t).join(",")).join(" ")}function pa(n,e,t,a=0){const o=[];for(let i=0;i<n;i+=1)o.push(A(e,360/n*i+a).join(",")),o.push(A(t,360/n*i+180/n+a).join(","));return o.join(" ")}function Cn(n,e,t,a){return[A(t,n),A((e+t)/2,n-a),A(e,n),A((e+t)/2,n+a)].map(o=>o.join(",")).join(" ")}function us(n,e,t,a){return[A(t,n),A(e,n-a),A(e,n+a)].map(o=>o.join(",")).join(" ")}function j(n,e,t,a,o){n.append(v("polygon",{class:o,points:Za(e,t,a)}))}function ms(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function eo(){const n=ms(),e=m("figure",{class:n?"pex-attention-instrument is-static":"pex-attention-instrument"});e.setAttribute("role","img"),e.setAttribute("aria-label","Original Psychical Excursion attention instrument: nested thin-line geometry around a fixed center. Looking toward the center is optional. Seeing no afterimage or mental picture is not required.");const t=v("svg",{viewBox:"0 0 1000 1000",focusable:"false"});t.style.setProperty("--pex-attention-inner",`${Pe.innerSeconds}s`),t.style.setProperty("--pex-attention-mid",`${Pe.midSeconds}s`),t.style.setProperty("--pex-attention-outer",`${Pe.outerSeconds}s`),t.style.setProperty("--pex-attention-breathe",`${Pe.breatheSeconds}s`);const a=v("g",{class:"pex-attention-layer pex-attention-beyond"});a.append(v("polygon",{class:"pex-attention-line pex-attention-faintest",points:Za(36,486,5)})),a.append(v("circle",{class:"pex-attention-line pex-attention-faintest",cx:String(_),cy:String(G),r:"478"}));for(let h=0;h<72;h+=1){const d=h*5,c=A(468,d),p=A(h%6===0?492:484,d);a.append(v("line",{class:"pex-attention-line pex-attention-faintest",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:p[0].toFixed(2),y2:p[1].toFixed(2)}))}const o=v("g",{class:"pex-attention-layer pex-attention-spin-outer"});o.append(v("circle",{class:"pex-attention-line pex-attention-strong",cx:String(_),cy:String(G),r:"438"})),o.append(v("circle",{class:"pex-attention-line",cx:String(_),cy:String(G),r:"418"}));for(let h=0;h<144;h+=1){const d=h*2.5,c=h%12===0,p=h%4===0,b=A(c?398:p?406:410,d),f=A(438,d);o.append(v("line",{class:c?"pex-attention-line pex-attention-strong":"pex-attention-line pex-attention-fine",x1:b[0].toFixed(2),y1:b[1].toFixed(2),x2:f[0].toFixed(2),y2:f[1].toFixed(2)}))}for(let h=0;h<8;h+=1){const d=h*45;o.append(v("polygon",{class:"pex-attention-line",points:Cn(d,442,462,3.2)}));const c=A(h%2===0?454:448,d);o.append(v("circle",{class:"pex-attention-fill pex-attention-fine",cx:c[0].toFixed(2),cy:c[1].toFixed(2),r:h%2===0?"2.1":"1.3"}))}for(const h of[18,138,258])o.append(v("path",{class:"pex-attention-line pex-attention-arc",d:ps(394,h,h+84)}));const i=v("g",{class:"pex-attention-layer pex-attention-spin-mid"});for(const h of[372,344,316,288])i.append(v("circle",{class:"pex-attention-line",cx:String(_),cy:String(G),r:String(h)}));j(i,12,372,0,"pex-attention-line pex-attention-soft"),j(i,12,372,15,"pex-attention-line pex-attention-soft");for(let h=0;h<24;h+=1)i.append(v("polygon",{class:"pex-attention-line",points:Cn(h*15,300,368,5.4)}));for(let h=0;h<36;h+=1)i.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:us(h*10,248,286,3.6)}));for(let h=0;h<24;h+=1){const d=h*15,c=A(132,d),p=A(h%2===0?368:316,d);i.append(v("line",{class:h%2===0?"pex-attention-line":"pex-attention-line pex-attention-fine",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:p[0].toFixed(2),y2:p[1].toFixed(2)}))}const r=v("g",{class:"pex-attention-layer pex-attention-breathe"});j(r,8,236,0,"pex-attention-line pex-attention-strong"),j(r,8,236,22.5,"pex-attention-line"),j(r,6,204,0,"pex-attention-line"),j(r,6,204,30,"pex-attention-line"),j(r,4,176,0,"pex-attention-line pex-attention-soft"),j(r,4,176,45,"pex-attention-line pex-attention-soft"),r.append(v("polygon",{class:"pex-attention-line",points:pa(16,220,148,0)})),r.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:pa(12,190,126,15)}));for(const h of[168,148,128])r.append(v("circle",{class:"pex-attention-line pex-attention-fine",cx:String(_),cy:String(G),r:String(h)}));const s=v("g",{class:"pex-attention-layer pex-attention-spin-inner"});for(let h=0;h<12;h+=1){const[d,c]=A(34,h*30);s.append(v("circle",{class:"pex-attention-line",cx:d.toFixed(2),cy:c.toFixed(2),r:"34"}))}for(let h=0;h<8;h+=1)s.append(v("polygon",{class:"pex-attention-line pex-attention-fine",points:Cn(h*45+22.5,58,108,8)}));j(s,16,96,0,"pex-attention-line pex-attention-fine"),j(s,8,72,0,"pex-attention-line"),s.append(v("circle",{class:"pex-attention-line",cx:String(_),cy:String(G),r:"54"}));for(let h=0;h<32;h+=1){const d=h*11.25,c=A(46,d),p=A(h%4===0?70:62,d);s.append(v("line",{class:"pex-attention-line pex-attention-fine",x1:c[0].toFixed(2),y1:c[1].toFixed(2),x2:p[0].toFixed(2),y2:p[1].toFixed(2)}))}const l=v("g",{class:"pex-attention-layer pex-attention-center"});return l.append(v("circle",{class:"pex-attention-line pex-attention-strong",cx:String(_),cy:String(G),r:"18"})),l.append(v("circle",{class:"pex-attention-line pex-attention-fine",cx:String(_),cy:String(G),r:"8"})),l.append(v("circle",{class:"pex-attention-fill pex-attention-center-point",cx:String(_),cy:String(G),r:"2.2"})),t.append(a,o,i,r,s,l),e.append(t),e}function ps(n,e,t){const a=A(n,e),o=A(n,t),i=t-e>180?1:0;return`M ${a[0].toFixed(2)} ${a[1].toFixed(2)} A ${n} ${n} 0 ${i} 1 ${o[0].toFixed(2)} ${o[1].toFixed(2)}`}function gs(n){const e=eo(),t=m("article",{class:"guidebook-article guidebook-landing"},[m("h1",{class:"visually-hidden"},["Psychical Excursion"]),m("div",{class:"guidebook-landing-mandala"},[e]),m("p",{class:"guidebook-landing-affirmation"},[hs]),m("p",{class:"guidebook-book-entry"},[m("a",{href:Be,class:"guidebook-next-link guidebook-book-entry-link",id:"guidebook-enter-book"},[ds,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])]),m("p",{class:"guidebook-landing-synopsis"},[cs])]);n.append(t)}function de(n){return m("section",{class:n?"guidebook-section pex-reveal is-visible":"guidebook-section pex-reveal"})}function no(n,e,t="guidebook-section-subheading"){for(const a of e){if(a.kind==="emphasis"){n.append(m("p",{class:"guidebook-emphasis-line"},[a.text]));continue}if(a.kind==="quote"){const o=m("blockquote",{class:"guidebook-pull"});o.append(je(a.text)),n.append(o);continue}if(a.kind==="list"){const o=m(a.ordered?"ol":"ul",{class:"guidebook-steps"});for(const i of a.items){const r=m("li",{});r.append(je(i)),o.append(r)}n.append(o);continue}if(a.kind==="rule"){n.append(m("hr",{class:"guidebook-rule"}));continue}if(!(a.kind==="heading"||a.kind==="practice"||a.kind==="attention")){if(a.kind==="subheading"){n.append(m("h3",{class:t},[a.text]));continue}n.append($n(a.text))}}}function ys(n){const e=m("section",{class:"guidebook-practice-part"});return e.append(m("h2",{class:"guidebook-practice-label"},[n.label])),no(e,n.blocks,"guidebook-practice-subheading"),e}function fs(n){return m("nav",{class:"guidebook-prev","aria-label":"Previous reading"},[m("a",{href:n.href,class:"guidebook-prev-link",id:n.id},[m("span",{class:"guidebook-prev-arrow","aria-hidden":"true"},["← "]),n.title])])}function x(n,e,t={}){const a=m("article",{class:"guidebook-article guidebook-chapter"});let o=de(!0);a.append(o),t.previous&&o.append(fs(t.previous)),o.append(m("h1",{class:"guidebook-chapter-title"},[e.title]));for(const i of e.blocks){if(i.kind==="heading"){o=de(!1),a.append(o);const r=m("h2",{class:"guidebook-section-title"},[i.text]),s=yo(i.text);s&&(r.id=s,r.tabIndex=-1),o.append(r);continue}if(i.kind==="practice"){o=de(!1),a.append(o);const r=m("aside",{class:"guidebook-practice","aria-label":"Summary, experiment, and intention"});for(const s of i.parts)r.append(ys(s));o.append(r);continue}if(i.kind==="attention"){o=de(!1),a.append(o),o.append(eo());continue}no(o,[i])}if(e.references.length){o=de(!1),a.append(o),o.append(m("h2",{class:"guidebook-section-title",id:"references"},["References"]));const i=m("ul",{class:"guidebook-references"});for(const r of e.references)i.append(Xa(r));o.append(i)}t.next&&(o=de(!1),a.append(o),o.append(m("p",{class:"guidebook-next"},[m("a",{href:t.next.href,class:"guidebook-next-link",id:t.next.id},[t.next.title,m("span",{class:"guidebook-next-arrow","aria-hidden":"true"},[" →"])])]))),n.append(a)}function I(n,e){const t=document.createElementNS("http://www.w3.org/2000/svg",n);for(const[a,o]of Object.entries(e))t.setAttribute(a,o);return t}function bs(n){const e=m("div",{class:`pex-ambient pex-ambient-${n}`,"aria-hidden":"true"}),t=I("svg",{viewBox:"0 0 1200 900",focusable:"false"}),a=I("g",{class:"pex-ambient-spin pex-ambient-spin-slow"});a.append(I("circle",{class:"pex-ambient-arc",cx:"620",cy:"430",r:"318"}),I("circle",{class:"pex-ambient-arc pex-ambient-arc-faint",cx:"620",cy:"430",r:"214"}),I("circle",{class:"pex-ambient-point",cx:"938",cy:"430",r:"2.2"}),I("circle",{class:"pex-ambient-point",cx:"406",cy:"238",r:"1.6"}));const o=I("g",{class:"pex-ambient-spin pex-ambient-spin-mid"});o.append(I("ellipse",{class:"pex-ambient-arc",cx:"580",cy:"400",rx:"430",ry:"168"}),I("path",{class:"pex-ambient-arc pex-ambient-arc-partial",d:"M220 520 C 380 220, 820 180, 1040 470"}),I("circle",{class:"pex-ambient-point",cx:"1010",cy:"400",r:"1.8"}));const i=I("g",{class:"pex-ambient-axis"});if(i.append(I("line",{class:"pex-ambient-line",x1:"600",y1:"40",x2:"600",y2:"860"}),I("line",{class:"pex-ambient-line pex-ambient-line-soft",x1:"80",y1:"390",x2:"1120",y2:"510"})),t.append(i,a,o),n==="memory"){const r=I("g",{class:"pex-ambient-memory-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"430",cy:"360",rx:"92",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"760",cy:"500",rx:"70",ry:"110"}),I("circle",{class:"pex-ambient-form pex-ambient-form-c",cx:"620",cy:"280",r:"36"})),t.append(r)}if(n==="notice"||n==="recognize"){const r=I("g",{class:"pex-ambient-notice-align"});r.append(I("path",{class:"pex-ambient-arc pex-ambient-align-a",d:"M260 300 C 480 220, 700 240, 940 360"}),I("path",{class:"pex-ambient-arc pex-ambient-align-b",d:"M300 620 C 520 420, 760 380, 980 520"}),I("circle",{class:"pex-ambient-point pex-ambient-align-point",cx:"640",cy:"390",r:"2.4"})),n==="recognize"&&r.append(I("circle",{class:"pex-ambient-form pex-ambient-recognize-ring",cx:"640",cy:"390",r:"5.5"})),t.append(r)}if(n==="body"){const r=I("g",{class:"pex-ambient-body-forms"});r.append(I("ellipse",{class:"pex-ambient-form pex-ambient-form-a",cx:"620",cy:"318",rx:"36",ry:"48"}),I("ellipse",{class:"pex-ambient-form pex-ambient-form-b",cx:"620",cy:"468",rx:"58",ry:"92"}),I("circle",{class:"pex-ambient-point",cx:"620",cy:"390",r:"1.8"})),t.append(r)}return e.append(t),e}const ws=[{name:"Aries",glyph:"♈︎"},{name:"Taurus",glyph:"♉︎"},{name:"Gemini",glyph:"♊︎"},{name:"Cancer",glyph:"♋︎"},{name:"Leo",glyph:"♌︎"},{name:"Virgo",glyph:"♍︎"},{name:"Libra",glyph:"♎︎"},{name:"Scorpio",glyph:"♏︎"},{name:"Sagittarius",glyph:"♐︎"},{name:"Capricorn",glyph:"♑︎"},{name:"Aquarius",glyph:"♒︎"},{name:"Pisces",glyph:"♓︎"}];function ga(n){const e=(n%360+360)%360;return ws[Math.floor(e/30)%12]}function vs(n){const e=new Intl.DateTimeFormat(void 0,{month:"short",day:"numeric"}).format(n),t=new Intl.DateTimeFormat(void 0,{hour:"numeric",minute:"2-digit"}).format(n),a=new Intl.DateTimeFormat(void 0,{month:"long",day:"numeric",hour:"numeric",minute:"2-digit"}).format(n);return{date:e,time:t,spoken:a}}function ya(n,e,t,a){const o=n==="sun"?`Sun in ${t}`:`Moon in ${t}`;return m("span",{class:"sky-widget-pair",tabindex:"0",role:"img","aria-label":o,"data-sky-body":n},[m("span",{class:"sky-widget-body","aria-hidden":"true"},[e]),m("span",{class:"sky-widget-sign","aria-hidden":"true"},[a]),m("span",{class:"sky-widget-tip","aria-hidden":"true"},[o])])}function Ts(){const n=new Date,e=ns(n),t=ga(e.sun.eclipticLongitudeDeg),a=ga(e.moon.eclipticLongitudeDeg),o=Xr(n),i=o?vs(o):null,r=i?`Sun in ${t.name}. Moon in ${a.name}. Next full moon ${i.spoken}.`:`Sun in ${t.name}. Moon in ${a.name}.`,s=i?m("span",{class:"sky-widget-full"},[m("span",{class:"sky-widget-full-label"},["Full Moon · "]),m("span",{class:"sky-widget-full-when"},[`${i.date} · ${i.time}`])]):m("span",{class:"sky-widget-full"},["Full Moon"]);return m("div",{class:"sky-widget-compact",role:"group","aria-label":r},[ya("sun",`${Ja}${ua}`,t.name,t.glyph),ya("moon",`${Ka}${ua}`,a.name,a.glyph),s])}function ks(){return Ge.filter(n=>n.id!=="landing").map((n,e)=>({number:String(e+1).padStart(2,"0"),title:n.id==="home"?Qa().openingHeading:n.title,path:n.path,id:n.id}))}const fa="guidebook-chapter-panel",Is="guidebook-chapter-trigger";let _e=null;function to(){_e?.abort(),_e=null}function xs(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function As(){const n=document.createElementNS("http://www.w3.org/2000/svg","svg");n.setAttribute("viewBox","0 0 24 24"),n.setAttribute("aria-hidden","true"),n.setAttribute("class","guidebook-chapter-icon"),n.setAttribute("focusable","false");const e=document.createElementNS("http://www.w3.org/2000/svg","polygon");e.setAttribute("points","12,0.4 23.6,12 12,23.6 0.4,12"),e.setAttribute("class","guidebook-chapter-icon-diamond");const t=document.createElementNS("http://www.w3.org/2000/svg","polygon");t.setAttribute("points","6.2,6.2 17.8,6.2 17.8,17.8 6.2,17.8"),t.setAttribute("class","guidebook-chapter-icon-square");const a=document.createElementNS("http://www.w3.org/2000/svg","circle");return a.setAttribute("cx","12"),a.setAttribute("cy","12"),a.setAttribute("r","1.1"),a.setAttribute("class","guidebook-chapter-icon-point"),n.append(e,t,a),n}function ba(n,e,t,a){n.classList.toggle("is-open",a),e.setAttribute("aria-expanded",a?"true":"false"),e.setAttribute("aria-label",a?"Close chapter menu":"Open chapter menu"),t.hidden=!a,a&&t.querySelector("[aria-current='page']")?.scrollIntoView({block:"nearest",behavior:xs()?"auto":"smooth"})}function Ss(n){to();const e=Ea(n),t=ks(),a=m("button",{type:"button",id:Is,class:"guidebook-chapter-trigger","aria-expanded":"false","aria-controls":fa,"aria-label":"Open chapter menu"},[As()]),o=m("ol",{class:"guidebook-chapter-list"});for(const h of t){const d=h.path===e?.path,c=m("a",{href:h.path,class:"guidebook-chapter-link",...d?{"aria-current":"page"}:{}},[m("span",{class:"guidebook-chapter-number"},[h.number]),m("span",{class:"guidebook-chapter-menu-title"},[h.title])]);o.append(m("li",{class:"guidebook-chapter-item"},[c]))}const i=m("div",{id:fa,class:"guidebook-chapter-panel",hidden:!0},[o]),r=m("nav",{class:"guidebook-chapter-nav","aria-label":"Book chapters"},[i,a]);_e=new AbortController;const{signal:s}=_e,l=(h=!1)=>{a.getAttribute("aria-expanded")==="true"&&(ba(r,a,i,!1),h&&a.focus())};return a.addEventListener("click",h=>{h.stopPropagation();const d=a.getAttribute("aria-expanded")!=="true";ba(r,a,i,d)},{signal:s}),document.addEventListener("keydown",h=>{h.key==="Escape"&&l(!0)},{signal:s}),document.addEventListener("pointerdown",h=>{const d=h.target;!(d instanceof Node)||r.contains(d)||l()},{signal:s}),r}function Es(n){return n==="chapter01"?"memory":n==="chapter02"?"notice":n==="chapter03"?"recognize":n==="chapter04"?"body":"orbit"}function Rs(n){const e=Ea(n);return e?Do(e.title):"Psychical Excursion"}function Ms(n,e){Zn(),document.title=Rs(e),n.replaceChildren();const t=m("a",{class:"skip-link",href:"#main"},["Skip to content"]),a=Es(e),o=bs(a),i=Cs(),r=m("header",{class:"app-header guidebook-header"},[m("a",{href:We,class:"brand-link","aria-label":"Psychical Excursion home"},[m("img",{class:"brand-logo brand-logo-light",src:"/brand/pex-logo-primary.svg",alt:"",width:"220",height:"52",decoding:"async"}),m("img",{class:"brand-logo brand-logo-reverse",src:"/brand/pex-logo-primary-reverse.svg",alt:"",width:"220",height:"52",decoding:"async"})]),m("div",{class:"header-tools guidebook-tools"},[Ts(),m("span",{class:"guidebook-tools-rule","aria-hidden":"true"}),i])]),s=m("div",{id:"live-status",class:"visually-hidden","aria-live":"polite"}),l=m("main",{id:"main",class:"main-stage guidebook-main",tabindex:"-1","data-guidebook-page":e}),h=m("footer",{class:"site-footer guidebook-footer"},[m("p",{class:"attribution"},[Xi("Website by "),m("a",{href:"https://hoopsnakedesigns.com/",rel:"noreferrer"},["Hoopsnake Designs"])])]),d=m("div",{class:"app-frame guidebook-frame","data-ambient":a},e==="landing"?[r,s,l,h]:[r,s,l,h,Ss(e)]);return n.append(t,o,d),{main:l}}function Ds(n){ss(n)}function Cs(){const n=Xn()==="bedtime",e=m("button",{type:"button",id:"theme-light-dark",class:"theme-switch",role:"switch",dir:"ltr","aria-checked":n?"true":"false","aria-label":"Dark appearance"},[m("span",{class:"theme-switch-track","aria-hidden":"true",dir:"ltr"},[m("span",{class:"theme-switch-thumb"})])]);return e.addEventListener("click",()=>{const a=co()==="bedtime";e.setAttribute("aria-checked",a?"true":"false")}),e}const Qn=[];function ao(){for(;Qn.length;)Qn.pop()?.()}function Ns(){return typeof window.matchMedia=="function"&&window.matchMedia("(prefers-reduced-motion: reduce)").matches}function Os(n){ao();const e=[...n.querySelectorAll(".pex-reveal")];if(e.length===0)return;if(Ns()||typeof IntersectionObserver!="function"){for(const a of e)a.classList.add("is-visible");return}e[0]?.classList.add("is-visible");const t=new IntersectionObserver(a=>{for(const o of a)o.isIntersecting&&(o.target.classList.add("is-visible"),t.unobserve(o.target))},{threshold:.08,rootMargin:"0px 0px -10% 0px"});for(const a of e)t.observe(a);Qn.push(()=>t.disconnect())}const wa=[];function Ps(){for(;wa.length;)wa.pop()?.()}async function Ls(n){{if(Da())return;Zi(),ts(),Ps(),ao(),to();const e=_i(),t=ji(),a=Gi(e);a&&!t&&Sn(),Ui(e);const{main:o}=Ms(n,e),i={href:Be,title:"Introduction",id:"guidebook-prev-home"};if(e==="chapter01"?x(o,ko(),{previous:i,next:{href:vt,title:wt,id:"guidebook-next-chapter-2"}}):e==="chapter02"?x(o,Po(),{previous:{href:wo,title:Ia,id:"guidebook-prev-chapter-1"},next:{href:Tt,title:Wn,id:"guidebook-next-chapter-3"}}):e==="chapter03"?x(o,Fo(),{previous:{href:vt,title:wt,id:"guidebook-prev-chapter-2"},next:{href:It,title:kt,id:"guidebook-next-chapter-4"}}):e==="chapter04"?x(o,Yo(),{previous:{href:Tt,title:Wn,id:"guidebook-prev-chapter-3"},next:Le()?{href:bt,title:Ln,id:"guidebook-next-chapter-5"}:void 0}):e==="chapter05"&&Le()?x(o,Ao(),{previous:{href:It,title:kt,id:"guidebook-prev-chapter-4"},next:{href:At,title:xt,id:"guidebook-next-chapter-6"}}):e==="chapter06"?x(o,_o(),{previous:{href:bt,title:Ln,id:"guidebook-prev-chapter-5"},next:{href:Et,title:St,id:"guidebook-next-chapter-7"}}):e==="chapter07"?x(o,Vo(),{previous:{href:At,title:xt,id:"guidebook-prev-chapter-6"},next:{href:Mt,title:Rt,id:"guidebook-next-chapter-8"}}):e==="chapter08"?x(o,$o(),{previous:{href:Et,title:St,id:"guidebook-prev-chapter-7"},next:{href:Ct,title:Dt,id:"guidebook-next-chapter-9"}}):e==="chapter09"?x(o,Zo(),{previous:{href:Mt,title:Rt,id:"guidebook-prev-chapter-8"},next:{href:Ot,title:Nt,id:"guidebook-next-chapter-10"}}):e==="chapter10"?x(o,ti(),{previous:{href:Ct,title:Dt,id:"guidebook-prev-chapter-9"},next:{href:Lt,title:Pt,id:"guidebook-next-chapter-11"}}):e==="chapter11"?x(o,ii(),{previous:{href:Ot,title:Nt,id:"guidebook-prev-chapter-10"},next:{href:Bt,title:Wt,id:"guidebook-next-chapter-12"}}):e==="chapter12"?x(o,li(),{previous:{href:Lt,title:Pt,id:"guidebook-prev-chapter-11"},next:{href:zt,title:Ft,id:"guidebook-next-chapter-13"}}):e==="chapter13"?x(o,di(),{previous:{href:Bt,title:Wt,id:"guidebook-prev-chapter-12"},next:{href:Yt,title:Ht,id:"guidebook-next-chapter-14"}}):e==="chapter14"?x(o,pi(),{previous:{href:zt,title:Ft,id:"guidebook-prev-chapter-13"},next:{href:jt,title:qt,id:"guidebook-next-chapter-15"}}):e==="chapter15"?x(o,fi(),{previous:{href:Yt,title:Ht,id:"guidebook-prev-chapter-14"},next:{href:Gt,title:_t,id:"guidebook-next-chapter-16"}}):e==="chapter16"?x(o,vi(),{previous:{href:jt,title:qt,id:"guidebook-prev-chapter-15"},next:{href:Vt,title:Ut,id:"guidebook-next-chapter-17"}}):e==="chapter17"?x(o,Ii(),{previous:{href:Gt,title:_t,id:"guidebook-prev-chapter-16"},next:{href:Kt,title:Jt,id:"guidebook-next-chapter-18"}}):e==="chapter18"?x(o,Si(),{previous:{href:Vt,title:Ut,id:"guidebook-prev-chapter-17"},next:{href:Qt,title:$t,id:"guidebook-next-chapter-19"}}):e==="chapter19"?x(o,Mi(),{previous:{href:Kt,title:Jt,id:"guidebook-prev-chapter-18"},next:{href:Zt,title:Xt,id:"guidebook-next-chapter-20"}}):e==="chapter20"?x(o,Ni(),{previous:{href:Qt,title:$t,id:"guidebook-prev-chapter-19"},next:{href:na,title:ea,id:"guidebook-next-chapter-21"}}):e==="chapter21"?x(o,Li(),{previous:{href:Zt,title:Xt,id:"guidebook-prev-chapter-20"},next:{href:zi,title:Bi,id:"guidebook-next-chapter-22"}}):e==="chapter22"?x(o,Hi(),{previous:{href:na,title:ea,id:"guidebook-prev-chapter-21"}}):e==="landing"?gs(o):ls(o),Os(o),Ds(n),Ki(),t)Vi(n,t);else if(a){Sn();const r=navigator.userAgent??"";typeof requestAnimationFrame=="function"&&r.length>0&&!r.includes("jsdom")&&requestAnimationFrame(()=>Sn())}return}}function Ws(){const n=document.getElementById("app");if(!n)throw new Error("Missing #app");return n}async function Bs(){Zn(),lo();const n=Ws(),e=()=>{Ls(n)};if(!Da()){window.addEventListener("popstate",e),window.addEventListener("hashchange",()=>{window.location.hash.startsWith("#/")&&e()}),e();try{await va.open(),await ho()}catch{}}}Bs();
